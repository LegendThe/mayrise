...
// 7/10/03 - Fix custom drag cursor problem
// 8/18/03 - So that multiselections will not be lost when dragging nodes from a multiselected treeview
}
interface
...
begin
   ...
   if Assigned(FOnMouseUp) or (tvoAutoURL in Options) or (tvoToolTips in Options) or
      fcUseThemes(self) or Assigned(FOnMouseMove) then // 4/21/05 - Add OnMouseMove assigned condition
      Node:= GetNodeAt(X, Y) as TfcTreeNode;
