...
begin
  if (Value <> nil) and (Image<>nil) then Image.Clear; // 8/27/03
  SetExtImages(Value, FExtImage);
end;

procedure TfcCustomImageBtn.SetExtImageDown(Value: TComponent);
begin
  if (Value <> nil) and (ImageDown<>nil) then ImageDown.Clear; // 8/27/03
...
begin
  ...
  DestBitmap.SetSizeInternal(s.cx, s.cy);
...
begin
  ...
  begin
    if Value then // 6/10/04 - Uncommented so parent clipping stays enabled
