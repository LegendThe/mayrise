...
// 2/20/05 - Don't allow CacheSize to be 0
// 3/16/05 - Replace above line with this code to fix bug with partially visible last row
// 2/13/06 - OnSelectRecord event
}
interface
...
type
  ...
  TfcDBTreeSelectRecordEvent =
     procedure (TreeView: TfcDBCustomTreeview;
        Selecting: boolean; var Accept: boolean) of object;
  ...
  private
    ...
    FOnSelectRecord : TfcDBTreeSelectRecordEvent;
...
//  published
    ...
    property OnMultiSelectRecord : TfcDBTreeSelectRecordEvent read FOnSelectRecord write FOnSelectRecord;
...
//    Flags: integer;
   ...
   begin
      ...
      // 2/20/05 - Don't let height be negative
      if HorzScrollBar.visible and fcUseThemes(self) and (HorzScrollBar.Top>10) then
      begin
        FPaintBitmap.Height := HorzScrollBar.Top
      end
      else
...
begin
   ...
   else if row>=CacheSize then exit;
...
begin
  ...
  // 2/13/07 - Support OnSelectRecord event
  accept:= True;
...
begin
  ...
  if (NewCacheSize<=0) then NewCacheSize:= 1; // 2/20/05 - If CacheSize of 0, data disappears
