...
begin
  ...
  // 1/24/07 - Do assignment if <=256 colors to avoid blank image problem
  MemDC := GetDC(0);
  if (GetDeviceCaps(MemDC, BITSPIXEL) <= 8) then
     ABitmap.Assign(JPEG);
  ReleaseDC(0, MemDC);
...
begin
  ...
  //11/1/2001 - Use Draw as this is more generic...
  // 1/24/07 - Do assignment if <=256 colors to avoid blank image problem
  MemDC := GetDC(0);
  if (GetDeviceCaps(MemDC, BITSPIXEL) <= 8) then
...
procedure TfcBitmap.SetSizeInternal(const AWidth, AHeight: Integer);
begin
...
procedure TfcBitmap.ChangeColor(OldColor: TfcColor; NewColor: TfcColor);
var x, y: Integer;
begin
  for y := 0 to FHeight - 1 do
    for x := 0 to FWidth - 1 do
