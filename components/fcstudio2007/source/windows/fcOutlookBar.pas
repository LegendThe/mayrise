...
procedure TfcCustomOutlookBar.WMMouseWheel(var Message: TWMMouseWheel);
begin
//  if not (Focused) then
//  begin
//     Message.Result:=1;
...
function TfcCustomOutlookBar.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
var list: TfcOutlookList;
begin

  if (Selected <> nil) then
  begin
     list:= TfcOutlookPage(Selected).OutlookList;
     if (list<>nil) then
     begin
       with list do
          if (TopItem <> nil) and (TopItem.Index < Items.Count - 1) then
          begin
             list.Patch[0] := true;
             try
                TopItem := GetNextVisibleItem(TopItem);
             finally
                list.Patch[0] := false;
             end;
          end;
...
function TfcCustomOutlookBar.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
var list: TfcOutlookList;
begin
  ...
  begin
     ...
     begin
       with list do
           if (TopItem <> nil) and (TopItem.Index > 0) then
           begin
              list.Patch[0] := true;
              try
                 TopItem := GetPriorVisibleItem(TopItem); //Items[TopItem.Index - 1];
              finally
                 list.Patch[0] := false;
              end;
           end;
