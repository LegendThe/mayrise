...
// 2/12/06 - Lookup should be based on StoreDataUsing, instead of just text
}
...
//  FAlignmentVertical := fcavTop; { RSW - Used to be fcavCenter }
  ...
  // Don't show child tree at design time
...
var ImageIndex,OldBkMode: Integer;
  ...
  begin
    ...
    if not fcIsInwwGrid(self) and { 4/27/99 - RSW, Adjust left,top in special case }
      ((Style = csDropDownList) and not IsDroppedDown and not showmatchtext and not Frame.Enabled) then
    begin
    ...
    if fcIsInwwObjectViewPaint(self) or
       (IsTransparentEffective and not FFocused) or fcIsInwwGridPaint(self) or useVistaComboPaint then
  ...
  try
     ...
     begin
        if (not IsTransparentEffective) and not fcIsInwwObjectViewPaint(self) and
           not (fcIsInwwGridPaint(self)) then
        begin
            InflateRect(Rect, 1, 1);
            UseVistaComboPaint:=true;
            fcFillEditThemeBackground(IsDroppedDown, self.MouseInControl, Rect, Canvas);
            InflateRect(Rect, -1, -1);
        end;
     end;
     ...
     begin
        ...
        if (not IsTransparentEffective) and not fcIsInwwObjectViewPaint(self) and
           not (fcIsInwwGridPaint(self)) then
        begin
            if not UseVistaComboPaint then Canvas.FillRect(Rect);
        end;
        ...
        // Draw the text
        if (not fcIsInwwGrid(self) or (not GridPaint)) and not UseVistaComboPaint then begin
           SetBkMode(Canvas.Handle, TRANSPARENT);
       ...
       ((csPaintCopy in ControlState) or Frame.IsFrameEffective or useVistaComboPaint) and // 8/31/07 - Fix paint bug with and/or logic
...
begin
  ...
  begin
    ...
    else Text:= self.Text; // 5/15/04 - Uninitialized otherwise
