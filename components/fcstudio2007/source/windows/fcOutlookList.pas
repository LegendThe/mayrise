...
// 7/4/03 - Memory leak otherwise when hints enabled
}
...
begin
  ...
  Patch[0]:= False; // used Internally by fcoutlookbar
...
procedure TfcCustomOutlookList.WMMouseWheel(var Message: TWMMouseWheel);
begin
  if (not (Parent.Focused)) and (not Focused) then
  begin
     self.SetFocus;
//     Parent.SetFocus;
     Message.Result:=1;
...
{
function TfcCustomOutlookList.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  if (TopItem <> nil) and (TopItem.Index < Items.Count - 1) then TopItem := GetNextVisibleItem(TopItem); //Items[TopItem.Index + 1];
  result := True;
end;

function TfcCustomOutlookList.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
begin
  if (TopItem <> nil) and (TopItem.Index > 0) then TopItem := GetPriorVisibleItem(TopItem); //Items[TopItem.Index - 1];
  result := True;
end;
}
...
function TfcCustomOutlookList.ScrollButtonVisible(Button: TfcScrollButtonStyle): Boolean;
 ...
 // 12/18/03 - Allow scroll buttons to appear sooner
 function GetBottomItem: TfcOutlookListItem;
 var i: Integer;
    Total: Integer;
 begin
  ...
  begin
    ...
    result := Items[i];
    inc(Total, Items[i].SpacingSize);
    if ((Layout = loVertical) and (Total > ClientHeight - Items[i].SpacingSize div 2)) or
       ((Layout = loHorizontal) and (Total > ClientWidth - Items[i].SpacingSize div 2)) then Break;
  end;
 end;
...
begin
  ...
  case Button of
    ...
    sbDown: begin
     result := (TopItem<>nil) and (GetNextVisibleItem(GetBottomItem)<>nil);
    end;
  end;
end;
...
var i: Integer;
begin
  if (TopItem<>nil) and (BottomItem<>nil) then // 12/18/03 - Per newsgroup
    for i := TopItem.Index to BottomItem.Index do
