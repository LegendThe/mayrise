...
// 9/2/03 - Delete not being recognized before
}
unit fcCombo;
...
//    Function LoadComboGlyph: HBitmap; virtual;
    ...
    function IsVistaComboNonEditable: boolean;
...
function TfcDropDownButton.IsVistaTransparentButton: boolean;
var combo: TfcCustomCombo;
begin
   combo:= TfcCustomCombo(parent.parent);
   if fcUseThemes(self) and fcIsVista and not IsVistaComboNonEditable and
...
function TfcDropDownButton.IsVistaComboNonEditable: boolean;
begin
   result:= TfcCustomCombo(parent.parent).IsVistaComboNonEditable;
end;

function TfcDropDownButton.ParentMouseInControl: boolean;
begin
   result:= TfcCustomCombo(parent.parent).MouseInControl;
end;

function TfcDropDownButton.ParentDroppedDown: boolean;
begin
   result:= TfcCustomCombo(parent.parent).IsDroppedDown;
end;
...
begin
  ...
  begin
    ...
    // 9/2/03 - Delete not being recognized before
    else if (Key=vk_delete) and (FDataLink<>nil) and
...
begin
  if ((Frame.enabled or SkipInheritedPaint) and (not FFocused)) or
     (csPaintCopy in ControlState) or IsVistaComboNonEditable then
  begin
     // 6/28/99 - Support unbound csPaintCopy }
...
var r: TRect;
begin
  if fcIsInwwObjectViewPaint(self) or IsVistaComboNonEditable or
...
function TfcCustomCombo.IsVistaComboNonEditable: boolean;
begin

   result:= fcUseThemes(self) and fcIsVista and
          (self is TfcCustomCombo) and not (self as TfcCustomCombo).Editable and
          (not Frame.Enabled);

   if result and ButtonEffects.Flat then
      result:=false;
end;

procedure TfcCustomCombo.WMNCPaint(var Message: TMessage);
  ...
  begin
    ...
    try
       ...
       if ThemeServices.ThemesEnabled then begin
         ...
         ExcludeClipRect(dc, 3, 3, R.Right-3, R.Bottom-3);
         ThemeServices.DrawElement(dc, Details, R);
//         SelectClipRgn(dc, 0);  // Restore normal clipping rectangle
       end;
       message.Result:=1;
    finally
       ReleaseDC(Handle, DC);
