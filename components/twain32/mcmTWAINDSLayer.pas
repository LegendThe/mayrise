...
begin
  ...
  *)
  ...
  LoadString(hInstance, VERSION_INFO, FdsIdentity.Version.Info,  SizeOf(TW_STR32));
  LoadString(hInstance, MANUFACTURER, FdsIdentity.Manufacturer,  SizeOf(TW_STR32));
  LoadString(hInstance, PRODFAMILY,   FdsIdentity.ProductFamily, SizeOf(TW_STR32));
  LoadString(hInstance, PRODNAME,     FdsIdentity.ProductName,   SizeOf(TW_STR32));
  ...
  {$IFDEF TWNDEBUG}
    ...
    FmcmTWAINLog.LogFilename := 'C:\DS-DELPHI.LOG';
  ...
  then begin // Make this a global for later.
       ...
       // If operations succeed.
            ...
            then begin
                 ...
                 PostMessage(FormSource.Handle, 1999, 0, 0);
            end;
            FDSState := STATE5;
       end;
  end
...
begin
  ...
  STATE6 : case pTWMsg^.MSG of
           MSG_SET   : begin
                         ...
                         hFXFile := OpenFile(FileName, FXof, OF_EXIST);
                         if (hFXFile = HFILE_ERROR)
                         then begin
                              hFXFile := OpenFile(FileName, FXof, OF_CREATE);
                              if (hFXFile = HFILE_ERROR)
                              then begin
                                   FDSStatus.ConditionCode := TWCC_BADVALUE;
                                   twRc := TWRC_FAILURE;
                              end
                              else _lclose(hFXFile);
                         end;
                       end;
           MSG_GETDEFAULT,
           MSG_GETCURRENT,
           MSG_GET   : begin
                         pSetup := pTW_SETUPFILEXFER(pTWMsg^.pData);
                         lstrcpyn(pSetup^.FileName,
                                  'c:\twain.bmp',
                                  SizeOf(pSetup^.FileName));
...
begin
  ...
  MSG_GET : begin
              ...
              FormSource.SaveToFile(FileName);
