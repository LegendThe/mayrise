...
begin
  ...
  DAT_USERINTERFACE   : begin
                          ...
                          sgData.Cells[1,3]  := IntToHex(TTypeCastTWAINKernel(mcmTWAIN).FAppWnd, 8);
                          sgData.RowCount := 4;
                        end;
  DAT_XFERGROUP       : begin
...
begin
  ...
  DAT_PENDINGXFERS    : begin
                          TTypeCastTWAINIntf(mcmTWAIN).PendingXfers(FMsg, wValue);
                          sgData.Cells[1,1]  := IntToStr(shortint(wValue));
                          sgData.Cells[1,2]  := ''; // Reserved.
                        end;
  DAT_SETUPMEMXFER    : begin
                          TTypeCastTWAINIntf(mcmTWAIN).SetupMemXfer(@MemSetup);
  ...
  DAT_SETUPFILEXFER   : begin
                          ...
                          MSG_RESET      : begin
                                             FileName   := '';
                                             FileFormat := TWFF_BMP;
                                             TTypeCastTWAINIntf(mcmTWAIN).SetupFileXferA(FMSG, FileName, FileFormat);
  ...
  DAT_USERINTERFACE   : begin
                          ...
                          MSG_ENABLEDS       : begin
                                                 ...
                                                 TTypeCastTWAINIntf(mcmTWAIN).EnableDS(ShowUI);
                                                 if ShowUI
                                                 then sgData.Cells[1,1] := 'TRUE'
                                                 else sgData.Cells[1,1] := 'FALSE';
                                                 sgData.Cells[1,2]  := '0';
...
//                                        Result := TWGrayResponse(pGrayResponse, FMSG, Handle);
  ...
  *)
  ...
  DAT_IMAGEFILEXFER   : begin
                          TTypeCastTWAINIntf(mcmTWAIN).ImageFileXfer;
