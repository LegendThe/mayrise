...
{$IFDEF VER100} {$DEFINE COMPV3} {$ENDIF}

{KM $DEFINE SENDDIALOGUE}
{KM $DEFINE PREFDIALOGUE}
{ $DEFINE ICONSOURCEDIALOGUE}

interface

uses Windows, Messages, Classes, SysUtils, Graphics, Controls, Forms, Dialogs,
     twain,
     mcmTWAINIntf,
     mcmTWAINContainer;
...
{------------------------------------------------------------------------------}
  ...
  private
    ...
    FOnNegotiation : TNotifyEvent;      // Capability negotiations can begin.
  ...
  protected
    ...
    procedure   SetManufacturer(Value : string);
    ...
    procedure   SetProductFamily(Value : string);
...
{$ENDIF}
  ...
  published
    ...
    property    OnNegotiation : TNotifyEvent
      read      FOnNegotiation
...
uses {$IFDEF VER100} {KM uTwnAbout3,} {$ENDIF} { DELPHI 3  = VER100 }
...
begin
  ...
  Height := 28;
    ...
    then begin
         ...
         hTWNInternal := Graphics.TBitmap.Create;
  ...
  hBmpImage := 0;
  FSrcInfo := Nil;
end; { End TmcmTWAIN.Create.                                                   }


destructor TmcmTWAIN.Destroy;
begin
  // Make sure that the source and source manager is closed !
  CloseSource;
  CloseSourceMgr;
  ...
  Inherited Destroy;
end; { End TmcmTWAIN.Destroy.                                                  }


procedure TmcmTWAIN.Clear;
begin
  Inherited Clear;
end; { End TmcmTWAIN.Clear.                                                    }
...
end; { End TmcmTWAIN.About.                                                    }


function TmcmTWAIN.GetDisEndAcq : bool;
begin
  Result := FDisEndAcq;
end; { End TmcmTWAIN.GetDisEndAcq.                                             }


procedure TmcmTWAIN.SetDisEndAcq(Value : bool);
begin
  FDisEndAcq := Value;
end; { End TmcmTWAIN.SetDisEndAcq.                                             }


procedure TmcmTWAIN.SetManufacturer(Value : string);
begin
  lstrcpy(FAppIdentity.Manufacturer, PChar(Value));
end; { End TmcmTWAIN.SetManufacturer.                                          }


function TmcmTWAIN.GetManufacturer : string;
begin
  Result := StrPas(FAppIdentity.Manufacturer);
end; { End TmcmTWAIN.GetManufacturer.                                          }


procedure TmcmTWAIN.SetProductFamily(Value : string);
begin
  lstrcpy(FAppIdentity.ProductFamily, PChar(Value));
end; { End TmcmTWAIN.SetProductFamily.                                         }


function TmcmTWAIN.GetProductFamily : string;
begin
  Result := StrPas(FAppIdentity.ProductFamily);
end; { End TmcmTWAIN.GetProductFamily.                                         }


procedure TmcmTWAIN.SetProductName(Value : string);
begin
  lstrcpy(FAppIdentity.ProductName, PChar(Value));
end; { End TmcmTWAIN.SetProductName.                                           }


function TmcmTWAIN.GetProductName : string;
begin
  Result := StrPas(FAppIdentity.ProductName);
end; { End TmcmTWAIN.GetProductName.                                           }
...
end; { End TmcmTWAIN.GetCountry.                                               }


function TmcmTWAIN.GetFileFormat : TTwnFileFmt;
begin
  Result := FPrefFormat;
end; { End TmcmTWAIN.GetFileFormat.                                            }


procedure TmcmTWAIN.SetFileFormat(Value : TTwnFileFmt);
begin
  if Not(Value in [TWFF_PICT, TWFF_XBM{, TWFF_SPIFF, TWFF_EXIF}])
  then begin
       FPrefFormat := Value;
       if (Length(FFileName) > 0)
       then begin
            FFileName := lowercase(RemoveExt(FFileName));
            FFileName := FFileName + '.' + FileFormat2Ext(FPrefFormat);
       end;
  end;
end; { End TmcmTWAIN.SetFileFormat.                                            }


function TmcmTWAIN.GetFileFormats : TTwnFileFmts;
begin
  Result := FAllFormats;
end; { End TmcmTWAIN.GetFileFormats.                                           }
...
begin
  ...
  FAllFormats := Value;
end; { End TmcmTWAIN.SetFileFormats.                                           }


function TmcmTWAIN.GetFilename : string;
begin
  Result := FFilename;
end; { End TmcmTWAIN.GetFilename.                                              }


procedure TmcmTWAIN.SetFilename(Value : string);
...
end; { End TmcmTWAIN.SetFilename.                                              }


function TmcmTWAIN.GetReturnHandle : boolean;
begin
  Result := FFileAsDIB;
end; { End TmcmTWAIN.GetReturnHandle.                                          }


procedure TmcmTWAIN.SetReturnHandle(Value : boolean);
begin
  if (FPrefFormat = TWFF_BMP)
  then FFileAsDIB := Value
  else FFileAsDIB := False;
end; { End TmcmTWAIN.SetReturnHandle.                                          }
...
begin
  ...
  Msg.Result := 0;
end; { End TmcmTWAIN.WMGetMaxMinInfo.                                          }
...
begin
  ...
  then OpenDSM;
  Result := IsDSMOpen;
end; { End TmcmTWAIN.OpenSourceMgr.                                            }
...
end; { End TmcmTWAIN.CloseSourceMgr.                                           }
...
begin
  ...
  then begin
       Clear;
       FAcqFlag := TWFG_NONE; // Not ready to transfer yet.
       ...
       then begin
	    ...
	    if (SetupXferMech = TWRC_SUCCESS)
            then begin
                 ...
                 then FOnNegotiation(Self);
            end
            else CloseSource;
       end;
  end;
  Result := IsDSOpen;
end; { End TmcmTWAIN.OpenSource.                                               }
...
begin
  ...
  then begin
       ...
       then begin
            ...
            then FAcqFlag := TWFG_CLOSE
            else FAcqFlag := TWFG_KEEPOPEN;
       end
       else begin
            if DisableAfterAcquire
            then FAcqFlag := TWFG_CLOSE
  ...
  Result := IsDSEnabled;
end; { End TmcmTWAIN.EnableSource.                                             }
...
begin
  if IsDSEnabled
  then DisableDS;
end; { End TmcmTWAIN.DisableSource.                                            }


procedure TmcmTWAIN.SelectSource;
var DSMwasOpen : boolean;
begin
  ...
  // - Close the SM.
  if IsDSMOpen
  then DSMwasOpen := True
  else DSMwasOpen := False;
  if OpenSourceMgr
  then begin
       SelectDS;
       if Not(DSMwasOpen)
...
end; { End TmcmTWAIN.SelectSource.                                             }
...
end; { End TmcmTWAIN.SelectSourceIcon.                                         }
{$ENDIF}
...
end; { End TmcmTWAIN.GetSourceList.                                            }
...
begin
  ...
  then begin
       ...
       else Result := False; // OpenDS failed -- Acquire.
...
begin
  Result := Inherited GetCapabilityMsg(Cap, Msg, Container);
end; { End TmcmTWAIN.GetCapabilityMsg.                                         }
...
begin
  Result := Inherited SetCapabilityMsg(Msg, AsOneValue, Container);
end; { End TmcmTWAIN.SetCapabilityMsg.                                         }
...
begin
  ...
  Result := TwnImageInfo;
end; { End TmcmTWAIN.GetImageInfo.                                             }
...
end; { End TmcmTWAIN.GetImageLayout.                                           }
...
end; { End TmcmTWAIN.SetImageLayout.                                           }
...
end; { End TmcmTWAIN.GetMaxFrames.                                             }
...
end; { End TmcmTWAIN.SetMaxFrames.                                             }
...
end; { End TmcmTWAIN.GetOrientation.                                           }
...
end; { End TmcmTWAIN.SetOrientation.                                           }
...
end; { End TmcmTWAIN.GetMinimumHeight.                                         }
...
end; { End TmcmTWAIN.GetMinimumWidth.                                          }
...
end; { End TmcmTWAIN.GetPhysicalHeight.                                        }
...
end; { End TmcmTWAIN.GetPhysicalWidth.                                         }
...
end; { End TmcmTWAIN.GetPixelType.                                             }
...
end; { End TmcmTWAIN.SetPixelType.                                             }
...
end; { End TmcmTWAIN.GetPlanarChunky.                                          }
...
end; { End TmcmTWAIN.SetPlanarChunky.                                          }
...
end; { End TmcmTWAIN.GetBrightness.                                            }
...
end; { End TmcmTWAIN.SetBrightness.                                            }
...
end; { End TmcmTWAIN.GetXResolution.                                           }
...
end; { End TmcmTWAIN.SetXResolution.                                           }
...
end; { End TmcmTWAIN.GetYResolution.                                           }
...
end; { End TmcmTWAIN.SetYResolution.                                           }
...
end; { End TmcmTWAIN.NumImagesToScan.                                          }
...
end; { End TmcmTWAIN.FeederEnabled.                                            }
...
end; { End TmcmTWAIN.GetFeederLoaded.                                          }
...
end; { End TmcmTWAIN.PaperDetectable.                                          }
...
end; { End TmcmTWAIN.GetDuplexEnabled.                                         }
...
end; { End TmcmTWAIN.SetDuplexEnabled.                                         }
...
begin
  ...
  Result := twRC;
end; { End TmcmTWAIN.GetImagePalette.                                          }
...
begin
  ...
  DlgTwnSend.Free;
end; { End TmcmTWAIN.ExecuteDlgSend.                                           }
{$ENDIF}


{$IFDEF PREFDIALOGUE}
procedure TmcmTWAIN.Preferrences;
var SaveNegotiation : TNotifyEvent;
    PrefResult      : integer;
    PrefName        : string;
begin
...
end; { End TmcmTWAIN.Preferrences.                                             }
{$ENDIF}
...
begin
  ...
  then begin
       ...
       FSrcInfo.Clear;
       if IsDSOpen
       then FSrcInfo.SetSourceIdentity(@FSourceID);
       }
       ...
       FSrcInfo.SetSourceIdentity(@FSourceID);
       {
            ...
            // Otherwise return information on the default (last selected) data
            // source.
            if OpenSourceMgr
            then begin
                 if OpenSource('') // Open default data source.
                 then begin
                      FSrcInfo.SetSourceIdentity(@FSourceID);
                      CloseSource;
                 end;
  ...
  FOnNegotiation := SaveNegotiation;
  Result := FSrcInfo;
end; { End TmcmTWAIN.GetSourceInfo.                                            }
