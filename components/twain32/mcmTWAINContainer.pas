...
//----------------------------------------------------------------------------//
//
// $Log:  68811: mcmTWAINContainer.pas 
{
{   Rev 1.0    10/02/2011 15:02:10  Kacper
{ init
}
//
...
//------------------------------------------------------------------------------
  ...
  public
{$WARNINGS OFF}
    constructor Create(AOwner: TComponent; NewFrame : pTW_FRAME); {$IFNDEF COMPV3} reintroduce; {$ELSE} virtual; {$ENDIF}
{$WARNINGS ON}
...
begin
  ...
  then begin
       ...
       TWON_ENUMERATION : begin
                            ...
                            pItemList := PChar(@pEnumeration^.ItemList) +
  ...
  then begin
       ...
       TWON_ENUMERATION : begin
                            ...
                            then begin
                                 with pEnumeration^
                                 do begin
                                    pPreList  := PChar(@ItemList) + Index * ItemSize;
  ...
  then begin
       ...
       TWON_ARRAY       : if Assigned(pArray)
                          then begin
                               with pArray^
                               do begin
                                  pPreList  := PChar(@ItemList) + i * ItemSize;
                                  pPostList := pPreList + ItemSize;
                                  inc(i);
...
begin
  ...
  TWTY_STR32   : Result := StrPas(pTW_STR32(pValue)^);
  TWTY_STR64   : Result := StrPas(pTW_STR64(pValue)^);
  TWTY_STR128  : Result := StrPas(pTW_STR128(pValue)^);
  TWTY_STR255  : Result := StrPas(pTW_STR255(pValue)^);
  end;
...
begin
  ...
  // TWTY_FRAME   : ;
  TWTY_STR32   : StrPCopy(PChar(pValue), Value);
  TWTY_STR64   : StrPCopy(PChar(pValue), Value);
  TWTY_STR128  : StrPCopy(PChar(pValue), Value);
  TWTY_STR255  : StrPCopy(PChar(pValue), Value);
                     ...
                     then with pEnumeration^
                          do if (0 <= CurrentIndex) and (CurrentIndex < NumItems)
                             then begin
                                  ...
                                  pItemList := PChar(@ItemList) + ItemSize;
...
function TTwnContainer.GetMaxValue : Variant;
  ...
  function GetMaxFromList(ItemList : Array of TW_UINT32;
                          NumItems : smallint;
  ...
  begin
    ...
    i := 1;
    if (NumItems > 0)
    then Result := GetValue(@ItemList[0], ItemType);
    while (i < NumItems)
    do begin
       if (Result < GetValue(@ItemList[i], ItemType))
       then Result := GetValue(@ItemList[i], ItemType);
       inc(i);
    end;
  end; { End GetMaxFromList.                                                   }
...
function TTwnContainer.GetMinValue : Variant;
  ...
  function GetMinFromList(ItemList : Array of TW_UINT32;
                          NumItems : smallint;
                          ItemType : TW_UINT32) : Variant;
  var i : integer;
  begin
  ...
  TWON_ARRAY       : if Assigned(pArray)
                     then with pArray^
                          do if (0 <= Index) and (Index < NumItems)
                             then begin
                                  ItemSize  := GetItemSize(FItemType);
                                  pItemList := PChar(@ItemList) + Index * ItemSize;
                                  Result := GetValue(pItemList, ItemType);
                             end;
  TWON_ENUMERATION : if Assigned(pEnumeration)
                     then with pEnumeration^
