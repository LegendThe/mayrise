...
uses Windows, Classes, SysUtils, twain;

{$IFDEF VER100} {$DEFINE COMPV3} {$ENDIF}
{$IFDEF VER110} {$DEFINE COMPV3} {$ENDIF}
...
type
  ...
  protected
    ...
    FTrackFile   : array[0..512] of char;
    FLogFileName : string;
  public
    { public declarations    }
...
begin
  ...
  FLogFileName := 'C:\APPTWN.LOG';
end; { End TmcmTWAINLog.Create.                                                }


destructor TmcmTWAINLog.Destroy;
begin
  Inherited Destroy;
end; { End TmcmTWAINLog.Destroy.                                               }


procedure TmcmTWAINLog.Clear;
begin
;
end; { End TmcmTWAINLog.Clear.                                                 }
...
end; { End TmcmTWAINLog.DeleteLogFile.                                         }
...
begin
  ...
  Result := RetStr;
end; { End TmcmTWAINLog.MatchTwainInt.                                         }
...
end; { End TmcmTWAINLog.Triplet2Str.                                           }
...
begin
  ...
  // Result := TmpText;
end; { End TmcmTWAINLog.ItemValue2Str.                                         }
...
end; { End TmcmTWAINLog.ItemType2Str.                                          }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.Identity2Str.                                          }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.Parent2Str.                                            }
...
begin
  ...
  TWDE_CHECKAUTOMATICCAPTURE  : begin
                                  ...
                                  FmtStr(ValueStr, '%0:sAutomatic capture=%1:d%2:s', [ValueStr, {$IFDEF COMPV3}longint(A){$ELSE}longword(A){$ENDIF}, CR_LF]);
                                  FmtStr(ValueStr, '%0:sTime before first capture=%1:d%2:s', [ValueStr, {$IFDEF COMPV3}longint(B){$ELSE}longword(B){$ENDIF}, CR_LF]);
                                  FmtStr(ValueStr, '%0:sTime between capture=%1:d%2:s', [ValueStr, {$IFDEF COMPV3}longint(C){$ELSE}longword(C){$ENDIF}, CR_LF]);
                                end;
  TWDE_CHECKBATTERY           : begin
                                  ...
                                  FmtStr(ValueStr, '%0:sBattery percentage=%1:d%2:s', [ValueStr, {$IFDEF COMPV3}longint(B){$ELSE}longword(B){$ENDIF}, CR_LF]);
                                end;
  TWDE_CHECKDEVICEONLINE      : ;
  TWDE_CHECKFLASH             : begin
                                  A := Value^.FlashUsed2;
                                  FmtStr(ValueStr, '%0:sFlask used 2=%1:d%2:s', [ValueStr, {$IFDEF COMPV3}longint(A){$ELSE}longword(A){$ENDIF}, CR_LF]);
                                end;
  TWDE_CHECKPOWERSUPPLY       : begin
                                  A := Value^.PowerSupply;
                                  FmtStr(ValueStr, '%0:sPower supply=%1:d%2:s', [ValueStr, {$IFDEF COMPV3}longint(A){$ELSE}longword(A){$ENDIF}, CR_LF]);
                                end;
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.DeviceEvent2Str.                                       }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.ImageInfo2Str.                                         }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.Frame2Str.                                             }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.ImageLayout2Str.                                       }
...
begin
  ...
  FmtStr(ValueStr, '%0:sMemory pointer=%1:x%2:s', [ValueStr, {$IFDEF COMPV3}longint(Value^.Memory.TheMem){$ELSE}longword(Value^.Memory.TheMem){$ENDIF}, CR_LF]);
  Result := ValueStr;
end; { End TmcmTWAINLog.ImageMemXfer2Str.                                      }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.PendingXfers2Str.                                      }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.SetupMemXfer2Str.                                      }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.SetupFileXfer2Str.                                     }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.SetupFileXfer22Str.                                    }
...
begin
  ...
  Result := ValueStr;
end; { End TmcmTWAINLog.UserInterface2Str.                                     }
...
end; { End TmcmTWAINLog.XferGroup.                                             }
...
end; { End TmcmTWAINLog.Container2Str.                                         }
...
end; { End TmcmTWAINLog.Msg2Str.                                               }


function TmcmTWAINLog.Dat2Str(DAT : integer) : string;
var DATStr    : array[0..64] of char;
...
end; { End TmcmTWAINLog.Dat2Str.                                               }


function TmcmTWAINLog.Cap2Str(CAP : integer) : string;
var CapStr    : array[0..64] of char;
    ReturnStr : array[0..255] of char;
begin
  Str(Cap, CapStr);
...
begin
  ...
  then begin
       ...
       do begin
          ...
          if (TmpStr = StrPas(@buffer[x]))
          then begin
               ReturnStr := StrPas(Token);
               break;
          end;
  ...
  Result := ReturnStr;
end; { End TmcmTWAINLog.Attribute2Str.                                         }
...
begin
  ...
  Result := ReturnStr;
end; { End TmcmTWAINLog.CapData2Str.                                           }
