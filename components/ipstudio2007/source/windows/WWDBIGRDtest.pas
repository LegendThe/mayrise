    ...
    begin
       ...
       begin
          if (ARect.Top>0) or (Columns[ACol].GroupName='') then
             ButtonOffset:=1
//           (not TitleClickGroupTitle and (ARect.Top>0))
       end;
    end;
...
//       if (ACol<FieldCount) and
//         UseRightToLeftAlignmentForField(Fields[Acol], Alignment) then
//         WriteOptions:= WriteOptions + [wtoRightToLeft];
       ...
       if ARect.Left<0 then // Group case where 1st part of group is not visible
       begin
...
//         WriteOptions:= WriteOptions + [wtoRightToLeft];
       ...
       if not (dgWordWrap in Options) then begin
          ...
          if (dgTrailingEllipsis in Options) then
             WriteOptions:= WriteOptions + [wtoEllipsis];
             // 7/11/01 - Don't change display rect, as it later can mess up scrolling operations
//          if not (dgRowLines in Options) and
//             not (dgRowLinesDisableFixed in Options) then
...
{         if not PaintOptions.HaveBackgroundForDataCells then
         begin
            if not IsDropDownGridFocused then
...
//                   ([dgRowSelect, dgRowSelectEditable] * Options <> []) then
...
//     if not (dgRowLines in Options) then TempRect.Bottom:= TempRect.Bottom+1;
...
{
              if (Screen.ActiveControl<>CustomEdit) or
                  ((Row<>ARow) or (Col<>ACol)) or
                  IsDropDownGridFocused or
...
{
       case FDataLink.DataSet.State of
	  dsEdit: if ARow = FDataLink.ActiveRecord - RowOffset then tempBitmap.assign(EditIndicatorBitmap);
	  dsInsert: if ARow = FDataLink.ActiveRecord - RowOffset then tempBitmap.assign(InsertIndicatorBitmap);
	  else begin
             if ARow = FDataLink.ActiveRecord - RowOffset then begin
                if (dgMultiSelect in Options) and
                   isSelectedRow(ARow) then
                   tempBitmap.assign(MultiArrowBitmap)
                else tempBitmap.assign(BrowseIndicatorBitmap);
             end
             else if (dgMultiSelect in Options) and isselectedRow(ARow) then
                tempBitmap.assign(MultiSelectBitmap);
          end;
       end;
}
     end;
...
(*     SRect  := classes.Rect(0, 0, tempBitmap.Width, tempBitmap.Height);
     ...
     if FIndicatorIconColor<>clBlack then begin
	...
	tempBitmap.Canvas.Brush.Color:= FIndicatorIconColor; // Uncomment to support any color
//	tempBitmap.Canvas.Brush.Color:= clYellow;
     ...
     // Use DefaultRowHeight so its aligned to the top when child grid is expaned
     Canvas.Draw(((ARect.right-tempBitmap.Width) div 2)+1,
		(ARect.Top + ARect.Top + DefaultRowHeight - tempBitmap.Height) div 2, tempBitmap);
...
{           if PaintOptions.Column1Bitmap<>nil then begin
               if not (dgColLines in Options) then TempRect.Left:= TempRect.Left-1;
               Canvas.CopyRect(TempRect, PaintOptions.Column1Bitmap.canvas,TempRect);
           end
           else begin
...
//                        TwwExpandButton(CurrentcustomEdit).Height:= CellRect(Col, Row).Bottom-Cellrect(Col, Row).Top;
	...
	VK_DELETE: begin
      ...
      if (Key in [vk_up, vk_down, vk_left, vk_right,
                 ...
                 vk_insert, vk_tab, vk_return]) and CustomEditChildHasFocus then
         SetFocus;
      case Key of
	VK_UP: PriorRow;
	VK_DOWN: NextRow;
...
begin
   ...
   if (not (Field.DataType in [ftMemo, ftOraclob])) and { 6/8/98 - return true if richedit field }
         ...
         begin
            ...
            try
              ...
              begin
                 ...
                 begin
                    if curField.dataType in [ftMemo, ftOraBlob] then
...
//                 ([dgRowSelectEditable, dgRowSelect] * Options <> []) then exit;
...
//   bidioffset:=0;
   ...
   bottom :=  CellRect(0, VisibleRowCount).Bottom;
   if bottom = 0 then
   ...
   begin
     ...
     if not (dgRowLinesDisableFixed in Options) or (dgTitles in Options) then
     begin
...
//             end
          ...
          begin
             if not (dgHideBottomDataLine in Options) then
             begin
//             if (PadColumnStyle<>pcsPlain) or
//               (dgRowLines in Options) then  // 7/9/01 - Don't draw bottom line for pcsPlain style
...
//         result:= True;  // Probably need to find out if it really has focus
         result:= (Control<>nil) and (Control.HandleAllocated) and
                  IsChild(Control.Handle, GetFocus)
      end;
   end
end;
