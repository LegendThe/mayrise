...
function DefaultCheckbox(defaultConst: string; regDefaultCheckBox: string): string;
var ipreg: TRegIniFile;
    defaultTrue: string;
begin
   ipreg := TRegIniFile.create('');
   defaultTrue:= ipreg.ReadString('\Software\Woll2Woll\InfoPower', regDefaultCheckBox, defaultConst);
   if defaultTrue='' then defaultTrue:=defaultConst;
   ipreg.free;
   result:= defaultTrue;
end;
...
begin
   ...
   if controlName='RichEdit' then begin
      ...
      CheckOnValue.Text:= DefaultCheckBox('True', 'DefaultCheckBoxTrue'); //'Yes';
      CheckOffValue.Text:= DefaultCheckBox('False', 'DefaultCheckBoxFalse'); //'No';
      ControlTypeNotebook.Activepage:= 'Checkbox';
   end
