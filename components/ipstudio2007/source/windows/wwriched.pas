...
uses
  ...
  ShellAPI, OleDlg, buttons, oleconst, grids, wwframe, commctrl, FileCtrl;
...
//     procedure PrintRange(const Caption: string; startPage: integer; endPage: integer);
     ...
     Procedure StartMSWordMonitoring(TempFileName: string); // Used internally by spellchecker
     Procedure StopMSWordMonitoring; // Used internally by spellchecker
...
//                 [reoSpellCheck, reoGrammarCheck];
    ...
    property OnSpellCheck   : TNotifyEvent  read FOnSpellCheck write FOnSpellCheck;
...
//  SpellCheckOptions:= [reoSpellCheck, reoGrammarCheck];
  ...
  Patch:= VarArrayCreate([0, 11], varVariant);
  ...
  Patch[7]:= True; { Set to True to disable checking of focus for MSWordSpellChecker}
  Patch[8]:= 0; { StartPage for Printing }
  Patch[9]:= 0; { EndPage for Printing}
  Patch[10]:=false; // Set to true to use old RICHEDIT20A
  Patch[11]:= 3;
end;
...
begin
   if (parent is TwwRichEditForm) then
      DialogRichEdit:= (parent as TwwRichEditForm).RichEdit1
   else
      DialogRichEdit:= RichEdit;
...
begin
  ...
  // frameOffset:=GetSystemMetrics(sm_cxframe) * 2;
   RightDragButton.Left:= (Width - TwipsToPixels(tempRight) {* - (frameOffset-8)*} - RightDragButton.Width);
...
//   Round to nearest 1/4 for centimeters
   ...
   if (Sender = LeftDragButton) then begin
      ...
      if DialogRichEdit.EffectiveEditWidth = rewPrinterSize then
      begin
         tempPixels:= (RightDragButton.MaxValue-10) - // Support RichEditBar
...
begin
   ...
   with form do try
      ...
      with RichEdit1 do begin
         ...
         RichEdit1.PrintPreviewCaption:= self.PrintPreviewCaption;
//         WordWrap := False;
         Parent:= form;
         Left := 0;
      ...
      with RichEdit1 do begin
         ...
         PrintHeader.Assign(self.PrintHeader);
         PrintFooter.Assign(self.PrintFooter);
      end;
      RichEdit1.OnPrintHeader:= self.OnPrintHeader;
      RichEdit1.OnPrintFooter:= self.OnPrintFooter;
      ...
      RichEdit1.OnSpellCheck:= self.OnSpellCheck;
      ...
      begin
         RichEdit1.EditorOptions:=
            [reoShowLoad, reoShowSaveAs, reoShowSaveExit, reoShowPrint, reoShowPrintPreview, reoShowPageSetup,
             reoShowFormatBar, reoShowToolBar, reoShowStatusBar,
      ...
      if result then begin
         if RichEdit1.modified then begin
             ...
             begin
                ...
                begin
                  {$ifdef wwDelphi2006Up}
                   if ((GetField as TBlobField).BlobType = ftWideMemo) then
                   begin
                      Stream := TStringStream.Create('');
                      try
                          RichEdit1.ILines.SaveToStream(Stream);
                          GetField.AsString := Stream.DataString;
                      finally
                          Stream.Free;
                      end;
                   end
...
Procedure TwwCustomRichEdit.Undo;
begin
   SendMessage(Handle, EM_UNDO, 0, 0);
end;

Procedure TwwCustomRichEdit.Redo;
begin
...
begin
   ...
   if not (csDesigning in parentform.ComponentState) then
   begin
     if DefaultPopupMenu=nil then exit;  // Required for Delphi 9 Pre-release as
                                         // csDesigning is false for parentform - this is a delphi 9 vcl problem
...
(*  if not (csDesigning in ComponentState) then
  ...
  UseVersion4:= false;
  if (FLibHandle=0) and (MSVersion>3) then begin
     UseVersion4:=true;
...
//    SystemDefaultCharSize = 3 - Win32Native.lstrlen(ptr);
  ...
  if UseVersion4 and (patch[1]=false)  and (patch[10]=false) then
  begin
      CreateSubClass(Params, 'RICHEDIT50W')
  end
  else if (FLibHandle<>0) and (Patch[1]=False) then
...
//        r.left:= GutterWidth+10;
        r.Left:= gutterWidth;  // 5/5/09 - Removed GutterWidth+10 as it makes the text indented too much
        r.top:= 0;
        ...
        // 5/5/09 - Comment out following code as otherwise not enough of the text range is used.  Perhaps could
        // still execute this code if the editwidth was larger pagewidth
...
procedure TwwRTFHeaderFooter.SetFont(Value: TFont);
begin
   FFont.assign(value);
end;

constructor TwwRTFHeaderFooter.Create(AOwner: TComponent);
begin
   VertMargin:= 0.5;
   FFont := TFont.Create;
end;

destructor TwwRTFHeaderFooter.Destroy;
begin
  FFont.Free;
  inherited Destroy;
end;

Procedure TwwRTFHeaderFooter.Assign(Source: TPersistent);
begin
  if Source is TwwRTFHeaderFooter then with TwwRTFHeaderFooter(Source) do
  begin
//     self.OnDrawText:= OnDrawText;
     self.VertMargin:= VertMargin;
...
procedure TwwCustomRichEdit.DoPrintHeader(DrawRect: TRect;
         PageNumber: integer;
         var LeftText, CenterText, RightText: string;
...
begin
    ...
    begin
       FOnPrintHeader(self, DrawRect, PageNumber,
          LeftText, Centertext, RightText, DoDefault);
    end;
end;

procedure TwwCustomRichEdit.DoPrintFooter(DrawRect: TRect;
         PageNumber: integer;
         var LeftText, CenterText, RightText: string;
         var DoDefault: boolean);
begin
    ...
    begin
       FOnPrintFooter(self, DrawRect, PageNumber,
          LeftText, Centertext, RightText, DoDefault);
    end;
end;
...
{begin
   PrintRange(Caption, 0, 0);
end;

procedure TwwCustomRichEdit.PrintRange(const Caption: string; startPage: integer; endPage: integer);
}
const
...
begin
  ...
  begin
    ...
    if TotalPages>1 then dec(TotalPages);  // otherwise ends up being 1 too much
    ...
    try
      TempPageNumber:= Printer.PageNumber;
      ...
      repeat
        ...
        Printer.Canvas.Font.assign(PrintHeader.Font);
        DoDefault:= True;
        LeftText:= PrintHeader.LeftText;
        CenterText:= PrintHeader.CenterText;
        RightText:= PrintHeader.RightText;

        // 6/26/03 - Provide way to abort header printing and prevent page from being on printed output.
        try
          StartPage:= Patch[8];
          EndPage:= Patch[9];
          if (StartPage>0) and (TempPageNumber<StartPage) then sysutils.abort;
          if (EndPage>0) and (TempPageNumber>EndPage) then sysutils.abort;

          DoPrintHeader(HeaderRect, TempPageNumber, LeftText, CenterText, RightText, DoDefault);
        except
          rc := SaveRect;
          chrg.cpMin := LastChar;
          LastChar := SendMessage(Self.Handle, EM_FORMATRANGE, 0, Longint(@Range));
          if LastChar<=chrg.cpMin then break;
          if (LastChar < MaxLen) and (LastChar <> -1) then TempPageNumber:= TempPageNumber + 1;
          continue;
        end;

        if DoDefault then
        begin
          if LeftText<>'' then Printer.Canvas.TextOut(HeaderRect.Left, HeaderRect.Top, LeftText);
          // Fix bug in right header not showing
          if RightText<>'' then Printer.Canvas.TextOut(HeaderRect.Right-Printer.Canvas.TextWidth(RightText), HeaderRect.Top, RightText);
          if CenterText<>'' then Printer.Canvas.TextOut(HeaderRect.Left+
             (HeaderRect.Right-HeaderRect.Left-Printer.Canvas.TextWidth(CenterText)) div 2,
               HeaderRect.Top, CenterText);
          if PrintHeader.LineSeparator then
          begin
             Printer.Canvas.Pen.Width:= TwipsToPrinterPixels(6);
             Printer.Canvas.MoveTo(HeaderRect.Left, HeaderRect.Top + Printer.Canvas.TextHeight('H') + TwipsToPrinterPixels(28));
             Printer.Canvas.LineTo(HeaderRect.Right, HeaderRect.Top + Printer.Canvas.TextHeight('H') + TwipsToPrinterPixels(28));;
          end;
        end;
...
//        TopOffsetTwips := PixelsToTwipsY(PrnPageOffset.Y);
        ...
        HeaderRect:=
           Rect(TwipsToPrinterPixels(rc.left),
                ...
                TwipsToPrinterPixels(rc.right),
                Trunc(PrnPhysPageSize.Y - PrintFooter.VertMargin*LogY - PrnPageOffset.Y));
        Printer.Canvas.Font.assign(PrintFooter.Font);
        DoDefault:= True;
        LeftText:= PrintFooter.LeftText;
        CenterText:= PrintFooter.CenterText;
        RightText:= PrintFooter.RightText;
        DoPrintFooter(HeaderRect, TempPageNumber, LeftText, CenterText, RightText, DoDefault);

        if DoDefault then
        begin
          th:= Printer.Canvas.TextHeight('H');
          if LeftText<>'' then
            Printer.Canvas.TextOut(HeaderRect.Left, HeaderRect.Bottom - th, LeftText);
          if RightText<>'' then
             Printer.Canvas.TextOut(HeaderRect.Right-Printer.Canvas.TextWidth(RightText), HeaderRect.Bottom-th, RightText);
          if CenterText<>'' then
             Printer.Canvas.TextOut(HeaderRect.Left+
             (HeaderRect.Right-HeaderRect.Left-Printer.Canvas.TextWidth(CenterText)) div 2,
               HeaderRect.Bottom-th, CenterText);
          if PrintFooter.LineSeparator then
          begin
             Printer.Canvas.Pen.Width:= TwipsToPrinterPixels(6);
             Printer.Canvas.MoveTo(HeaderRect.Left, HeaderRect.Bottom - Printer.Canvas.TextHeight('H') - TwipsToPrinterPixels(28));
             Printer.Canvas.LineTo(HeaderRect.Right, HeaderRect.Bottom - Printer.Canvas.TextHeight('H') - TwipsToPrinterPixels(28));;
          end;
        end;
      ...
      Patch[8]:= 0; // Reset start and end page
      Patch[9]:= 0;
      SendMessage(Handle, EM_FORMATRANGE, 0, 0);  // flush buffer
...
//  FLastSetRect:= Rect(0,0,0,0);
  ...
  begin
     ...
     begin
         if not (csDesigning in ComponentState) then
         begin
           if LastColor=clNone then // 12/20/02 - So LastColor is not lost when recreating window
              LastColor:= Color;
           Color:= Frame.NonFocusColor;
         end
     end;
...
begin
  ...
  // 3/13/04 - Reload from dataset as CreateWnd streamed in wrong value for data-bound case
  if (useReloadStream) and (ReloadStream<>nil) then
     if (GetField<>nil) then DataChange(self);
end;
...
procedure TwwCustomRichEdit.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
    inherited MouseDown(Button, Shift, X, Y); // 9/10/03 Use Button intead of mbLeft
...
begin
   with pENLink^ do begin
      SetLength(sz, 2 * chrg.cpMax - chrg.cpMin); // 5/9/03 - Allocate a string large enough to hold possible MBCS chars
      SetLength(szwide, chrg.cpMax-chrg.cpMin);

      TextRange.chrg:= chrg;
      if patch[11]=true then
        TextRange.lpstrText:= Pointer(szwide)
      else
        TextRange.lpstrText:= Pointer(sz);
      Perform(em_getTextRange, 0, Integer(@TextRange));
//      SendMessageW(Handle, em_getTextRange, 0, Integer(@TextRange));
...
begin
  ...
  begin
    ...
    Find.lpstrText:= PChar(PWideChar(WideSearchStr));
  end
  else
...
type
  ...
  public
    constructor Create(CreateSuspended: boolean; TempFileName: string);
  end;
...
begin
  Result := True;
  key:= 'bosa_sdm_Microsoft Office Word';
  key:= 'bosa';
  GetWindowText(Window, @buf, 100);
  t:= buf;
  GetClassName(Window, @buf, 100);
  s:= buf;
  TempFileName:= PCheckTaskInfo(Data)^.TempFileName;
  ProcessPath(TempFileName, Drive, DirPart, FilePart);
  FilePart:= Copy(FilePart, 1, length(FilePart)-4);

//  if copy(t, 1,8)='Spelling' then
  if (copy(s, 1,length(key))=key) then
  begin
     //fgw:= getparent(window);
     newtext:= buf;
     if (pos(FilePart, newText)=1) then
     begin
       windows.SetForegroundWindow(window);
       PCheckTaskInfo(Data)^.Finished:= True;
     end;
  end;

end;

constructor TMonitorThread.Create(CreateSuspended: boolean; TempFileName: string);
begin
...
begin
  ...
  begin
     ...
     if Info.Finished then break;
  end;
end;

Procedure TwwCustomRichEdit.StartMSWordMonitoring(TempFileName: string);
begin
   if Patch[7]=True then exit;

   MSWordThread:= TMonitorThread.create(false, TempFileName);
   MSWordThread.FreeOnTerminate:= true;
end;

Procedure TwwCustomRichEdit.StopMSWordMonitoring;
begin
   if Patch[7]=True then exit;
   MSWordThread.Terminate;  // Also frees with FreeOnTerminate property
end;

Function TwwCustomRichEdit.MSWordSpellChecker: boolean;
begin
  if Assigned(FOnSpellCheck) then
    FOnSpellCheck(self);
  result:= True;
end;
...
procedure TwwCustomRichEdit.SetRTFText(InsertText: string);
var
   MemoryStream : TMemoryStream;
begin
   MemoryStream := TMemoryStream.Create();
   ...
   try
     MemoryStream.Write(Pointer(insertText)^, Length(insertText));
     MemoryStream.Position := 0;
     PutRTFSelection (MemoryStream);
//     NotesRE.Lines.LoadFromStream(MemoryStream);
   finally
     MemoryStream.Free();
   end;
end;
...
begin
  ...
  if ErrorCode<>0 then
  begin
     raise EwwRTFError.Create(Msg, ErrorCode, 0);
  end;
end;
...
constructor EwwRTFError.Create(Message: string; ErrCode: integer; Dummy : integer);
begin
  FErrorCode := ErrCode;
  inherited Create(Message);
end;

procedure TwwDBRichEdit.SetController(Value: TwwController);
begin
...
procedure TwwCustomRichEdit.InsertBitmap(Pict: TBitmap);
var s: string;
begin
   s:= BitmapToRtf(pict);
   self.SetRtfText(s);
...
procedure TwwCustomRichEdit.SetHideSelection(Value: Boolean);
begin
...
function TwwCustomRichEdit.StoreScrollBars: boolean;
begin
   if (parent<>nil) and (parent.parent<>nil)
      and wwIsClass(parent.parent.ClassType, 'TwwRichEditBar') then
      result:=true
   else result:= (scrollbars <> ssNone);

end;

procedure TwwCustomRichEdit.SetScrollBars(Value: TScrollStyle);
begin
   inherited ScrollBars := Value;
end;

function TwwCustomRichEdit.GetScrollBars: TScrollStyle;
begin
   result:= inherited ScrollBars;
end;

procedure TwwCustomRichEdit.PrintPreview;
begin
   TwwRTFPreviewform.Preview(self);
end;
...
begin
  ...
  begin
     ...
     SetLength(wides, CharRange.cpMax - CharRange.cpMin + 1);
     len:= SendMessageW(Handle, EM_GETSELTEXT, 0, Longint(PWidechar(wides)));
     SetLength(wides, Len);
     result:= wides;
  end
  else begin
...
procedure TwwCustomRichEdit.SetSelText(value: string);
var wides: WideString;
begin
   if patch[11]=true then
   begin
      wides:=value;
      SendMessageW(Handle, EM_REPLACESEL, 0, Longint(PWideChar(wides)));
   end
   else
...
initialization
  ...
  // 6/26/03
  cfRTFInUTF8 := RegisterClipboardFormat('RTF in UTF8');
  cfRTFNoNCRs := RegisterClipboardFormat('RTF with NCRs for nonASCII');
  CF_vmRTF := RegisterClipboardFormat(CF_RTF);
finalization
