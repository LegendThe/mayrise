...
begin
   result:=
    ...
    begin
      ...
      Patch:= VarArrayCreate([0, 10], varVariant);
...
//      Coord:= MouseCoord(x,y);
      ...
      validCell:= IsValidCell(Coord.X, Coord.Y); // 3/12/04 - Fix logic so indicator column still selects range
...
//      if not (isValidCell(Coord.x, Coord.y)) then exit;
      ...
      if ValidCell and (isCustomEditCell(Coord.x, Coord.y, lookupControl) and not lookupControl.visible) then
      begin
         if (Coord.x=Col) and (Coord.y=Row) then {is Selected Cell}
            DrawCell(Coord.x, Coord.y, CellRect(Coord.x, Coord.y), [gdSelected]);
      end;

      if (ValidCell or (Coord.X=0)) and
         (dgMultiSelect in Options) and (ssShift in Shift) and TempShiftSelectMode then
      begin
         with Datalink.Dataset do begin
     ...
     { 8/24/99 - Don't free bookmark if ssDouble as this can cause exception if
      ...
      if not (ssDouble in Shift) and (TempShiftSelectBookmark <> ShiftSelectBookmark) then
         if (ShiftSelectBookmark<>nil) then  // 12/12/05 - If nil, then dataset was re-opened so bookmark was already freed
...
{         if Datalink.ActiveRecord>=0 then
     ...
     else if (Field.DataType in [ftMemo, ftOraClob
...
begin
  if visible and (currentCustomEdit<>Nil) and
     (currentCustomEdit.visible) and
     (currentCustomEdit.focused or CustomEditFocus) then // 3/31/03 - Call CustomEditFocus in case activecontrol is a child component of currentcustomedit
     if CanFocus then
        setFocus;
            ...
            if tempField.ReadOnly or (fields[i].ReadOnly) or
               ...
               begin
                 ...
                 Regfile.WriteString(SectionName,tempField.FieldName,
                    ...
                    tempField.DisplayLabel+Delimiter+'F') // 10/1/03 - Don't save readonly if usetfields is true
...
procedure TwwDBGrid.ScrollCurrentToTop(currentRow: integer=0); // Scroll's current record to top if possible
var scrollCount, moveByCount, actualMovecount: integer;
begin
...
//      (canEditGrid or AlwaysShowControls) then
   begin
      ...
      IsDateTime:=
            (GetActiveField is TDateTimeField) or
            ((GetActiveField<>nil) and
             (GetActiveField.DataType in [ftDate, ftDateTime, ftTimeStamp, ftTime]) );

      if (not (ecoDisableDateTimePicker in EditControlOptions)) and
         IsDateTime then begin
