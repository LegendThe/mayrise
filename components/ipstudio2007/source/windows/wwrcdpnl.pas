...
begin
   ...
   // 9/25/06 - If CtrlGrid is owner then set parent to DBCtrlGrid;
   if (Parent=nil) and (Owner<>nil) and
      wwIsClass(Owner.ClassType, 'TDBCtrlGrid') then
     Parent:= Owner as TWinControl;

   inherited;
end;

procedure TwwRecordViewPanel.SetController(Value: TwwController);
begin
...
function TwwRecordViewPanel.CreateDefaultEditControl(AOwner: TComponent): TCustomedit;
begin
   result:= TwwDBEdit.create(AOwner);
end;

// Implement csPaintCopy
procedure TwwRecordViewPanel.WMPaint(var Message: TWMPaint);
var rect: TRect;
begin
   ...
   begin
      rect.Left:= 0;
      rect.top:=0;
      rect.Bottom:= self.boundsrect.Bottom - self.boundsrect.Top;
      rect.right:= self.boundsrect.right-self.boundsrect.left;
      Windows.FillRect(message.dc, rect, brush.handle);
      self.PaintControls(message.DC, nil);
      message.result:=1;
   end
   else inherited;
end;
...
procedure TwwRecordViewPanel.PaintWindow(DC: HDC);  // labels not shown otherwise in vista themes
begin
  // Do nothing
end;
{$endif}
