...
//              if (stat<>0) and (charcode = vk_escape) then exit;
              if (stat<>0) then
              begin
...
begin
  ...
  if Key <> 0 then begin
    ...
    if (Key in [VK_UP, VK_DOWN, VK_NEXT, VK_PRIOR, VK_F4] ) then
    begin
       if not FGrid.Visible then
       begin
         if ((ssAlt in Shift) and (Key in [VK_UP, VK_DOWN, VK_NEXT, VK_PRIOR]))
            or ((key=vk_f4) and not (ssAlt in Shift)) then begin
            DropDown;
            Key:= 0;
         end
       end
...
var tempModified: boolean;
  ...
  function AutoEditDisable: boolean;
  begin
     Result := (FFieldLink.DataSource = nil) or (DataField='') or
        (datasource.state in [dsEdit, dsInsert]) or (dataSource.autoEdit);
     Result:= Not Result;
...
begin
   ...
   if AllowClearKey and (not AutoEditDisable) then begin // 3/26/03 - Fix bug where still cleared even though AutoEdit=False
      if FGrid.Visible then begin
         ...
         if ((key=vk_delete) or (key=vk_back)) and
            ((SelText=Text) or ((Style=csDropDownList) and not FGrid.visible)) then
         begin
...
begin
   ...
   if (Picture.PictureMask<>'') and
     (Key in [#32..#254]) then
   begin
     if Editable then EnableEdit; // Enable edit mode if picture mask assigned
     if CanEdit then
     begin
        if wwApplyPictureMask(self, Picture.PictureMask,
            True, Key) then
        begin
          selStart:= length(Text);
          if FGrid.Visible or ShowMatchText then
          begin
             TempAutoDropDown:= InAutoDropDown;
             InAutoDropDown:= True;
             OnEditTimerEvent(Self);
             InAutoDropDown:= TempAutoDropDown;
          end;
        end
     end
   end;
   ...
   if (((FLastSearchKey<>NewText) or (length(FLastSearchKey)=1))
         and (ord(Key)>=32) or ((key=#8) and (loSearchOnBackspace in Options)))
         and (FGrid.visible or (ShowMatchText and CanEdit)) then
   begin
      if ShowMatchText and (LookupTable is TwwTable) and // 2/17/03
           ((LookupTable as TwwTable).Query.Count=0) and  (key=#8) and
           (loSearchOnBackSpace in Options) and
           (LookupTable as TwwTable).narrowSearch and (NewText = '') then
           (LookupTable as TwwTable).FastCancelRange
      else begin
...
//            if not (dgAlwaysShowEditor in TwwCustomDBGrid(parent).Options) then  { 6/6/98 }
             ...
             // 7/18/01 - Don't setfocus if parentform not visible (i.e. modal form that is auto-created has this issue }
             if (parentForm<>Nil) and parentform.visible then
                parent.setFocus;
         end

         // 7/28/01 - bottom line of parent grid not painting unless we invalidate row
         else if (parent is TwwCustomDBGrid) and useRightToLeftAlignment and
            FGridWasVisible then
            TwwCustomDBGrid(parent).InvalidateCurrentRow;

      end
      else begin
         DisplayValue:= OldDisplayValue;
         LookupValue:= OldLookupValue;
         if Navigator then FGrid.DoLookup(False); // 11/13/03 - Restore to original value
         DoSelectAll;
      end;
      key:= #0;
      exit;
...
begin
  if FFieldLink.Editing then begin
    FFieldLink.Modified;
...
begin
  ...
  else if not Assigned(OnPerformCustomSearch) then
  begin
   // 5/6/03 Otherwise text is cleared when form containing lookupcombo is undocked
    if (csLoading in ComponentState) or (DataSource<>nil) then
      Text := EmptyStr;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
         ...
         // 10/24/06
         if (Navigator) then
            FGrid.DisplayValue:= NewValue  // Update table pointr
         else
            FGrid.FDisplayValue := NewValue;
         //FGrid.DisplayValue:= NewValue;
      end
      else
         Text:= FGrid.DisplayValue;
    ...
    begin
       ...
       begin
          ...
          InCustomSearch:= True;
          try
             PerformCustomSearch(LookupField, NewValue, True, Found);
             if Found and (FGrid.FDisplayFld<>nil) then
             begin
                FGrid.FDisplayValue:= FGrid.FDisplayFld.asString;
             end
          finally
...
begin
   ...
   try
     ...
     if modified then begin
        ...
        if (ADataSet<>Nil) and not (ADataSet.canmodify) then exit; { 5/3/00 }
...
{            FFieldLink.Field.AsString := Text;}
            ...
            // 5/21/01 - Update lookupvalue in case dropped down or exit
            // 8/8/01 - Don't update value if Auto dropping down
            if (FGrid.Value<>Text) and (not InAutoDropDown) then
               FGrid.Value:= Text;
       end
       else begin
...
begin
  ...
  begin
//    if not Assigned(OnPerformCustomSearch) then
//    if (not InDropDownEvent) and (not InCustomSearch) then
    if (not InCustomSearch) then
    begin
...
//      if (Grid.visible) and (Parent is TwwDBGrid) then
     { 10/23/99 - Skip grid's hide controls when going into edit mode so
       that focus does not move to the grid }
...
begin
  ...
  if CanEdit then begin
     if AutoDropDown and (not Grid.visible) and (Style=csDropDownList) then exit; { 11/24/98}
...
begin
  ...
  if PreciseEditRegion or (BorderStyle=bsNone) or self.IsVistaComboNonEditable then  // check IsVista so edit box is not cleared momentarily
...
begin
   ...
   { 10/23/99 - optimization only for lookupcombo in grid }
   if (not FCombo.visible) and (parent is TwwCustomDBGrid) then exit;
...
procedure TwwDBCustomLookupCombo.WMNCPaint(var Message: TMessage);
...
//  if IsInGridPaint(self) and (message.dc<>0) then
  ...
  try
    ...
    // Have csPaintCopy support temporary grid colors
//    if IsInGridPaint(self) and (message.dc<>0) then
//       FCanvas.Font.Color:= GetTextColor(message.dc);
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  wwFillEditThemeBackground(Grid.Visible, FMouseInControl, ARect, FCanvas);
               end
               else
                  FillRect (ARect);
...
//      if (not IsTransparentEffective) and (not IsInGridPaint(self)) then
//         FillRect (ARect);
      if Showbutton and not IsInGridPaint(self) then ARect.Right:= FBtnControl.Left;
...
{           if IsInwwObjectViewPaint(self) then begin
      ...
      if DrawFocusControl then begin
         ...
         // 3/4/00 - Honor grid calccellcolors
...
begin
  ...
  begin
    ...
    { 11/28/2000 - PYW - Check based on actual work area. }
    if (Y + FGrid.Height) > GetDeskTopHeight then Y := P.Y - FGrid.Height + 1;
    ...
    // 12/16/02 - Delphi 7 needs topmost so that hint window doesn't send the listbox to the background
    // Alternatively we can disable the hint while its dropped down, but we choose
    // not to do this at this time
    WinAttribute:= 0;
{    WinAttribute:= HWND_TOPMOST; } // Setting topmost disables messages from being on top.
                                    // This is unacceptable so we instead disable hints during drop-down
                                    // Alternatively we can consider providing the developer a property for this.
...
var
  ...
  WasInAutoDropDown: boolean; // 4/30/03 - Fix bug where InAutoDropDown state was cleared due to calling PerformSearch in one of the events.
                              // Thus we save this state before the events and refer to it in this method
...
begin
 if InDropDownEvent then exit;

 try
  InDropDownEvent:= True;
  WasInAutoDropDown:= InAutoDropDown;

  // 5/21/01 - Flush typed in contents to variables before drop-down
   if {(not ShowMatchText) and}
      (FGrid.FValueFld = FGrid.FDisplayFld) and
      // 8/11/01 - Don't update tfield if autodropdown as then
      // modified flag comes out false when it should come out true
      not InAutoDropDown then
  ...
  if Assigned(FOnBeforeDropDown) then // 8/29/00 - New event
...
{  if (not LookupTable.active) then begin
  ...
  FGrid.Color:= DropDownGridOptions.Color;
  FGrid.TitleLines:= DropDownGridOptions.TitleLines;
  FGrid.TitleAlignment:= DropDownGridOptions.TitleAlignment;
  ...
  begin
    ...
    if (not WasInAutoDropDown) and wwIsClass(LookupTable.classType, 'TwwQuery') and
       (wwGetControlDataSource(LookupTable)<>Nil) then begin
       if (Datasource<>nil) and (DataSource.DataSet<>nil) and
          datasource.dataset.CanModify then { 5/3/00 }
           EnableEdit;
       FGrid.DoLookup(True); { Called in case lookupTable was moved by another control }
    end
    else if not WasInAutoDropDown then FGrid.DoLookup(True); { Called in case lookupTable was moved by another control }
    ...
    FGrid.Height := (FGrid.DefaultRowHeight + BorderWidth) *
      (FItemCount + FGrid.FTitleOffset*FGrid.TitleLines) + 2 + 3*FGrid.FTitleOffset + BorderOffset + ExtraHeight;
...
//      (FItemCount + FGrid.FTitleOffset) + 5 + BorderOffset + ExtraHeight;
    ...
    if not (loFixedDropDownHeight in Options) and // 8/17/00
      (FItemCount > FGrid.DataLink.RecordCount) and
    ...
    begin
      ...
      {8/26/98 - Height not accurate when titles are not shown }
      {10/17/00 - Height not accurate when multiple titles are shown }
      FGrid.Height := (FGrid.DefaultRowHeight + BorderWidth) *
         (FItemCount + FGrid.FTitleOffset*FGrid.TitleLines) + 2 + 3*FGrid.FTitleOffset + BorderOffset + ExtraHeight;
//      FGrid.Height := (FGrid.DefaultRowHeight + BorderWidth) *
//               (FItemCount + FGrid.FTitleOffset) + 5 + BorderOffset + ExtraHeight;
      ...
      if ShowMatchText and WasInAutoDropDown then
      begin
...
{      AssignStr(FGrid.FDisplayValue,Text);}
    ...
    FGrid.UpdateRowCount;
    FGrid.UpdateScrollBar; // 6/14/00 - RSW Fix problem with scroll thumb being too small as visiblerowcount is incorrect
    FGrid.Visible := True;
{    Invalidate;}
    ...
    { 4/12/00 - Don't unselectall if showmatch text and partial selection from start already there }
//    if (not InAutoDropDown) and
    if (not WasInAutoDropDown) and
       not (ShowMatchText and (sellength>=1) and (selstart>0)) then DoSelectAll;       { If style = DropDownList, then doselectall and showcaret required }
    ShowCaret(Handle);
...
begin
  ...
  if not FGrid.Visible then begin
     ...
     begin
        if (not ProcessingNotInList) and { 11/4/99 - Don't reset text during OnNotInList event }
           (not inList) then begin
           DisplayValue:= OldDisplayValue;
           FGrid.FValue:= OldLookupValue;
           Text:= OldDisplayValue;
     ...
     begin
        ...
        begin
           ...
           if Accept then begin
              ...
              DisplayValue:= OldDisplayValue;
              if Text<>OldDisplayValue then { 12/11/99 - Fix problem of text not getting restored }
                 Text:= OldDisplayValue;
              LookupValue:= OldLookupValue;
     ...
     if Modified and {(FGrid.FHiliteRow>=0) and}
        ...
        if UnboundCombo then begin
           ...
           begin
              ...
              begin
                 if not Accept then begin  // 12/12/02 - Don't unclear unbound control if accept is true
                 ...
                 {9/7/2001 - Don't Clear unbound control if Accept was True.
                  This implies user set lookupvalue and text in OnNotInList event.
                  Before this would prevent the unbound case from selecting the new value}
                 if not Accept then begin
                   // 3/5/03 - Fix bug where unbound has value that is not in list
                   // with style=csDropDownList. This occurs after the user selects a value
                   // from the drop-down list, and then tries to type into the control
                   if UnboundCombo and (Style=csDropDownList) and (Text<>OldDisplayValue) then
                      Text:= OldDisplayValue;
           ...
           FGrid.Visible := False;
           wwChangeFromLink(lookupTable, Modified);
           if FFieldLink.Field<>nil then // 6/26/02
...
begin
  ...
  begin
    ...
    FFieldLink.Modified;
    SetModifiedInChangeEvent:=True; // 7/31/00 - Makes sure modified is set in combo's change
    Text := FGrid.DisplayValue;
    SetModifiedInChangeEvent:=False; // 7/31/00 - Makes sure modified is set in combo's change
...
begin
  if FButtonWidth=0 then { 12/3/99 - Fix bug with button width not being used }
...
begin
  ...
  FLookupField := '';
  FHiliteRow := -1;
  inherited Options := [dgRowSelect,dgRowLinesDisableFixed];
  FixedCols := 0;
  FixedRows := 0;
  Width := 121;
  Height := 97;
...
{  FSelected.Free;
  ...
  if FNavigatorLink<>nil then
  begin
    FNavigatorLink.OnDataChange := nil;
    FNavigatorLink.Free;
    FNavigatorLink:=nil;
  end;
...
begin
  parentForm := GetParentForm(self); // 9/28/06
  if (parentForm<>nil) then HandleNeeded;
//  HandleNeeded;  {Added 5/19/96 so initialization of Text property works right away}
  ...
  begin
    ...
    begin
      ...
      if FCombo.visible or not (parent is TwwCustomDBGrid) then
//         or (IsInwwObjectView(FCombo)) then
      begin
         DoLookup(False);
...
var curTable: TDataSet;
  ...
  // 7/9/02 - Support multi-field lookup for TTable and .lookup field
  function MultiFieldLookup: boolean;
  var i: integer;
      TempFieldValues: Variant;
  begin
     TempFieldValues:= VarArrayCreate([0, links.count div 2 -1], varVariant);
     for i:= 1 to links.count div 2 do
       TempFieldValues[i-1]:= curTable.FieldByName(links[i * 2-1]).asString;
     result:= LookupTable.Locate(
          FCombo.FFieldLink.Field.KeyFields, TempFieldValues, []);
  end;
...
begin
  ...
  if not HandleAllocated then Exit;
   with (self as TwwPopupGrid) do
     if (not FCombo.visible) and (parent is TwwCustomDBGrid) then exit; {Optimization }
          ...
          if FCombo.isLookup then begin
             ...
             if (LookupTable is TwwTable) and (LTable.MasterSource<>Nil) then begin
                ...
                if (LookupTable is TwwTable) then begin  {7/10/97 - Support Non TwwTable against lookup field }
                   ...
                   if links.count>=4 then
                      FFoundValue:= MultiFieldLookup
                   else
                      FFoundValue:= FCombo.DoFindRecord(curTable.FieldByName(links[links.count-2]).asString,
                          links[links.count-1], mtExactMatch);
                end
...
begin
  ...
  if FCombo.Navigator and (FDisplayFld<>nil) then
  begin
//     FFieldLink.FieldName:= FDisplayFld.FieldName;
//     FFieldLink.DataSource:=GetLookupSource;
     FNavigatorLink.FieldName:= FDisplayFld.FieldName;
     FNavigatorLink.DataSource:= GetLookupSource;
  end;
...
{    if not (GetLookupSource.DataSet.InheritsFrom(TTable)) then
      RaiseException(SLookupTableError);}
//    if not Assigned(FCombo.OnPerformCustomSearch) then
//    (not FCombo.InDropDownEvent) and (not FCombo.InCustomSearch) then
    if (not FCombo.InCustomSearch) and (not FCombo.Navigator) then // 11/13/03 - Don't update for navigator mode then
    begin
...
begin
  ...
  begin
    FOptions := Value;
    FTitleOffset := 0;
    NewGridOptions := [dgRowSelect, dgRowLinesDisableFixed];
...
begin
  ...
  begin
    ...
    AddBiDiModeExStyle(ExStyle);  // Support bidi mode
    {$endif}
...
//    if not Assigned(FCombo.OnPerformCustomSearch) then
//    if (not FCombo.InDropDownEvent) and (not FCombo.InCustomSearch) then
    if (not FCombo.InCustomSearch) and (not FCombo.Navigator) then // 11/13/03 - Don't update for navigator mode then
    begin
      SetValue ('');   {force a data update}
...
{ TwwLookupComboButton }

procedure TwwLookupComboButton.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited MouseUp (Button, Shift, X, Y);
...
procedure TwwLookupComboButton.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // 6/29/02 - Don't reset focus or onenter/onexits can fire more than once
//        if Handle<>GetFocus then
//           windows.SetFocus(Handle);

        { 3/14/00 - MDI forms should check ActiveControl instead of focused}
        parentForm:= GetParentForm(FGrid.FCombo);
        if (parentForm<>nil) and
           (TForm(parentForm).formstyle = fsMDIChild) then
        begin
           if (GetFocus <> Handle) and
              (parentForm.ActiveControl<>FGrid.FCombo) then exit;
        end
        else if (GetFocus <> Handle) then Exit;
      end;
    end;
  end;
  inherited MouseDown (Button, Shift, X, Y);
...
procedure TwwLookupComboButton.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  inherited MouseMove (Shift, X, Y);
...
function TwwLookupComboButton.IsVistaTransparentButton: boolean;
var combo: TwwDBCustomLookupCombo;
begin
   combo:= TwwDBCustomLookupCombo(parent.parent);
   if wwUseThemes(self) and IsVista and
     ((not IsVistaComboNonEditable) {or (IsVistaComboNonEditable and combo.Grid.Visible)} ) and
...
function TwwLookupComboButton.IsVistaComboNonEditable: boolean;
begin
   result:= TwwDBCustomLookupCombo(parent.parent).IsVistaComboNonEditable;
end;

function TwwLookupComboButton.ParentMouseInControl: boolean;
begin
   result:= TwwDBCustomLookupCombo(parent.parent).MouseInControl;
end;

function TwwLookupComboButton.ParentDroppedDown: boolean;
begin
   result:= TwwDBCustomLookupCombo(parent.parent).Grid.Visible;
end;



function TwwDBCustomLookupCombo.IsVistaComboNonEditable: boolean;
begin
  result:= wwUseThemes(self) and IsVista and
            ((FStyle = csDropDownList) and not AutoDropDown);
//          not Editable;
end;
...
procedure TwwLookupComboButton.paint;
var R : TRect;
...
begin
   ...
   begin
      DefaultPaint:= True;
      FMouseInButtonControl:= self.MouseInControl;
      if (FButton.Glyph.Handle=0) or self.MouseInControl or
      ...
      { Draw edges  }
      if self.MouseInControl or (not ButtonEffects.Flat) or
         FFocused or wwisClass(Parent.classType, 'TwwDBGrid') then
      begin
         if not wwUseThemes(self.parent.parent) then
         begin
...
var searchIndex: integer;
    ...
    table: TTable;
begin
   table:= LookupTable as TTable;
   doSeqSearch:= False;
   if HasMasterSource then begin
      searchIndex:= strCount(Table.masterFields, ';')+1;
      if searchIndex >= Table.indexFieldCount then doSeqSearch:= True { Changed > to >=, 3/27/97}
      else if not wwEqualStr(Table.indexFields[searchIndex].fieldName,ASearchField) then
         doSeqSearch:= True;
   end;

   if (matchType=mtExactMatch) and
      wwEqualStr(ASearchValue, Table.FieldByName(ASearchField).asString) then
      ...
      result:= LTable.wwFindNearest(ASearchValue, strCount(Table.MasterFields, ';')+1)
   else begin
      searchIndex:= GetSearchIndex;
...
//var r: TRect;
begin
  if IsInwwObjectViewPaint(self) or   IsVistaComboNonEditable or
    (IsTransparentEffective and not FFocused) then
  begin
     Message.result:= 1;
  end
...
//   else begin
//      result:= FFieldLink.Field.Alignment;
      if FGrid.DisplayFld<>nil then
        result:= FGrid.DisplayFld.Alignment
...
procedure TwwDBCustomLookupCombo.DoMouseEnter;
begin
  try
...
procedure TwwDBCustomLookupCombo.DoMouseLeave;
var    oldtext:string;
begin
  try
...
procedure TwwDBCustomLookupCombo.CMMouseEnter(var Message: TMessage);
begin
  inherited;
  DoMouseEnter;
end;

procedure TwwDBCustomLookupCombo.CMMouseLeave(var Message: TMessage);
var r:TRect;
   pt:TPoint;
//   oldtext:string;
begin
...
procedure TwwDBCustomLookupCombo.PerformCustomSearch(
   SearchField: string; SearchValue: string;
   PerformLookup: boolean; var Found: boolean);
begin
   if Assigned(FOnPerformCustomSearch) then
      FOnPerformCustomSearch(self, LookupTable,
...
// borders will be ok
procedure TwwDBCustomLookupCombo.WMSetFont(var Message: TWMSetFont);
begin
  inherited;
...
// 3/12/02 - Support mousewheel
function TwwDBCustomLookupCombo.DoMouseWheelDown(Shift: TShiftState; MousePos: TPoint): Boolean;
var key: word;
begin
  result := False;
  if not FGrid.Visible then exit;
  key:= vk_down;
  if (GetLookupSource <> nil) and (GetLookupSource.DataSet <> nil) then
     if (GetLookupSource.DataSet.Active) then
     begin
         EnableEdit;
...
function TwwDBCustomLookupCombo.DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint): Boolean;
var key: word;
begin
  result := False;
  if not FGrid.Visible then exit;
...
Procedure TwwDBCustomLookupCombo.SetNavigator(Value: boolean);
begin
  if Navigator <> Value then
  begin
     FNavigator:=Value;
     if Value then with FGrid do begin
       if FNavigatorLink=nil then  // 4/9/03 - Always initiailze
       begin
          FNavigatorLink:= TFieldDataLink.Create;
          FNavigatorLink.Control:= FCombo;
       end;
       FNavigatorLink.OnDataChange := NavigatorDataChange;
       if (FDisplayFld<>nil) then
       begin
         FNavigatorLink.FieldName:= FDisplayFld.FieldName;
         FNavigatorLink.DataSource:= GetLookupSource;
       end
...
procedure TwwDBLookupList.NavigatorDataChange(Sender: TObject);
begin
  if not (csLoading in ComponentState) and FCombo.Navigator and
     (FDisplayFld<>nil) and (FDisplayFld.DataSet<>nil) then    // 6/10/03 - Don't execute code if no associated dataset (other gpf)
  begin
     FCombo.text:= FDisplayFld.DisplayText;
     FCombo.FGrid.Value:= FValueFld.asString;
     FCombo.SelectAll;
  end
end;


procedure TwwDBCustomLookupCombo.SetController(Value: TwwController);
begin
