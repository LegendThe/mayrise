...
//    FButtonGlyph: TBitmap;
    ...
    procedure WMMOUSEMOVE(var Message: TMessage); message WM_MOUSEMOVE;  //Bryan 25/10/2016
...
//    property ButtonTransparent: boolean read FFlatButtonTransparent write SetFlatButtonTransparent default false;
    ...
    property MouseInControl: boolean read FMouseInButtonControl default false; //Bryan 26/10/2016
...
Procedure TwwDBCustomCombo.DrawButton(Canvas: TCanvas;
                                      R: TRect;
                                      State: TButtonState;
...
{$ifdef win32}
  ...
  if State=bsDown then
    begin
      Canvas.Brush.Color:=$BD7944;
      Canvas.FillRect(R);
    end
  else
    if FMouseInButtonControl  and (State<>bsDisabled) then //Hovered over button
      begin
        Canvas.Brush.Color:=$CECECE;
        Canvas.FillRect(R);
      end;

  if ButtonStyle=cbsDownArrow then
     wwDrawDropDownArrow(Canvas, R, bsUp{State}, Enabled, ControlState)
  else
     wwDrawEllipsis(Canvas, R, bsUp{State}, Enabled, true{Transparent},
                    true {ButtonEffects.Transparent} ,
                    ControlState);
  {$ELSE}
...
function TwwDBCustomCombo.IsVistaComboNonEditable: boolean;
begin
   result:= inherited IsVistaComboNonEditable;
   if result and ButtonEffects.Flat then
      result:=false;
end;

procedure TwwDBCustomCombo.WMMOUSEMOVE(var Message: TMessage);   //Bryan 25/10/2016
begin
  Inherited;
...
procedure TwwDBCustomCombo.WMMOUSELEAVE(var Message: TMessage);  //Bryan 25/10/2016
begin
  Inherited;
  FMouseInButtonControl := false;
  Invalidate;
