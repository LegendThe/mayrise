...
// 02/8/06 - Fix memo positioning problem when shown
}
...
uses classes, messages, db, dbtables, forms, wwdbedit, wwdatsrc, stdctrls, extctrls, controls,
     wwcommon, dbctrls, windows, graphics, wwstr, wwtypes, dialogs, menus,
...
//     FNavigatorVisibleButtons: TButtonSet;
  ...
  protected
     ...
     procedure SetNavigatorVisibleButtons(Value: TNavButtonSet);
     {$else}
...
//     procedure NavVisButtonsWriteProc(Writer: TWriter);
  ...
  published
     ...
     {$ifdef wwDelphi2006Up}
     property NavigatorVisibleButtons: TNavButtonSet write SetNavigatorVisibleButtons stored False;
     {$else}
...
// 4/9/08 - Fix vista label problem
{$ifndef wwDelphi2006Up}
TwwScrollBox = class(TScrollBox)
protected
  procedure PaintWindow(DC: HDC); override;  // labels not shown otherwise in vista themes
end;

procedure TwwScrollBox.PaintWindow(DC: HDC);
begin
  //  Do nothing
end;
{$endif}
...
begin
   ...
   // vista
   FixedEditWidthConst:=GetSystemMetrics(sm_cxframe)*2 + 12;
...
//           Width := 240;
      ...
      {$ifndef wwDelphi2006Up}
      ScrollBox:= TwwScrollBox.create(RecordPanel);
      {$else}
...
//      TwwRecordViewScrollBox(ScrollBox).Dialog:= self;
      ...
      for i:= 0 to FControlList.count-1 do begin
         if (TWinControl(FControlList.items[i]) is TCustomEdit) then // 10/21/02 - Only set if TCustomEdit
            TCustomEdit(FControlList.items[i]).Modified := False
         else if (TWinControl(FControlList.items[i]) is TWinControl) then
         begin
            if TWinControl(FControlList.Items[i]).HandleAllocated then // else send windows message if handle allocated
               SendMessage(TWinControl(FControlList.Items[i]).Handle, EM_SETMODIFY, Byte(False), 0)
         end
      end;
...
begin
  ...
  begin
     ...
     form := wwGetOwnerForm(self); // 5/20/05 - Check for nil form in case of frame
     if (form<>nil) then
     begin
        with form do begin
...
procedure TwwRecordViewDialog.SetNavigatorVisibleButtons(
   Value: {$ifdef wwDelphi2006Up} TNavButtonSet {$else} TButtonSet {$endif});
var i: TNavigateBtn;
begin
  Exit;
  FNavigatorButtons := [];
...
procedure TwwRecordViewDialog.SetController(Value: TwwController);
begin
   ...
   begin
      ...
      begin
         FDefaultEditFrame.Assign(FController.Frame);
      end
   end
end;
...
function TwwRecordViewDialog.CreateDefaultEditControl(AOwner: TComponent): TCustomedit;
begin
   result:= TwwDBEdit.create(AOwner);
end;
