...
//    property OnValidationErrorUsingMask: TwwPictureValidationError read FOnValidationErrorUsingMask write FOnValidationErrorUsingMask;
    ...
    property LeftCol;
    property PadColumnStyle: TwwPadColumnStyle read FPadColumnStyle write FPadColumnStyle default pcsPlain;
...
var ac: TWinControl;
  ...
  begin
    ...
    with ac do
      with Selection do
        Result := ((StartPos = 0) or ((EndPos = StartPos) and (dgAlwaysShowEditor in Grid.Options))) and
          (EndPos = GetTextLen);
  end;
  ...
  begin
    ...
    with ac do
      with Selection do
        Result := (StartPos = 0) and
        (((EndPos = 0) and (dgAlwaysShowEditor in Grid.Options)) or (EndPos = GetTextLen) {or (isMasked and (EndPos=1))});
...
{    else if (wparam = vk_f2) and
    ...
    begin
        ...
        begin
          if (wparam in [VK_HOME, VK_END, VK_LEFT, VK_RIGHT]) {and grid.EditorMode }then exit;
        end;
...
//            r.Left:= wwMin(ARect.Right-2, ARect.Left + ACanvas.TextWidth(s) + 2);
            r.Left:= ARect.Left + ACanvas.TextWidth(s) + 2; // 9/23/03 - Bidi - Presever right-side display when cell smaller than text
            r.Right:= ARect.Left;
         end
         else begin
            Flags:= AlignFlags[taLeftJustify] {or DT_RTLREADING};
  ...
  begin
     ...
     begin
        ...
        if (lastbookmark<>nil) and
           datasource.dataset.eof and datasource.dataset.bof then
           ShouldUpdateFooter:= True
...
begin
   ...
   if FGroupFieldName<>'' then invalidate;
end;
...
var curBookmark: TBookmark;
    ...
    begin
      ...
      begin
         ...
         if GroupFieldName<>'' then result:= true;
      end
    end;
{    function SpecialCellChanged: boolean;
    begin
   ...
   try
      if (Datasource<>nil) and (Datasource.dataset<>Nil) and
         ...
         (Assigned(FOnRowChanged) or
          ...
          Assigned(FOnUpdateFooter) or
          SpecialCellChanged) then
      begin
...
begin
  ...
  begin
     ...
     // 3/17/06
     IsDateTime:=
            (Fields[SelectedIndex] is TDateTimeField) or
            ((Fields[SelectedIndex]<>nil) and
             (Fields[SelectedIndex].DataType in [ftDate, ftDateTime, ftTimeStamp, ftTime]) );
...
begin
  ...
  procedure TwwCustomDBGrid.DoOnBeforeDrawCell(ATextWide: WideString; var DrawCellInfo: TwwCustomDrawGridCellInfo);
  var AText: string;
  begin
       AText:= ATextWide;

       if GroupFieldName<>'' then with DrawCellInfo do
       begin
         // 9/3/03 - Make case insensitive for GroupFieldName
         if (Field<>nil) and (AnsiUpperCase(Field.FieldName) = AnsiUpperCase(FGroupFieldName)) and
            (DrawCellInfo.dataRow>=0) then
         begin
            if GetNextRecordText(Field.FieldName, AText) and
  ...
  procedure TwwCustomDBGrid.DoOnAfterDrawCell(var DrawCellInfo: TwwCustomDrawGridCellInfo);
  var curField: TField;
      AText: String;
      OldActive: integer;
  begin
    ...
    if Assigned (OnAfterDrawCell) or (FGroupFieldName<>'') then
    begin
       ...
       try
          ...
          if (FGroupFieldName<>'') and
             (DataSource<>nil) and (DataSource.DataSet<>nil) then with DrawCellInfo do
          begin
            curField:= DataSource.DataSet.FindField(FGroupFieldName);
            if (curField<>nil) and
              (DrawCellInfo.dataRow>=0) then
            begin
...
//  ACursor: TPoint;
  ...
  procedure Display(S: WideString; Alignment: TAlignment; Transparent: boolean=True);
  const
    Formats: array[TAlignment] of Word = (DT_LEFT, DT_RIGHT,
      DT_CENTER or DT_WORDBREAK or DT_EXPANDTABS or DT_NOPREFIX);
  var
  ...
  begin
    ...
    if (ARow<0) then begin
       ...
       begin
          ...
          WriteTextLines(Canvas, ATextRect, 2+ButtonOffset, startOffset+ButtonOffset, PWideChar(s),
	  ...
	  WriteTextLines(Canvas, TempRect, 2{+ButtonOffset} { 2/20/99 - Comment out ButtonOffset, Text too far to right before },
              (NormalPad div 2)+ButtonOffset, PWideChar(s), Alignment, WriteOptions); { 5/21/98 }
       end
       else begin
          ...
          if not (HaveAnyRowLines) then { 1/8/99 - Fix descendor cutoff problem when wordwrap is true }
             WriteTextLines(Canvas, TempRect, 2+ButtonOffset,
                (NormalPad div 2)+ButtonOffset, PWideChar(s), Alignment, WriteOptions)
          else WriteTextLines(Canvas, TempRect, 2+ButtonOffset,
                (NormalPad div 2)+1+ButtonOffset, PWideChar(s), Alignment, WriteOptions);
       end;
    end;
  end;
  ...
  var Details: TThemedElementDetails;
     ...
     begin
        result:= false;
        if MouseOverGroupTitle and (ACol>=0) and (Columns[ACol].GroupName<>'') then
        begin
  ...
  {$ifdef wwUseThemeManager}
       ...
       if pressed then
          ...
          begin
             ...
             tempRect.Right:= tempRect.Right + 5; // Match title color - but don't show vertical line
             ...
             (not UseDragCanvas) then
          begin
             if (ACol<0) or (Columns[ACol].GroupName<>'') then //5/9/03 - Prevent index out of range if ACol<0
             begin
       ...
       ThemeServices.DrawElement(Canvas.Handle, Details, tempRect, @tempClipRect);
  {$endif}
  end;
       ...
       DoOnBeforeDrawCell(AText, DrawCellInfo);
{
       if GroupFieldName<>'' then with DrawCellInfo do
       begin
         if (Field<>nil) and (Field.FieldName = FGroupFieldName) and
            (DrawCellInfo.dataRow>=0) then
         begin
            if GetNextRecordText(Field.FieldName, AText) and
               (AText = Field.asstring) then
       ...
       if
          ...
          (FastColumns[DrawCellInfo.DataCol].ColumnFlags<>nil) then with DrawCellInfo do
       begin
         if not DefaultDrawHorzBottomLine then
            FastColumns[DataCol].ColumnFlags[DataRow]:= FastColumns[DataCol].ColumnFlags[DataRow] or wwDisableDrawBottomLine
         else
            FastColumns[DataCol].ColumnFlags[DataRow]:= FastColumns[DataCol].ColumnFlags[DataRow] and not wwDisableDrawBottomLine;
         if not DrawCellInfo.DefaultDrawHorzTopLine then
            FastColumns[DataCol].ColumnFlags[DataRow]:= FastColumns[DataCol].ColumnFlags[DataRow] or wwDisableDrawHorzTopLine
         else
            FastColumns[DataCol].ColumnFlags[DataRow]:= FastColumns[DataCol].ColumnFlags[DataRow] and not wwDisableDrawHorzTopLine;
       end;
...
//        Canvas.FillRect(ARect);
        ...
        begin
           DoThemeBackground(False, [{loNoVertLine}], ARect) // 5/1/03 - Don't use loNoVertLine style
        end
        else if DrawCellInfo.DefaultDrawBackground then
    ...
    begin
      ...
      if UseThemesInTitle then // ThemeServices.ThemesEnabled then
      begin
         TempRect:= Rect(ARect.Left-1, ARect.top, ARect.Right, ARect.Bottom+1); // 5/1/03
...
//       if not PaintOptions.HaveBackgroundForDataCells then FillRect(ARect)
	...
	begin
           ...
           // 4/30/07 - Fix multi-select paint bug with themes enabled (when control has focus).
           if wwUseThemes(self) and IsChild(self.handle, GetFocus) and (not IsSelected) and
              (ARow=dbRow(Row)) and (ACol=dbCol(Col))            // 11/15/07 - Only skip if active cell
           then
...
{             if ThemeServices.ThemesEnabled then
             begin
              ...
              begin
                 ...
                 begin
                    TempRect:= Rect(ARect.Left, ARect.Top-1, ARect.Right, ARect.Bottom+1);
                    FillWithBlendBitmap(TempRect, CurRelRow)
                 end
              end
          end
        end;
	...
	if DefaultDrawing and (not SkipDefaultDrawing) and
           ...
           begin
             ...
             else begin // Numeric so ignore bidi issues
               if Alignment = taLeftJustify then  Alignment := taRightJustify
               else if Alignment = taRightJustify then Alignment := taLeftJustify;
               Display(Value, Alignment, (not Highlight) and (not ChangedBrushColor));
             end
           end
           else
...
function TwwCustomDBGrid.DittoField(SelectedField: TField; Direction: TwwDittoDirection=wwDittoPrior): boolean; // New method
var AllowDitto: boolean;
    Value: string;
    ...
    begin
       ...
       begin
          if SelectedField.ReadOnly then exit;
          if ColumnByName(SelectedField.FieldName).ReadOnly then exit;
       end;

       if (wwdoSkipBlobFields in DittoAttributes.Options) and (SelectedField is TBlobField) then exit;
    ...
    begin
       ...
       begin
           ...
           begin
              AllowDitto:= True;
              try
                FOnDitto(self, FDataLink.DataSet, SelectedField,
                  Value, AllowDitto);
                if not AllowDitto then exit;
              finally
              end;
           end;
  ...
  end;
{
  ...
  function CheckAutoSizeColumn: boolean;  // Return true if auto-sized column
  var gc:TGridCoord;
      pt:TPoint;
  begin
...
//                LeftCol:= FixedCols;
	  ...
	  if NewRow then begin
          ...
          // Support url links
          if (x>=0) and (Button = mbLeft) and (x<=FCacheColInfo.Count-1) and // 10/4/04
...
Procedure TwwCustomDBGrid.SetRegex(FieldName: string; Mask: string; CaseSensitive: boolean; ErrorMsg: string);
begin
...
begin
    ...
    begin
       tempField:=GetColField(dbCol(ACol));
       if tempField=nil then result:= False // 10/16/04 - make sure is calculated
       else result:= (lowercase(tempField.fieldName)='selected') and tempField.calculated;
    end
    else result:= False;
end;
...
begin
  ...
  begin
     ...
     begin
        OldFontSize:= Canvas.Font.Size;  // 8/25/03 - Don't rely upon unknown font height
...
//       FooterCellRect:= CellRect(ACol, RowCount-1);
       ...
       begin
          if ((ACol>0) and (FooterCellRect.Left<CellRect(0,0).Right)) then exit;
       end;
...
//          SetTextColor(ACanvas.Handle, ACanvas.Font.Color);
          ...
          if UseRightToLeftAlignment then  // 4/15/03 - Fix bidi mode alignment problem
          begin
            if (dbCol(ACol) < FieldCount) and (Fields[dbCol(ACol)].Alignment = taRightJustify) then begin
              ...
              FooterCellRect.Right:=Temp;
               ...
               Temp:= footerCellRect.left;
               FooterCellRect.Left:= wwMin(FooterCellRect.Right, FooterCellRect.Left + ACanvas.TextWidth(FooterText) + 2);
               FooterCellRect.Right:= Temp;
            end;
...
//          PaintClipRect:= (inherited Canvas).ClipRect;
          ...
          if not ((gdFocused in DrawState) and (CurCol=Col)) or
            ...
            (TwwCacheColInfoItem(FCacheColInfo[ACol]).ControlType = 'CheckBox') then // 3/23/04 - Still paint for checkbox
          begin
            curRect:= Where;
...
begin
   ...
   IsDateTime:=
            (DataLink.Fields[dbCol(col)] is TDateTimeField) or
            ((DataLink.Fields[dbCol(col)]<>nil) and
             (DataLink.Fields[dbCol(col)].DataType in [ftDate, ftDateTime, ftTimeStamp, ftTime]) );
...
//           Canvas.Pen.Color:= clBtnHighlight;
 ...
 begin
    ...
    begin
      ...
      begin
         if (TempCol<ColItems.count) and (FastColumns[TempCol].ColumnFlags<>nil) and
            (TempRow<256) and
...
//   bidioffset:=0;
   ...
   if (not HideAllLines) and (dataLink<>nil) and ( datalink.active) and
     ...
     begin
        ...
        if FFocused or
           ((Columns[Col-FIndicatorOffset].ColumnFlags[Row-FTitleOffset] and wwDisableDrawBottomLine)=0) then
           if (GroupFieldName='') then // 5/11/06 - Group line missing for active cell before when it loses focus
             Canvas.PolyLine([Point(ARect.Left, currowheight),Point(ARect.Right, currowheight)])
     end
     else begin
        if (GroupFieldName='') {or FFocused }then // 5/11/06 - Group line missing for active cell before when it loses focus
           Canvas.PolyLine([Point(ARect.Left, currowheight),Point(ARect.Right, currowheight)]);
     end
   end;
      ...
      begin
        for i:= 0 to VisibleRowCount do begin
          ...
          begin
            ...
            if (i=VisibleRowCount) and (i>=TitleOffset) and (dgIndicator in Options) then
            begin
...
//         if (ThemeServices.ThemesEnabled) then continue;
   ...
   begin
       for i:= 0 to LeftCol+VisibleColCount do begin
         ...
         if UseThemesInTitle then continue; // 5/1/03 - Indicator column should not paint lines with themes
...
begin
   result:=
      ...
      // can't edit grid
      ((Col<FCacheColInfo.count) and
       TwwCacheColInfoItem(FCacheColInfo[Col]).AlwaysPaints))
...
begin
   ...
   if FDataLink.ActiveRecord<0 then exit;
   if FDataLink.ActiveRecord>=FDataLink.RecordCount-1 then exit;
...
Procedure TwwcustomDBGrid.WriteTextLines(ACanvas: TCanvas;
      const ARect: TRect; DX, DY: Integer; S: PWideChar; Alignment: TAlignment;
       WriteOptions: TwwWriteTextOptions);
var NonWideStr: string;
begin
  NonWideStr:= S;
  wwWriteTextLines(ACanvas, ARect, DX, DY, PChar(NonWideStr), Alignment, WriteOptions);
end;

{function TwwcustomDBGrid.CreateDataLink(AOwner: TComponent): TwwGridDataLink;
begin
   result:= TwwGridDataLink.create(self);
end;
}
