...
begin
   ...
   {$ifdef ver130}
   HelpFile := 'IP4000D5.HLP';
   {$endif}

   {$ifdef ver135}
   HelpFile := 'IP4000C5.HLP';
   {$endif}

   {$ifdef ver140}
   HelpFile := 'IP4000D6.HLP';
   {$endif}

   {$ifdef ver150}
   HelpFile := 'IP4000D7.HLP';
   {$endif}
...
begin
   ...
   {$ifdef ver120}
   WinHelp(Handle, 'IP50D4.HLP', HELP_KEY, longint(@context));
   {$endif}
   {$ifdef ver125}
   WinHelp(Handle, 'IP50C4.HLP', HELP_KEY, longint(@context));
   {$endif}
   {$ifdef ver130}
   WinHelp(Handle, 'IP4000D5.HLP', HELP_KEY, longint(@context));
   {$endif}
   {$ifdef ver135}
   WinHelp(Handle, 'IP4000C5.HLP', HELP_KEY, longint(@context));
   {$endif}
   {$ifdef ver140}
   WinHelp(Handle, 'IP4000D6.HLP', HELP_KEY, longint(@context));
   {$endif}
   {$ifdef ver150}
   WinHelp(Handle, 'IP4000D7.HLP', HELP_KEY, longint(@context));
   {$endif}
end;
*)
...
begin
   ...
   begin
      ...
      if (PropInfo<>nil) and (PropInfo^.Proptype^.Kind in [tklString, tkWString]) then begin // 12/13/07
      {$ELSE}
...
begin
   Result:= False;
   PropInfo:= Typinfo.GetPropINfo(ctrl.ClassInfo,'SessionName');
   if (PropInfo<>nil) and (PropInfo^.Proptype^.Kind = tklString) then begin
      SetStrProp(Ctrl,PropInfo,df);
      result:= True;
   end
...
(*
      if false and (ACanvas.CanvasOrientation = coRightToLeft) then { 9/25/98 - Support bidi mode }
      begin
...
procedure UnloadRegexLibrary;
begin
   if (FLibRegexHandle<>0) then FreeLibrary(FLibRegexHandle);
   @regexMatchProc:=nil;
   @regexSyntaxValidProc:=nil;
end;

function LoadRegexLibrary: boolean;
var RegexModuleName: PAnsiChar;
  OldError: Longint;
begin
  if FLibRegexHandle<>0 then
  begin
    result:=true;
    exit;
  end;

  RegexModuleName:= 'WWREGEX.DLL';
  OldError := SetErrorMode(SEM_NOOPENFILEERRORBOX);
  FLibRegexHandle := LoadLibrary(RegexModuleName);
  if (FLibRegexHandle > 0) and (FLibRegexHandle < HINSTANCE_ERROR) then FLibRegexHandle := 0;
  SetErrorMode(OldError);
  result:= (FLibRegexhandle<>0);

  if (not result) and (not messageShown) then
  begin
     ShowMessage('Unable to load WWREGEX.DLL for regular expression support.');
     messageShown:=true;
  end;


  @regexMatchProc:= GetProcAddress(FLibRegexHandle, 'RegexMatch');
  @regexSyntaxValidProc:= GetProcAddress(FLibRegexHandle, 'RegexSyntaxValid');
...
begin
    ...
    res:= RegexMatchProc(pchar(pattern), icasePattern, pchar(input));
    if (res<0) then result:=false
    else if res=0 then result:=false
    else result:=true;
end;
...
begin
    ...
    res:= RegexSyntaxValidProc(pchar(pattern));
...
initialization
   ...
   MutexHandle:= OpenMutex(MUTEX_ALL_ACCESS, false, 'IP1538C91CD4785DD9');
