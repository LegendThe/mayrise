...
begin
  inherited Change;
  if FCombo.CanEdit and Visible then begin  // 8/28/07 - Only set when visible (prevents problem where cnnotify sets value when monthcalendar is dropped down
     FCombo.EnableEdit;
...
begin
    inherited DoExit;
    PostMessage(Handle, WM_DATEPICKER_DESTROYCALENDAR, 0, 0);
    {
    FCalendar.Free;
    FCalendar := nil; }
end;
...
//  inherited DoEnter;
{  inherited }DoEnter; // 10/5/01 - Call DoEnter so that it still calls subclassed DoEnter
...
{
procedure TwwDBCustomDateTimePicker.WMNCPaint(var Message: TMessage);
...
//      ARect:= GetClientEditRect;
      ...
      if DrawFocusControl then begin
         ...
         begin
            ...
            if ParentGridFocused(self) then
               CalcCellColorsEvent(parent, FDataLink.Field, [], True, Font, Brush); {new code}
         end
//         if ParentGridFocused(self) then
//	    TwwCheatGridCast(Parent).DoCalcCellColors(FDataLink.Field, [], True, Font, Brush); {new code}
      end;
...
//             TwwDBGrid(parent).HaveAnyRowLines then
            Indent:=2
         else Indent:=1
      end
...
//  SetCursor(0); { Don't set cursor }
  ...
  CurrentDigitEditDateTime:= 0; // 11/7/02
...
//       DateTime := Now;
       ...
       SetModified(True);
       if self.text='' then
          if (sysutils.date <= trunc(GetValidMaxDate)) and
             (sysutils.date >= trunc(GetValidMinDate)) then begin
             // 2/13/2002 - Respect new property AutoFillDateAndTime.
             if AutoFillDateAndTime then begin
               if (isDateOnlyField) or (isCFormat) then self.DateTime := sysutils.Date
               else self.DateTime := sysutils.Now;
             end
             else self.DateTime := sysutils.Date; { RSW - 11/5/98 - Check if text is still blank}
          end
          else self.DateTime := self.GetValidMinDate; { PYW - 10/12/00 - Set to MinDate if not in valid range}
    end;
...
//    HighlightToken(DateTime);
    ...
    if FCalendarOptions.Alignment = wwdtaLeft then begin
       ...
       TempButtonWidth := max(GetSystemMetrics(SM_CXVSCROLL), 15);
       X := P.X + Self.Width - TempButtonWidth - FCalendar.Width - 1;
       if P.X + FCalendar.Width >= Screen.Width then X := P.X + Self.Width - 1 - FCalendar.Width;
       if X < 0 then X := 0;  //5/30/2000 - PYW - Make sure calendar is visible.
    end
    else begin
       TempButtonWidth := max(GetSystemMetrics(SM_CXVSCROLL), 15);
       X := P.X + Trunc((Self.Width - FCalendar.Width -TempButtonWidth-1)/2);
       if P.X + FCalendar.Width >= Screen.Width then X := P.X + Self.Width - 1 - FCalendar.Width;
       if X < 0 then X := 0;  //5/30/2000 - PYW - Make sure calendar is visible.
    end;
...
begin
  if (FDataLink.Field<>Nil) then begin
    ...
    if (FDatalink.Field.isNull) then begin
       ...
       FDateTime := TDateTime(FDataLink.Field.AsDateTime);  //10/26/2001 - PYW - This helps support TSQLDateTimeType...
...
//       update; { 9/19/99 - Comment out as this prevents background from getting erased for transparency}
    end;
  end
...
{               if not LessThanOrEqual(aMinDate,NewDate) then
  ...
  function GetNextMonth(ChangeDayOrYear: boolean): boolean;
  var i:integer;
      tempm:word;

      function NewMonthIsValid(NewMonth: integer): boolean;
      var tempy, tempm, tempd: word;
      begin
         DecodeDate(Trunc(DateTime),tempy,tempm,tempd);
         result:= (tempd<=DaysThisMonth(NewMonth, tempy));
      end;
  ...
  begin
     ...
     for i:= tempm +1 to 12 do begin
       ...
       begin
          // 1/11/01 - Fix problem when entering 31/Jul, which yields 30/June
          if ChangeDayOrYear or NewMonthIsValid(i) then begin
...
//            cursellength:=sellength;
  ...
  begin
    if Parent.Visible then Parent.setFocus;  //12/12/2001 - Make certain parent is visible before setting focus to it.
...
//  if (parent is TCustomGrid) then begin
     case Key of
       ...
       VK_LEFT: if IsInwwObjectView(self) then
                begin
                   if (not IsDroppedDown) and (Ctrl or LeftSide) then SendToObjectView
                end
                else if ForwardMovement and (Ctrl or LeftSide) then SendToParent;
       VK_RIGHT: if IsInwwObjectView(self) then
                 begin
                    if (not IsDroppedDown) and (Ctrl or RightSide) then SendToObjectView
                 end
...
//  else
  ...
  begin
    ...
    if (Text = '') and
       ((DateTime = 0.0) or
        (not isTimeOnlyField and (Trunc(DateTime)=0)) or { 6/7/00 - RSW - If datetime field but date is 0, then still initialize}
    ...
    begin
       ...
       begin
          ...
          if ((Trunc(FMinDate)=0) or (Trunc(sysutils.Date) >= FMinDate)) and
             ...
             // 2/13/2002 - PYW - Need to initialize time as well when time is part of displayformat
             //                   when AutoFillDateAndTime is True (new property).
             if AutoFillDateAndTime then begin
               if (isDateOnlyField) or (isCFormat) then FDateTime := sysutils.Date
               else FDateTime := sysutils.Now;
             end
             else begin
                FDateTime:= sysutils.Date; {RSW - Autofill date and time separately}

                // 7/1/02 - Make day 1 if no day in date specification
                if AnsiPos('d', GetFormatStr)=0 then
                begin
                   DecodeDate(FDateTime,y,m,d);
                   FDateTime:= EncodeDate(y, m, 1);
                end;
             end;
...
{pw 6/26/98 - Changed Date/Time Seperators to AutoAdvance}
    ...
    case char(datecode) of
      '-': key:= vk_down;
     '+','=': key:= vk_up;
      '/': { 10/10/99 - Support keyboard / to auto-advance }
...
{                if m>mm then m:=mm;
       ...
       begin
          ...
          begin
            ...
            begin
               if not GetNextMonth(False) then
                  GetNextMonth(True)
            end
...
//                      if (y=0) then begin
                      ...
                      begin
                        ...
                        if LessThanOrEqual(aMinDate,TempDate) and LessThanOrEqual(TempDate,aMaxDate) then
                           CurrentDigitEditDateTime := Trunc(EncodeDate(y,m,d)) + Frac(EncodeTime(h,n,sec,msec)) { 8/16/00}
                      end
                   end;
                end;
             end
     ...
     begin
        ...
        CurrentDigitEditDateTime:=0; { 8/16/00 }
...
begin
  ...
  SetModified(False);
  FPos := 1;  //1/4/2000 - Need to reset position when resetting Date Information
end;
...
//        if not CanEdit then key:=#0;
    ...
    ^H, ^X, #32..#47, #58..#255: // 10/4/06 - Support Ctrl-V for paste
    //^H, ^V, ^X, #32..#47, #58..#255:
      begin
        key:=#0;
      end;
...
function TwwDBCustomDateTimePicker.IsVistaComboNonEditable: boolean;
begin
  result:= false;
end;
...
function TwwDateComboButton.IsVistaTransparentButton: boolean;
var combo: TwwDBDateTimePicker;
begin
   combo:= TwwDBDateTimePicker(parent.parent);
...
function TwwDateComboButton.IsVistaComboNonEditable: boolean;
begin
   result:= false;
end;

function TwwDateComboButton.ParentMouseInControl: boolean;
begin
   result:= TwwDBDateTimePicker(parent.parent).MouseInControl;
end;

function TwwDateComboButton.ParentDroppedDown: boolean;
begin
   result:= TwwDBDateTimePicker(parent.parent).IsDroppedDown;
end;
...
begin
   ...
   begin
      ...
      if self.MouseInControl or (not ButtonEffects.Flat) or //FButton.Flat) or
         FFocused or wwisClass(Parent.classType, 'TwwDBGrid') then
      begin
         if not wwUseThemes(self.Parent.Parent) then
         begin
...
begin
  ...
  if IsInwwObjectView(self) then begin
     ...
     Loc.Top := 2;
     if (parent<>nil) and wwIsClass(Parent.classType, 'TwwDBGrid') and not
//     if (parent is TwwDBGrid) and not
        HaveAnyRowLines(self) then
//        TwwDBGrid(parent).HaveAnyRowLines then
        Loc.Top:=1;
     Loc.Left := 2;
...
begin
  ...
  if not isTimeOnlyField then
     Result := Trunc(TDate(FDateTime))          //3/3/2002 - PYW - Make sure that Date is integer type.
  else Result := 0;
end;
...
begin
  ...
  if not isDateField then
     Result := Frac(FDateTime)
                                // 7/17/02 (Don't return date)
                                // Fixes bug in TwwFilterDialog when entering
                                // ending date range.  Clicking on ViewSummary failed
                                // in this case for some countries
...
begin
  if FButtonWidth=0 then  { 10/6/2000 - Fix bug with button width not being used.PYW}
...
begin
  ...
  // Comment out as datetimepicker in large grid painting wrong background
  // color (i.e. export demo)
{  else if ThemeServices.ThemesEnabled and
...
Procedure TwwDBCustomDateTimePicker.UpdatebuttonGlyph;
begin
...
function TwwDBCustomDateTimePicker.GetButtonGlyph: TBitmap;
begin
...
// 8/17/00 - Preserve AM/PM value
Function TwwDBCustomDateTimePicker.GetNewHour(oldhour: integer; digit: integer): integer;
begin
   if isAMPM(getformatstr) then begin
      if (digit=12) and (oldhour<12) then result:=0 { 12:00 AM}
      else if (oldhour>=12) and (digit<12) then result:= digit+12
      else result:= digit;
   end
   else result:= digit;
end;

procedure TwwDBCustomDateTimePicker.CMMouseEnter(var Message: TMessage);
begin
  inherited;
  DoMouseEnter;
end;

procedure TwwDBCustomDateTimePicker.CMMouseLeave(var Message: TMessage);
var r:TRect;
    pt:TPoint;
begin
  GetCursorPos(pt);
...
procedure TwwDBCustomDateTimePicker.DoMouseEnter;
begin
  try
...
procedure TwwDBCustomDateTimePicker.DoMouseLeave;
begin
  try
...
// borders will be ok
procedure TwwDBCustomDateTimePicker.WMSetFont(var Message: TWMSetFont);
begin
  inherited;
...
procedure TwwDBCustomDateTimePicker.SetController(Value: TwwController);
begin
...
initialization
  WM_DATEPICKER_DESTROYCALENDAR := RegisterWindowMessage('IPDATEPICKERDESTROYCALENDAR');
end.
