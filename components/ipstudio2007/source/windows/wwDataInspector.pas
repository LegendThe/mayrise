   ...
   begin
      ...
      begin
         ...
         begin
            ...
            if (ptinrect(cr, sp)) and (Col=ACol) and not (csPaintCopy in ControlState) then begin
               HotCol:= ACol;
               HotRow:= ARow;
   ...
   try with Obj do begin
      ...
      if ACol=0 then begin
         ...
         begin
            ...
            else Canvas.Font.Color:= CaptionFont.color;
         end;
...
{            if FFocused then
            ...
            else }if (Field is TBlobField) and (TBlobField(Field).datatype in [ftMemo, ftOraBlob, ftOraCLob
            ...
            begin
               ...
               try
                  ...
                  begin
                     if (Field=nil) then begin // 3/29/03 - Fix bug where mapped text not working in unbound
                        StoredValue:= EditText;
                     ...
                     begin
                        ...
                        if not PickList.MapList then // 10/7/02 - DefaultCombobox not using display text before
                           PaintText:= Field.DisplayText
                        else
                           PaintText:= obj.GetMappedPaintText(StoredValue);
                     end
...
begin
   ...
   else if (obj.Field is TBlobField) and (TBlobField(obj.Field).DataType in [ftMemo, ftOraBlob, ftOraCLob
    {$ifdef wwDelphi2006Up}
    , ftWideMemo
    {$endif} ]) then
   begin
...
//   if UseDefaultEditor then ShowEditor;
   if UseDefaultEditor then InvalidateEditor; // 7/29/04 - Call InvalidateEditor instead of ShowEditor
  ...
  if {((CurrentCustomEditRow<>ARow) or (CurrentCustomEditCol<>ACol)) and }(CurrentCustomEdit<>nil) then
   ...
   begin
       ...
       begin
          ...
          begin
             if (DataSource=nil) and (Style=csDropDownList) then CloseUp(True)
          end;
...
//      if TEdit(CurrentCustomEdit).modified then
       ...
       begin
           ...
           if (DataSource=nil) and (CurrentCustomEdit=nil) and (InplaceEditor<>nil) and ie.HavePictureMask then
           begin
...
//           if (DataSource=nil) and
           if CurrentCustomEdit.Focused then begin
              sysutils.abort; // 3/29/03 - Don't allow exit for unbound case when tied to a TwwDBEdit
              result:= false;
              exit;
           end
           else CurrentCustomEdit.visible:= False;
         end
         else begin
            result:=false;
            SkipErase:= False;
...
begin
  ...
  begin
    ...
    if (ssDouble in Shift) and
       (DataSource=nil) and (CurrentCustomEdit=nil) and (InplaceEditor<>nil) and ie.HavePictureMask then
    begin
       if (not ie.IsValidPictureValue(ie.Text)) and
          (not ie.Picture.AllowInvalidExit) then
       begin
...
//             Row:= DataRow(0); // 6/20/03 - Should have been removed as in IP 3000
      ...
      vk_tab:
       if not (ssCtrl in Shift) then begin // 11/23/02 - If control pressed then ignore tab
       ...
       prIncomplete: begin
	    ...
	    DisplayTextIsInvalid:= True;
            if ParentGrid.DataSource=nil then
               modified:= true; { 3/29/03 - Allow FlushContents to do modified test for picture masks}
	    end;
...
begin
  ...
  if (obj.Field<>nil) and
     (obj.datasource<>nil) and (obj.Datasource.state in [dsEdit, dsInsert]) then begin // 3/31/03 - fix problem where cancel from navigator after expand button opened with navigator
     { Handle 2 digit year logic }
     TextIsSame:= (obj.Field.Text = FEditText);

     if (obj.Field is TBlobField) and (obj.Field.DataType in [ftMemo, ftOraBlob, ftOraCLob
     {$ifdef wwDelphi2006Up}
     ,ftWideMemo
     {$endif}
     ]) then
...
begin
  ...
  if obj=nil then exit; // 9/16/03 - in case no items visible
      ...
      if obj<>nil then begin
         ...
         begin
            ...
            begin
                ...
                begin
                   if (not ie.IsValidPictureValue(ie.Text)) and  //  3/29/03 - Fix unbound case with picture mask
                      (not ie.Picture.AllowInvalidExit) then
                   begin
                     MessageBeep(0);
                     sysutils.Abort;
...
begin
  ...
  Result := FDatalink.Active and
    curField.IsValidChar(Key);
end;
