...
rtlconsts, variants,
{$endif}
 consts, wwdbdatetimepicker, wwcommon, Types;
...
begin
   ...
   // 8/30/07 - Change Date assignment to FDateTime assignment
...
//         if STA[1].wMonth > 0 then begin
           if Trunc(EncodeDate(STA[1].wYear,STA[1].wMonth,min(dy,DaysThisMonth(STA[1].wMonth,STA[1].wYear))))<MinDate then
           begin
              FDateTime:=Trunc(MinDate);
              MessageBeep(0); // 7/24/1998 - pw - Added Message Beep when setting to MinDate
           end
           else FDateTime:=EncodeDate(STA[1].wYear,STA[1].wMonth,
              min(dy,DaysThisMonth(STA[1].wMonth,STA[1].wYear)));
//         end
{         else begin
           if Trunc(EncodeDate(STA[1].wYear-1,12+STA[1].wMonth,
               min(dy,DaysThisMonth(12+STA[1].wMonth,STA[1].wYear-1))))<MinDate then
           begin
              FDateTime:=Trunc(MinDate);
              MessageBeep(0);  // 7/24/1998 - pw - Added Message Beep when setting to MinDate
           end
           else FDateTime:=EncodeDate(STA[1].wYear-1,12+STA[1].wMonth,
               min(dy,DaysThisMonth(12+STA[1].wMonth,STA[1].wYear)));
         end;
         }
         Change;
      end
      else begin
//         if STA[2].wMonth<= 12 then begin
           if (MaxDate<>0.0) and (Trunc(EncodeDate(STA[2].wYear,STA[2].wMonth,min(dy,DaysThisMonth(STA[2].wMonth,STA[2].wYear))))>Trunc(MaxDate)) then //9/14/07 - Remove (months) increment
           begin
              FDateTime:=Trunc(MaxDate);
              MessageBeep(0); // 7/24/1998 - pw - Added Message Beep when setting to MaxDate
           end
           else
              FDateTime:=EncodeDate(STA[2].wYear,STA[2].wMonth,
                  min(dy,DaysThisMonth(STA[2].wMonth+(Months),STA[2].wYear)));
//         end
  {       else begin
           if (MaxDate<>0.0) and (Trunc(EncodeDate(STA[2].wYear+1,STA[2].wMonth-12,min(dy,DaysThisMonth(STA[2].wMonth+(Months)-12,STA[2].wYear+1))))>Trunc(MaxDate)) then
           begin
              FDateTime:=Trunc(MaxDate);
              MessageBeep(0);  // 7/24/1998 - pw - Added Message Beep when setting to MaxDate
           end
              ...
              if (STA[2].wYear+1<10000) then
                FDateTime:=EncodeDate(STA[2].wYear+1,STA[2].wMonth-12,
    ...
    case code of
      ...
      MCN_SELCHANGE:
        begin
          ...
          if (MaxDate<>0) and (StartDate>MaxDate) then FStartDate:= MaxDate;
          ...
          else FEndDate:=StartDate; // 8/30/07 - Bypass exceptions
...
//  FPaintControl := TwwMonthPaintControl.Create(Self, MONTHCAL_CLASS);
//  FPaintControl := TwwMonthCalendar.Create(self);
...
//     DateTimeToSystemTime(Value, ST);
//     ST.wDayOfWeek := DayOfWeek(Value);
//     if MonthCal_SetCurSel(handle, ST) then
     SetMonthCalDateTime(FDataLink.Field.asDateTime);
     inherited;
     SetMonthCalDateTime(PrevDateTime);
...
//    FPaintControl := TPaintControl.Create(Self, MONTHCAL_CLASS);
    FPaintControl := TwwDBMonthCalendar.Create(self);
    FPaintControl.ParentWindow:= Application.Handle;
    FPaintControl.Options:= FPaintControl.Options - [mdoDayState];
    FPaintControl.Font.Assign(Font);
  end;
