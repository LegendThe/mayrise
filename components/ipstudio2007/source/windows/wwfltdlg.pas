...
uses
  ...
  { wsdlintf, }
  {$ifdef wwDelphi2006Up}  widestrings, {$endif}
  typinfo;
               ...
               SearchDelimiter := wwGetFilterOperator(FilterValue,FwwOperators,OrFlg,AndFlg);
                ...
                if (not FldInfo.CaseSensitive) and
                   not (dbInfo.FieldType in
                        [ftDate, ftDateTime,
                        {$ifdef wwDelphi6Up}
                        ftTimeStamp, ftFMTBcd, ftLargeInt,
                        {$endif}
                        ftTime, ftSmallInt, ftInteger, ftWord, ftFloat, ftCurrency,
                         ftAutoInc, ftBoolean, ftBCD]) then // 8/20/03 - Added ftbcd
                begin
                   if dbinfo.Tablename<>'' then
                ...
                begin
                   ...
                   repeat
                      ...
                      case dbInfo.FieldType of
                         ...
                         ftBCD, ftSmallInt, ftInteger, ftWord, ftFloat, ftCurrency:
                         ...
                         ftTime:
                           ...
                           if (MatchType=fdMatchExact) and
                              {$ifdef wwDelphi5Up}
                              not (dbInfo.FieldType in [ftMemo,ftBlob,ftOraBlob, ftOraClob
                             {$ifdef wwDelphi2006Up}
                              , ftWideMemo
                              {$endif}
                              ]) then begin
...
//                             TempMaxValue:= FldInfo.MaxValue;
                   ...
                   begin
                      // 2/12/03 - Support OnEncodeValue event
                      TempMinValue:=FldInfo.MinValue;
                      TempMaxValue:= FldInfo.MaxValue;
                      if Assigned(FOnEncodeValue) then
                      begin
         ...
         case MatchType of
           fdMatchStart, fdMatchAny, fdMatchExact:
             begin
                ...
                begin
                   ...
                   until CurPos=-1;
                   line := line + ')'; // 10/23/02 - Missing trailing parenthesis before
                   if (line<>'()') then
                      FilterExpr.add(line);
                end
             end;
           ...
           fdMatchRange:
             begin
                if FoundField then
                begin
                   if Assigned(FOnEncodeDateTime) then // 5/18/04 - dbinfo unintialized otherwise in OnEncodeDateTime
                      dbInfo:= GetDBInfoForField(FilterField.FieldName);
   ...
   try
    ...
    if ApplyFilter then begin
       ...
       if not ((datasource.dataset is TQuery) or (not ExecuteQuery) or
               (Typinfo.GetPropInfo(GetCommandTextDataSet.ClassInfo,GetSQLPropertyName)<>Nil)) then
       begin
          SetOnFilterEvent;
       end
       else begin
      ...
      if MinValueEditControl is TwwDBDateTimePicker then begin
         ...
         begin
            if (TwwDBDateTImePicker(MinValueEditControl).Time<>0) and (FieldsListBox.ItemIndex>=0) and
               {$ifdef wwDelphi6Up}
               (GetFieldType(FieldsListBox.items[LastItemIndex]) in [ftDateTime, ftTimeStamp]) then // 1/16/08 - Use LastItemIndex
               {$else}
               (GetFieldType(FieldsListBox.items[LastItemIndex]) in [ftDateTime]) then
               {$endif}
...
//         isValueField(DisplayLabel) then
      ...
      else if ((DlgDataSet is TQuery) or
               (Typinfo.GetPropInfo(TwwFilterDialog(DlgComponent).GetCommandTextDataSet(DlgDataset).ClassInfo,GetSQLPropertyName)<>Nil)) and
              ((DlgComponent as TwwFilterDialog).FilterMethod = fdByQueryModify) and
              {$ifdef wwDelphi5Up}
              (GetFieldType(DisplayLabel) in [ftmemo,ftBlob,ftOraBlob, ftOraClob
              {$ifdef wwDelphi2006Up}
              , ftWideMemo
              {$endif}
              ]) then { 5/20/00 - Check ftBlob,ftOraBlob too }
              {$else}
...
//     Panel1.visible:= false;
  ...
  if (fdTabsAtTop in Dlg.Options) then // 5/26/04 - Move code before Selectfocus
  begin
...
begin
   ...
   if curField=nil then // 3/8/03 - Fix problem where didn't check valid values for secondary tables
   begin
      ft:= GetFieldType(FieldsListBox.items[LastItemIndex]);
      if ft<>ftUnknown then
         result:= wwIsValidValue(ft, val)
      else
         result:= True
   end
   else
      result:= wwFieldIsValidValue(curField, val);
end;
...
begin
   ...
   else if datasource.dataset=nil then FilterMethod:= val
   else if (datasource.dataset is TQuery) or
           (Typinfo.GetPropInfo(datasource.dataset.ClassInfo,GetSQLPropertyName)<>Nil) then
   begin
      FFilterMethod:= val;
   end
   else if wwIsClass(datasource.dataset.classType, 'TSimpleDataSet') then
   begin
      FFilterMethod:= val; // Later check that CommandType is ctQuery
   end
   else begin
...
begin
   if FSQLPropertyName<>'' then result:= FSQLPropertyName
      ...
      (wwIsClass(DataSource.Dataset.classtype, 'TClientDataSet') or
       wwIsClass(DataSource.Dataset.classtype, 'TADODataSet') or
       wwIsClass(DataSource.DataSet.classtype, 'TSimpleDataSet')) then
   begin
      result:= 'CommandText';
   end
...
constructor TwwFilterDialogRounding.Create(Owner: TComponent);
begin
   FRoundingMethod := fdrmFixed;
   FEpsilon:= 0.0001;
end;
