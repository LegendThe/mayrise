...
// 11/11/16 - Added new enumerate for  TwwEditFocusStyle
//
}
unit wwframe;
{$i wwIfDef.pas}
...
type
  ...
  public
     ...
     class Function Get(Control: TComponent): TwwButtonEffects;
  ...
  TwwEditFocusStyle = (efsFrameBox, efsFrameSunken, efsFrameRaised, efsFrameEtched,
                       efsFrameBump, efsFrameSingle, efsFrameCxBlue); //Bryan 25/10/2016
...
//    procedure AdjustEditRect;
  public
    ...
    class Function Get(Control: TComponent): TwwEditFrame;
...
procedure wwDrawEdge(Control: TWinControl;
                     Frame: TwwEditFrame;
                     Canvas: TCanvas;
                     Focused: boolean);
var
  cr: TRect;
  fb:TwwEditFrameEnabledSet;
{$IFNDEF FLATNESS}
  Flags: integer;
...
{$ENDIF}
    ...
    begin
      fb:=Frame.NonFocusBorders;
{$IFNDEF FLATNESS}
      focusStyle:= Frame.NonFocusStyle;
{$ENDIF}
    end;

  if not (efRightBorder in Frame.FocusBorders) and frame.transparent then
    cr.right:= cr.right-2;

{$IFDEF FLATNESS}
  Canvas.Pen.Color:=$646464;
  Canvas.Pen.Width:=1;
  Canvas.Pen.Style:=psSolid;

  if efLeftBorder in fb then
    begin
      Canvas.MoveTo(cr.Left,cr.Top);
      Canvas.LineTo(cr.Left,Cr.Bottom+1);
    end;

  if efBottomBorder in fb then
    begin
      Canvas.MoveTo(cr.Left,cr.Bottom);
      Canvas.LineTo(cr.Right+1,Cr.Bottom);
    end;

  if efTopBorder in fb then
    begin
      Canvas.MoveTo(cr.Left,cr.Top);
      CanVas.LineTo(cr.Right+1,Cr.Top);
    end;

  if efRightBorder in fb then
    begin
      Canvas.MoveTo(cr.Right,cr.Top);
...
{$ELSE}
  ...
  case FocusStyle of
    efsFrameSingle:  DrawEdge(Canvas.Handle, cr, BDR_SUNKENOUTER, flags or bf_mono );
    efsFrameBox:     DrawEdge(Canvas.Handle, cr, EDGE_SUNKEN, flags or bf_mono);
    efsFrameSunken:  DrawEdge(Canvas.Handle, cr, EDGE_SUNKEN, flags);
    efsFrameRaised:  DrawEdge(Canvas.Handle, cr, EDGE_RAISED, flags);
    efsFrameEtched:  DrawEdge(Canvas.Handle, cr, EDGE_ETCHED, flags);
    efsFrameBump:    DrawEdge(Canvas.Handle, cr, EDGE_BUMP, flags);
    efsFrameCxBlue:
    with Canvas do
     begin
        Brush.Color := clBlack;
        if Control is TwwDBCustomCombo  then
          if Focused OR TwwDBCustomCombo(Control).MouseInControl then
            Brush.Color := $FF9933;
