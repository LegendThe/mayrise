...
begin
  ...
  if csPaintCopy in ControlState then
    AUpdateRect := RotateRect(Rect(0, 0, FullWidth, FullHeight))
  else
    AUpdateRect := RotateRect(Canvas.Canvas.ClipRect);

  if not IsRectEmpty(AUpdateRect) then
  begin
    FRoot.SetAnimXY(0, 0, AUpdateRect, AnimationStep = 0);
    ATempFont := TFont.Create;
    try
      ATempFont.Assign(Font);
      ATempFont.Height := DoZoom(Font.Height);
      Painter.DrawBackground(Canvas, ClientBounds);
      PaintNode(Canvas, ATempFont, FRoot, AUpdateRect);
    finally
      ATempFont.Free;
      FUpdated := False;
    end;
...
begin
  ...
  if R.Right > Owner.ClientBoundsWidth then
    LeftX := LeftX + Owner.ClientBoundsWidth - R.Right;
  if R.Bottom > Owner.ClientBoundsHeight then
    TopY := TopY + Owner.ClientBoundsHeight - R.Bottom;

  TopY := Max(TopY, 0);
  LeftX := Max(LeftX, 0);
  if (LeftX <> R.Left) or (TopY <> R.Top) then
...
function TdxOrgChartCustomPainter.GetLookAndFeelPainter: TcxCustomLookAndFeelPainterClass;
begin
