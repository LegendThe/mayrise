...
uses Menus, Clipbrd, Consts, Registry, IniFiles
  {$IFNDEF DELPHI3}, DBTables{$ENDIF}, dxGrExpt{filter}, dxGrFltr, dxGrFCmn{filter}, Types;
...
var
  FlagGrp: Boolean;
begin
  {$IFDEF READONLYFIELDS}
//  if de_NotifyLinks_List.indexOf(self)>-1 then  //fixed by DevEx do not need it anymore
//    exit;
  {$endif}
...
begin
  ...
  try
    ...
    if Assigned(FOnReloadGroupList) then FOnReloadGroupList(nil);  //nil is before
    LoadGroupList(nil);
    if Assigned(FOnReloadGroupList) then FOnReloadGroupList(Self); //self is after
  finally
    EndUpdate;
  end;
end;
