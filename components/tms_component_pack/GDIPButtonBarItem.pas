...
begin
  if (Key = VK_SPACE) and (FFocusedElement >= 0) and (FFocusedElement <= Elements.Count - 1) then
  begin
    prev := FDownElement;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Enabled and PtInGPRect(ir, Point(pX, pY)) then
      begin
        Result := Elements[i];
        break;
      end;
...
begin
  ...
  for I := Elements.Count - 1 to 0 do
  begin
    if Elements[i].Enabled then
    begin
      Result := I;
...
begin
  ...
  begin
    ...
    begin
      ...
      while not Elements[idx].Enabled do
      begin
        if Backwards then
        begin
          Dec(idx);
