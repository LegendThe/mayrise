...
uses
  Windows, Graphics, Classes, Forms, SysUtils, Math, Clipbrd, IniFiles, AdvGDIP, ImgList, GDIPPictureContainer
  {$IFDEF DIRECT2D}
...
type
  ...
  //--- CapacityBar
  TLegendPosition = (lpTop, lpBottom);
  ...
  protected
    ...
    property Left: Integer read FLeft write FLeft;       // Auto Calc
    property Width: Integer read FWidth write FWidth;    // Auto calc
  public
...
begin
  ...
  begin
    path := CreateRoundRectangle(MakeRect(hs + x,vs + y, Width - (hs * 2), Height - (vs * 2)), Rounding, RoundingType, false);
  ...
  begin
    ...
    case PictureAlignment of
      ...
      taCenter: l := (Width - pic.Width) div 2;
    end;
    ...
    begin
      pic.GDIPDraw(g, Bounds(l + x, t + y, pic.Width, pic.Height));
    end;
...
begin
  ...
  g.FillRectangle(LGB, MakeRect(RF.X, (RF.Y + h3 - 1), RF.Width, h3 + 1));
  LGB.Free;
...
procedure TGDIPCapacityBar.DrawLegend(g: TGPGraphics; R: TGPRectF;
  ...
  procedure DrawMark(pt: TPoint; Clr, ClrTo: TColor);
  var
    LGB: TGPLinearGradientBrush;
    Pen: TGPPen;
    bmp: TBitmap;
  ...
  begin
    ...
    try
      ...
      try
        ...
        if Reflection then
        begin
          g.DrawImageRect(GetReflection(bmp, ReflectionOpacityStart, ReflectionOpacityEnd, FMarkSize - 1, ReflectionAxis), pt.X, pt.Y + ReflectionAxis + bmp.Height, bmp.Width, bmp.Height);
        end;
      finally
        gr.Free;
      end;
...
var
  I, w, MGap, LGap, x, y, IC, l: Integer;
  R2: TRect;
  RF: TGPRectF;
  d: Double;
  fds: string;
begin
  ...
  begin
    y := R.Bottom - Round(FLegendRect.Height);
  end;
  ...
  begin
    ...
    //if (I < Items.Count - 1) then
      x := x + FMarkSize + Round(Max(Items[I].FDesRect.Width, Items[I].FFormatRect.Width)) + LGap;
    ...
    if Items[I].Value > 0 then
      Items[I].Width := Round(Items[I].Value / d)
    else
      Items[I].Width := 0;
    l := l + Items[I].Width;
  end;
  ...
  if (Items.Count > 0) then
    ...
    FFreeLeft := Round(FBarRect.x);
  FFreeWidth := Round(FBarRect.x + FBarRect.Width) - FFreeLeft;
