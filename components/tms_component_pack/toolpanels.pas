...
type
  ...
  TToolPanelStyle = (esOffice2003Blue, esOffice2003Silver, esOffice2003Olive, esOffice2003Classic, esOffice2007Luna, esOffice2007Obsidian, esWindowsXP, esWhidbey, esCustom, esOffice2007Silver, esWindowsVista, esWindows7, esTerminal, esOffice2010Blue, esOffice2010Silver, esOffice2010Black);
...
//          FPanelForm.Repaint;
    ...
    ppBottom:
      begin
        ...
        begin
          while FPanelForm.Height < APanel.OpenWidth do
          begin
            t := GetTickCount;
...
begin
  ...
  if ((Msg.Msg = WM_MOVING) or (Msg.Msg = WM_PAINT)) and (Position = ppRight) and (FRollOutPanel <> -1) then
  begin
    pt := Point(Left, Top);
    pt := Parent.ClientToScreen(pt);
...
begin
  ...
  begin
    FInternalCall := true;
    if (Height - FSectionItemHeight) >= (CAPTION_HEIGHT + 2) then
      Height := Height - FSectionItemHeight;
    FInternalCall := false;
  end;
end;
...
{$ENDIF}
        ...
        FInternalCall := true;
        Height := FSectionItemHeight * SectionItems.Count + TAdvToolPanelSections(Collection).FOwner.SectionHeight+6 {CAPTION_HEIGHT }+ a;
        FInternalCall := false;
      end;
...
begin
  if ShowCaption then
  begin
    if (Rect.Top < CAPTION_HEIGHT) and not Floating then
...
begin
  ...
  begin
    if Locked and Tab.AutoDock and AllowDocking and (Y < CAPTION_HEIGHT) and not floating and (x > 0) and ((not FDockDots and (x < Width - 40)) or (FDockDots and (x < DOCKDOTS_WIDTH))) then
    begin
  ...
  begin
    if Locked and Tab.AutoDock and AllowDocking and (Y < CAPTION_HEIGHT) and not floating and ((not FDockDots and (x < Width - 40)) or (FDockDots and (x < DOCKDOTS_WIDTH))) then
    begin
      DragKind := dkDock;
      DragMode := dmAutomatic;
      BeginDrag(true);
...
//*******
  ...
  begin
    ...
    begin
      ...
      r2.Bottom := r2.Top + CAPTION_HEIGHT;
 ...
 { if Locked and not FLockedGlyph.Empty then
  begin
    ...
    begin
      ...
      r2 := Rect(r.Left, r.Top, r.Left + DOCKDOTS_WIDTH, r.Top + CAPTION_HEIGHT);
