...
uses
  ...
  PictureContainer,  AdvGDIP, GDIPicture, AdvHintInfo, AdvStyleIF
{$IFDEF TMSTOOLBAR}
  , AdvToolBar
{$ENDIF}
  ;
...
begin
  ...
  begin
    ...
    HTMLDrawEx(Canvas, SimpleText, dr, fImages, 0, 0, -1, -1, 1, true, false, false, false, (FTimerCount > 5), false,
...
function TAdvOfficeStatusBar.IsAnchor(x, y: integer): string;
var
  r: trect;
  xsize, ysize: integer;
  anchor, stripped: string;
...
begin
  ...
  //  if (frm.BorderStyle = bsSizeAble) or FMetro then
      Exit;
...
begin
 // optimized repaint
  //SendMessage(TAdvOfficeStatusPanels(fOwner.Collection).fStatusbar.handle, SB_GETRECT, fOwner.index, longint(@r));
