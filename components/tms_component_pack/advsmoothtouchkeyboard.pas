...
{TAdvSmoothTouchKeyCollection}
  ...
  private
    ...
    procedure AddKey(Caption, ShiftCaption, AltGrCaption : ShortString;
        ...
        SpecialKey : TAdvSmoothTouchSpecialKey; Color: TColor = clSilver);
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TAdvSmoothPopupTouchKeyBoard = class(TComponent)
  private
    FTimer: TTimer;
  ...
  published
    ...
    property HighlightAltGr: TColor read FHighlightAltGr write SetHighlightAltGr default clNone;
    property KeyboardType : TAdvSmoothTouchKeyBoardType read FKeyboardType write SeTAdvSmoothTouchKeyBoardType;
    property KeyboardWidth: integer read FKeyboardWidth write FKeyboardWidth default 602;
    property KeyboardHeight: integer read FKeyboardHeight write FKeyboardHeight default 202;
...
begin
  if FCapsDown then
    PosTAdvSmoothTouchSpecialKeys(VK_CAPITAL, [], True);
  FEditControl.Free;
  FFill.Free;
  FKeys.Free;
...
begin
  ...
  begin
    ...
    img.GDIPDraw(g, MakeRect(rimg.X + (rimg.Width - imgw) / 2, rimg.X + (rimg.Height - imgh) / 2,
      imgw, imgh));
  end;
...
begin
  ...
  begin
    b := TGPLinearGradientBrush.Create(MakeRect(r.X - 1, r.Y - 1, r.Width + 1, r.Height + 1), MakeColor(10, clWhite),
    MakeColor(150, clWhite), LinearGradientModeBackwardDiagonal);
  ...
  begin
    ...
    b := TGPLinearGradientBrush.Create(MakeRect(rgl.X - 1, rgl.Y - 1, rgl.Width + 1, rgl.Height + 1), MakeColor(150, clWhite),
    MakeColor(10, clWhite), LinearGradientModeBackwardDiagonal);
    g.FillPath(b, path);
    path.Free;
    b.Free;
  end;
...
begin
  ...
  begin
    ...
    FAutoPostKey := (Source as TAdvSmoothPopupTouchKeyBoard).AutoPostKey;
    FAutoCapsDisplay := (Source as TAdvSmoothPopupTouchKeyBoard).AutoCapsDisplay;
    FHighlightCaps := (Source as TAdvSmoothPopupTouchKeyBoard).HighlightCaps;
    FHighlightAltGr := (Source as TAdvSmoothPopupTouchKeyBoard).HighlightAltGr;
    FKeyboardType := (Source as TAdvSmoothPopupTouchKeyBoard).KeyboardType;
...
begin
  ...
  begin
    ...
    Selection.EndPos := -1;
    SendMessage(wnd, EM_GETSEL, Longint(@Selection.StartPos), Longint(@Selection.EndPos));
...
begin
  ...
  if FKeyBoard.Owner is TForm then
    SetWindowLong((FKeyBoard.Owner as TForm).Handle, GWL_WNDPROC, LongInt(NewWndProc))
  else if FKeyBoard.Owner is TAdvSmoothTouchKeyBoardToolForm then
    SetWindowLong((FKeyBoard.Owner as TAdvSmoothTouchKeyBoardToolForm).Handle, GWL_WNDPROC, LongInt(NewWndProc));
    ...
    begin
      ...
      begin
        if KeyBoard.Owner is TForm then
          SetWindowLong((KeyBoard.Owner as TForm).Handle, GWL_WNDPROC, LongInt(OldWndProc))
        else if KeyBoard.Owner is TAdvSmoothTouchKeyBoardToolForm then
          SetWindowLong((KeyBoard.Owner as TAdvSmoothTouchKeyBoardToolForm).Handle, GWL_WNDPROC, LongInt(OldWndProc))
      end;
    end
