...
private:
	...
	void __fastcall SetClosedListButtonHint(const AnsiString Value);
	TAdvTabSheet* __fastcall GetClosedTab(int Index);
	int __fastcall GetClosedTabCount(void);
	TAdvTabSheet* __fastcall GetPageFromName(AnsiString TabName);
	TAdvTabSheet* __fastcall GetPageFromCaption(AnsiString TabCaption);
...
public:
	...
	bool __fastcall CloseOpenedTab(AnsiString TabName)/* overload */;
...
__published:
	...
	__property AnsiString ClosedListButtonHint = {read=FClosedListButtonHint, write=SetClosedListButtonHint};
