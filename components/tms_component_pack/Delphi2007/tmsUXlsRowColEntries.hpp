...
// (DO NOT EDIT: machine generated header) 'Tmsuxlsrowcolentries.pas' rev: 11.00
...
private:
	DynamicArray<Tmsuflxformats::TFlxFont >  XFFonts;
...
public:
	...
	int __fastcall CalcCellWidth(const int Row, const int Col, const Tmsuflxmessages::TRichString &val, const int XF, const void * Workbook, const Extended RowMultDisplay, const Extended ColMultDisplay);
...
public:
	...
	void __fastcall DeleteRows(const Word aRow, const Word aCount, const Tmsxlsmessages::TSheetInfo &SheetInfo);
	void __fastcall DeleteCols(const Word aCol, const Word aCount, const Tmsxlsmessages::TSheetInfo &SheetInfo);
	...
	__fastcall TBaseRowColList(const TMetaClass* aListClass);
public:
...
private:
	...
	WideString __fastcall GetFormula(int Row, int Col);
	void __fastcall SetFormula(int Row, int Col, const WideString Value);
	void __fastcall AutofitColumn(const void * Workbook, const int Column, const TColWidthCalc* ColCalc, const Extended RowMultDisplay, const Extended ColMultDisplay, const bool IgnoreStrings, const Extended Adjustment);
...
public:
	...
	__property WideString Formula[int Row][int Col] = {read=GetFormula, write=SetFormula};
	void __fastcall AssignFormulaX(const int Row, const int Col, const WideString Formula, const Variant &Value, const bool Options1904);
	...
	WideString __fastcall GetSheetName(const int SheetNumber);
	int __fastcall AddExternSheet(const int FirstSheet, const int LastSheet);
	bool __fastcall FindSheet(WideString SheetName, /* out */ int &SheetIndex);
	virtual void __fastcall InsertAndCopyCols(const int FirstCol, const int LastCol, const int DestCol, const int aCount, const Tmsxlsmessages::TSheetInfo &SheetInfo, const bool OnlyFormulas);
	virtual void __fastcall ArrangeInsertRowsAndCols(const int InsRowPos, const int InsRowCount, const int InsColPos, const int InsColCount, const Tmsxlsmessages::TSheetInfo &SheetInfo);
	void __fastcall ArrangeInsertSheet(const Tmsxlsmessages::TSheetInfo &SheetInfo);
	WideString __fastcall GetName(const int ExternSheet, const int NameId);
...
public:
	...
	void __fastcall DeleteRowsOrCols(const Word aRow, const Word aCount, const Tmsxlsmessages::TSheetInfo &SheetInfo, const bool UseCols);
public:
	#pragma option push -w-inl
