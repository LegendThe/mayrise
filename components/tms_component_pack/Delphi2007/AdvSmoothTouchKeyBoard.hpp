...
// (DO NOT EDIT: machine generated header) 'Advsmoothtouchkeyboard.pas' rev: 11.00
...
typedef void __fastcall (__closure *TAdvSmoothCompletionColumnHintEvent)(System::TObject* Sender, int Column, AnsiString Hint);
...
protected:
	...
	Types::TRect __fastcall GetSelectedRect();
...
private:
	...
	AnsiString FDateTimeValueFormat;
	...
	void __fastcall SetDateTimeValueFormat(const AnsiString Value);
...
protected:
	...
	Advgdip::TGPRectF __fastcall GetColumnRect();
...
__published:
	...
	__property AnsiString DateTimeValueFormat = {read=FDateTimeValueFormat, write=SetDateTimeValueFormat};
...
private:
	...
	int FImageIndexDown;
	Advgdip::TAdvGDIPPicture* FImageDown;
	AnsiString FImageNameDown;
	int FImageHeight;
	int FImageWidth;
	...
	void __fastcall SetImageDown(const Advgdip::TAdvGDIPPicture* Value);
	void __fastcall SetImageIndexDown(const int Value);
	void __fastcall SetImageNameDown(const AnsiString Value);
	void __fastcall SetImageHeight(const int Value);
	void __fastcall SetImageWidth(const int Value);
...
__published:
	...
	__property AnsiString ShiftCaption = {read=FShiftCaption, write=SetShiftCaption};
	...
	__property AnsiString ShortCut = {read=FShortCut, write=SetShortCut};
	...
	__property int ImageIndexDown = {read=FImageIndexDown, write=SetImageIndexDown, default=-1};
	__property AnsiString ImageNameDown = {read=FImageNameDown, write=SetImageNameDown};
	__property Advgdip::TAdvGDIPPicture* ImageDown = {read=FImageDown, write=SetImageDown};
	__property int ImageWidth = {read=FImageWidth, write=SetImageWidth, default=-1};
	__property int ImageHeight = {read=FImageHeight, write=SetImageHeight, default=-1};
...
protected:
	...
	void __fastcall UpdateMainWindow(Byte Alpha);
...
private:
	...
	void __fastcall SetColor(const Graphics::TColor Value);
	void __fastcall SetFont(const Graphics::TFont* Value);
	void __fastcall SetLookupSpacing(const int Value);
...
protected:
	...
	void __fastcall UpdateCompletion(AnsiString Filter = "");
...
__published:
	...
	__property int LookupSpacing = {read=FLookupSpacing, write=SetLookupSpacing, default=30};
};


typedef void __fastcall (__closure *TAdvSmoothTouchKeyboardTextChangedEvent)(System::TObject* Sender, AnsiString &Text);
...
private:
	...
	DynamicArray<TKeyCachedBitmap >  FCachedBitmaps;
	bool FMouseUp;
	bool FMouseDown;
	...
	bool FAllowAutoZoom;
	bool FUseFocusedControl;
	TAdvSmoothTouchKeyboardTextChangedEvent FOnTextChanged;
	AnsiString FText;
	int FKeyDownOffset;
	...
	void __fastcall AddSubKey(AnsiString Caption);
	...
	void __fastcall PostAdvSmoothTouchSpecialKeys(Word Key, const Classes::TShiftState pShift, bool SpecialKey);
	...
	void __fastcall SetAllowAutoZoom(const bool Value);
	void __fastcall SetShift(const Classes::TShiftState Value);
	void __fastcall SetUseFocusedControl(const bool Value);
	HIDESBASE void __fastcall SetText(const AnsiString Value);
	
protected:
	void __fastcall TextChanged(System::TObject* Sender);
	int __fastcall GetItemOnShift(void);
	int __fastcall GetItemOnKey(Word AKey);
	int __fastcall GetItemOnKeyCaption(AnsiString AKeyCaption);
	HIDESBASE void __fastcall Changed(void);
...
public:
	...
	void __fastcall LoadKeybdLayout(AnsiString FileName, TAutoSizeLayout AutoSize = (TAutoSizeLayout)(0x1));
	...
	void __fastcall SetKey(Word AKey, AnsiString AKeyCaption = "", Classes::TShiftState pShift = Classes::TShiftState() );
	
__published:
	__property int KeyDownOffset = {read=FKeyDownOffset, write=FKeyDownOffset, default=1};
	__property TAdvSmoothTouchKeyboardTextChangedEvent OnTextChanged = {read=FOnTextChanged, write=FOnTextChanged};
	__property bool UseFocusedControl = {read=FUseFocusedControl, write=SetUseFocusedControl, default=1};
	__property AnsiString Text = {read=FText, write=SetText};
	__property bool AllowAutoZoom = {read=FAllowAutoZoom, write=SetAllowAutoZoom, default=1};
...
typedef DynamicArray<TKeyCachedBitmap >  AdvSmoothTouchKeyBoard__61;
...
protected:
	...
	void __fastcall TextChanged(System::TObject* Sender, AnsiString &Text);
...
__published:
	...
	__property Graphics::TColor HighlightAltGr = {read=FHighlightAltGr, write=SetHighlightAltGr, default=536870911};
	__property TAdvSmoothTouchKeyBoardType KeyboardType = {read=FKeyboardType, write=SeTAdvSmoothTouchKeyBoardType, nodefault};
	__property int KeyboardWidth = {read=FKeyboardWidth, write=FKeyboardWidth, default=602};
	__property int KeyboardHeight = {read=FKeyboardHeight, write=FKeyboardHeight, default=202};
