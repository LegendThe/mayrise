...
type
  ...
  TCustomAdvPanel = class(TCustomPanel, ITMSTones)
  private
    FText: string;
    FImage:TAdvImage;
...
//    property OnKeyUp;
  ...
  TAdvPanelStyle = (psXP, psFlat, psTMS, psClassic, psOffice2003Blue, psOffice2003Olive, psOffice2003Silver, psOffice2003Classic, psOffice2007Luna, psOffice2007Obsidian, psWhidbey, psWindowsXP, psCustom, psOffice2007Silver, psWindowsVista, psWindows7, psTerminal, psOffice2010Blue, psOffice2010Silver, psOffice2010Black);
  ...
  [ComponentPlatformsAttribute(pidWin32 or pidWin64)]
  {$ENDIF}
  TAdvPanelStyler = class(TComponent, ITMSStyle, ITMSTones)
  private
...
begin
  ...
  begin
    ...
    begin
      ...
      TCustomAdvPanel(PL.Items[i]).Width := (Width - 2* HorzPadding - FScrollBar.Width) div Columns
    end
    else
    begin
      TCustomAdvPanel(PL.Items[i]).Left := HorzPadding + ((Width - HorzPadding) div Columns) * (i mod Columns);
      TCustomAdvPanel(PL.Items[i]).Width := (Width - 2* HorzPadding) div Columns;
    end;
...
begin
  {
  outputdebugstring('before move');
  for i := 0 to FPanels.Count - 1 do
  begin
    outputdebugstring(pchar(TCustomAdvPanel(fpanels[i]).Text+':'+inttostr(TCustomAdvPanel(fpanels[i]).Index)+':'+inttostr(TCustomAdvPanel(fpanels[i]).Top)));
  end;
  }
  ...
  {
  outputdebugstring('after list move');
  for i := 0 to FPanels.Count - 1 do
  begin
    outputdebugstring(pchar(TCustomAdvPanel(fpanels[i]).Text+':'+inttostr(TCustomAdvPanel(fpanels[i]).Index)+':'+inttostr(TCustomAdvPanel(fpanels[i]).Top)));
  end;
  ...
  {
  outputdebugstring('after panel move');
  for i := 0 to FPanels.Count - 1 do
  begin
    outputdebugstring(pchar(TCustomAdvPanel(fpanels[i]).Text+':'+inttostr(TCustomAdvPanel(fpanels[i]).Index)+':'+inttostr(TCustomAdvPanel(fpanels[i]).Top)));
  end;
