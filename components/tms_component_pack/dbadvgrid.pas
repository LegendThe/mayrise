...
uses
  ...
  Dialogs, AdvGrid, Menus, Grids, AdvUtil, INIFiles, AdvObj, ImgList, Forms
  {$IFNDEF TMSNOADO}
  , ADODB
  {$ENDIF}
  , JPeg
...
const
  ...
  // 2.1.9.3 : Fixed : issue with search footer button painting
  // 2.1.9.4 : Fixed : issue with RemoveAllColumns when grid.FixedColAlways = true
  //         : Fixed : issue with aborting post from BeforePost
  // 2.1.9.5 : Fixed : issue with scrolling 1 line at a time with mouse wheel
  // 2.1.9.6 : Fixed : issue with grid reacting to dataset changes when DB controls are disabled
  // 2.1.9.7 : Fixed : issue with insert row & default field values
  // 2.1.10.0: New : property GridColumnItem.UseLookupEditor added to selectively control the use of a lookup editor
  // 2.1.10.1: Fixed : issue with presetting column filter
  // 2.1.10.2: Fixed : issue with calling event OnAutoAddRow when Navigation.AppendOnArrowDown = true
  // 2.1.10.3: Fixed : issue with adding non DB columns
  // 2.1.10.4: Fixed : issue with design-time scroll handling
  // 2.1.10.5: Fixed : issue with OnGetColumnFilter event
  // 2.1.10.6: Fixed : issue with using RowIndicator and ShowDBIndicator simultanously
  // 2.1.10.7: Fixed : display issue when scrolling with keyboard
  // 2.1.11.0: New : ShowBlankRow property added to show empty (insert) row or not
  //         : Fixed : issue when using PageMode = false with FixedRows = 0
  // 2.1.12.0: Improved : Painting performance
  // 2.1.12.1: Improved : For blob memo fields dataset.OnGetText is triggered
  // 2.1.12.2: Fixed : Issue with setting FixedCols > 1
  // 2.1.12.3: Fixed : Issue with ShowMemoFields = true
  // 2.1.12.4: Improved : Behaviour to put dataset out of insert mode
  //         : Improved : Triggering of OnKeyDown event
  // 2.1.12.5: Fixed : Display issue when scrolling in grid
  // 2.1.12.6: Fixed : Issue for click on partial displayed row
  // 2.1.12.7: Fixed : Issue with Columns[x].PictureStretch for PageMode = false
  // 2.1.12.8: Fixed : Issue with click on last row when searchfooter is visible
  // 2.1.13.0: Improved : initialization of column width
  // 2.1.13.1: Fixed : Issue with LoadColPositions / SaveColPositions
  // 2.1.14.0: New : property UseDBFieldWidths added
  //         : Fixed : Issue with DirectEdit = true and DB lookup fields
  // 2.1.14.1: Fixed : Issue with FilterDropDownAuto
  // 2.1.14.2: Fixed : Issue with epRow edit mode and spin edit inplace editor
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (ComboBox.Items.Objects[idx] <> nil) then
        begin
          v := integer(ComboBox.Items.Objects[idx]);
          Field.AsInteger := v;
        end
        ...
        begin
          Field.AsString := ComboBox.Items[idx];
        end;
      end
      else
      begin
          ...
          //OutputDebugString(Pchar('TopRow='+inttostr(TopRow)+' Row='+inttostr(Row)));
            ...
            //FDoNotUpdateMe:= true;
                ...
                begin
                  ...
                  else if SI.nTrackPos >= RecordCount then FDataLink.DataSet.MoveBy(RecordCount)
                  else Row := SI.nTrackPos;
...
// CheckBox for String Fields
  if (Result = nil) and (FColumnCollection.Items[ACol].CheckBoxField) and (ARow >= FixedRows) and (ACol >= IndCol) and
     not ((ARow = RowCount - 1) and (FloatingFooter.Visible and (FloatingFooter.FooterStyle <> fsColumnPreview))) then
  begin
    Fld := GetDBFieldAtColumn(ACol);
...
begin
  ...
  //if SearchFooter.Visible and (RowCount > VisibleRowCount + FixedRows) then
  // Inc(i);
...
begin
  ...
  if (FEditPostMode = epCell) and PageMode then
  begin
    Result := inherited CanEditModify;
