...
{$ENDIF}
  ...
  // 2.5.13.0 :New : PrintOptions.TopMargin property added
  // 2.5.13.1 :Fixed : Issue in recurrency handler with BYMONTH instruction
...
const
  ...
  CORNER_EFFECT = 10;
{$IFDEF FREEWARE}
  COPYRIGHT = 'Printed by %s © 1998 - 2012 by TMS software';
{$ENDIF}
...
{$ENDIF}
  ...
  TTimeIndicatorType = (tiLine, tiGlyph, tiLineGlyph);
  ...
  for RowIndex := FromRow to ToRow do
    ...
    begin
      ...
      begin
        ...
        FGrid.PaintSideCol(ACanvas, SRect, RowIndex, PositionIndex, 0, False,
          True);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            APlannerItem := Items.FindItemPosIdx(RowIndex, ColIndex - 1,
              PosIndex);
    ...
    plDay:
      begin
        ...
        if (res div 60 < 24) then
          dtStart := PosToDay(SelPosition) + EncodeTime(res div 60, res mod 60,
            0, 0);
...
begin
  ...
  if Mode.PlannerType = plDay then
  begin
    Result := EncodeDate(Mode.Year, Mode.Month, Mode.Day) + Pos;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Bitmap := TBitmap.Create;
          APlannerItem.FPlanner.PlannerImages.GetBitmap(APlannerItem.ImageID,
            Bitmap);
    ...
    begin
      ...
      begin
        if ARect.Top + ih + APlannerItem.FPlanner.PlannerImages.Height <
          ARect.Bottom + 4 then
        begin
          if Print then
          begin
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        if ARect.Top + iw + APlannerItem.FPlanner.PlannerImages.Height <
          ARect.Bottom then
        begin
          if Print then
          begin
    ...
    begin
      ...
      if Print then
        ...
        DrawText(Canvas.Handle, PChar(DT), Length(DT), DRect,
          DT_NOPREFIX or DT_WORDBREAK or DT_VCENTER or DT_LEFT or
            DT_SINGLELINE);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        tzd :=
          FPlanner.Sidebar.TimeZoneMinDelta div FPlanner.Display.DisplayUnit;
  ...
  begin
    ...
    {and FPlanner.ShowSelection} and (ARow < NumColors) then
    begin
...
begin
  ...
  begin
    ...
    if not FPlanner.Sidebar.RotateOnTop then
...
{$ENDIF}
              ...
              (APlannerItem.ItemEnd = AColumn + 1))) or
...
begin
  ...
  if FPlanner.Positions = 0 then
    Exit;
{$IFDEF TMSCODESITE}
  CodeSite.SendMsg(Format('Updatepositions : %d * %d', [FPlanner.Positions,
      ColCount]));
{$ENDIF}
  ColOffset := FPlanner.FSidebar.FColOffset;
  RowOffset := FPlanner.FSidebar.FRowOffset;
...
begin
  ...
  begin
    if ((FPlanner.Items.Items[ItemIndex] as TPlannerItem)
        .ItemBegin >= TopRow - 1) and
      ((FPlanner.Items.Items[ItemIndex] as TPlannerItem)
        .ItemEnd <= TopRow + VisibleRowCount + 1) then
  (FPlanner.Items.Items[ItemIndex] as TPlannerItem)
      .UpdateWnd;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Assigned(nplIt)) and (APlannerItem <> nplIt)
      {and Assigned(APlannerItem)} then
      begin
        FPlanner.ItemSelected(nplIt);
        if Self.CanFocus then
          Self.SetFocus;
    ...
    begin
      ...
      if (Assigned(nplIt)) and (APlannerItem <> nplIt) and not Assigned
        (APlannerItem) then
      begin
        FPlanner.ItemSelected(nplIt);
        if Self.CanFocus then
      ...
      begin
        ...
        if Assigned(FPlanner.PlannerImages) and Assigned
          (FPlanner.FOnItemImageClick) then
        begin
    ...
    begin
      ...
      begin
        ...
        if ((ItemBegin <> FItemXY.Y + dy) or (ItemPos <> FItemXY.X + dx)) and
          (FItemXY.X + dx >= 0) and not FPlanner.Items.HasItem(FItemXY.Y + dy, FItemXY.Y + dy + h, FItemXY.X + dx) and
        ...
        begin
          ...
          begin
            FPlanner.FGrid.RepaintRect(Rect(FPlanner.FGrid.FixedCols, 1,
                                       FPlanner.FGrid.ColCount - 1, FPlanner.FGrid.RowCount - 1))
          end
          else
          begin
    ...
    begin
      ...
      begin
        if (ItemBegin <> FItemXY.Y + dy) and (FItemXY.Y + dy >= 0) and
          ...
          (FItemXY.Y + dy < ItemEnd) and not FPlanner.Items.HasItem(FItemXY.Y + dy, ItemEnd, ItemPos) then
        begin
          Allow := True;
    ...
    begin
      ...
      begin
        ...
        if ((ItemEnd <> FItemXY.Y + dy)) and (FItemXY.Y + dy >= 0) and
          ...
          and not FPlanner.Items.HasItem(ItemBegin, FItemXY.Y + dy, ItemPos) then
        begin
          Allow := True;
...
{$ENDIF}
      ...
      begin
        ...
        if ((ItemBegin <> FItemXY.X + dx) or (ItemPos <> FItemXY.Y + dy)) and
          (FItemXY.Y + dy >= 0) and not FPlanner.Items.HasItem(FItemXY.X + dx, FItemXY.X + dx + h, FItemXY.Y + dy) and
    ...
    begin
      ...
      begin
        if (ItemBegin <> FItemXY.X + dx) and (FItemXY.X + dx >= 0) and
          ...
          (FItemXY.X + dx < ItemEnd) and not FPlanner.Items.HasItem(FItemXY.X + dx, ItemEnd, ItemPos) then
        begin
          Allow := True;
    ...
    begin
      ...
      begin
        if ((ItemEnd <> FItemXY.X + dx)) and (FItemXY.X + dx >= 0) and
          ...
          and not FPlanner.Items.HasItem(ItemBegin, FItemXY.X + dx, ItemPos) then
        begin
          Allow := True;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if FPlanner.InsertAlways or not FPlanner.Items.HasItem(FromSel,
              ToSel, Row - 1) then
          ...
          begin
            ...
            begin
              if (ItemPos < FPlanner.Positions - 1)
                and not FPlanner.Items.HasItem(ItemBegin, ItemEnd, ItemPos + 1)
      ...
      VK_LEFT:
        begin
          ...
          begin
            with FPlanner.Items.Selected do
            begin
              if (ItemPos > 0) and not FPlanner.Items.HasItem(ItemBegin, ItemEnd, ItemPos - 1)
                and not FixedPos and not FixedPosition and
                (FPlanner.UserMode in [puNormal, puReadOnly]) then
              begin
                Allow := True;
          ...
          if Assigned(FPlanner.Items.Selected) then
            ...
            begin
              ...
              begin
                ...
                begin
                  if not FPlanner.Items.HasItem(ItemBegin, ItemEnd + 1,
...
{$IFDEF FREEWARE}
  ...
  begin
    ...
    Canvas.TextOut(R.Left + 4, R.Top + 4, FPlanner.ClassName +
        ' : Copyright © 1999 - 2012 by TMS software');
  end;
{$ENDIF}
  if (csDesigning in ComponentState) and FPlanner.ShowDesignHelper then
  begin
  ...
  if Assigned(FPlanner.OnAfterPaint) then
    FPlanner.OnAfterPaint(Self);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Canvas.MoveTo(m,LR1F.Top + (LR1F.Bottom -  LR1F.Top) div 2);
        Canvas.LineTo(m,LR1T.Top + (LR1T.Bottom -  LR1T.Top) div 2);
...
begin
  FDisplayStart := value;
  FDisplayStartPrecis := (FDisplayStart)
    * FDisplayUnit + FDisplayOffset;
  FUpdateUnit := True;
  UpdatePlanner;
  FUpdateUnit := False;
...
begin
  ...
  begin (FOwner.Items.Items[ItemIndex] as TPlannerItem)
    .UpdateWnd;
  end;
...
begin
  ...
  begin
    ...
    TopRowPrecis := FDisplayUnit * FOwner.FGrid.LeftCol +
      FDisplayOffset;
    ...
    FDisplayEnd := ((FDisplayEndPrecis - FDisplayOffset)
        div FDisplayUnit) - 1;
...
begin
  ...
  if Frac((FItemEndPrecis - FPlanner.Display.DisplayOffset) / FPlanner.Display.DisplayUnit) > 0 then
    FItemEnd := 1 + ((FItemEndPrecis - FPlanner.Display.DisplayOffset) div FPlanner.Display.DisplayUnit)  - FPlanner.Display.DisplayStart
  else
    FItemEnd := ((FItemEndPrecis - FPlanner.Display.DisplayOffset) div FPlanner.Display.DisplayUnit) - FPlanner.Display.DisplayStart;
...
begin
  ...
  begin
    ...
    plDay:
      begin
        delta := 0; BeginOffset := 0; EndOffset := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ItemSubPos > APlannerItem.FConflictPos * w) and (ItemSubPos < (APlannerItem.FConflictPos + 1) * w - FOwner.FItemGap) then
      begin
...
procedure TPlannerMemo.DoEnter; begin inherited; end;
...
begin
  case PlannerType of
  plDay, plHalfDayPeriod, plActiveDayPeriod: Result := EncodePlannerDate(PeriodEndYear, PeriodEndMonth, PeriodEndDay);
...
begin
  case PlannerType of
  plDay, plHalfDayPeriod, plActiveDayPeriod: Result := EncodePlannerDate(PeriodStartYear, PeriodStartMonth, PeriodStartDay);
...
begin
  ...
  begin
    ...
    AllPainted := False; repeat MergeNum := 1;
    ...
    begin
      ...
      begin
        if (SectionIndex < Sections.Count - FLeftPos) then
          AIdx := SectionIndex + FLeftPos else AIdx := 0;
...
//          if ((SectionIndex - 1 + FLeftPos) mod Planner.PositionGroup = 0) or AllPainted then
        ...
        Font := Self.Font; Brush.Color := FColor;
        Pen.Color := FLineColor; Pen.Width := 1;
      end;
    until AllPainted;
  end;
end;
...
begin
  ...
  if (FLastHintPos.X <> -1) then
    ...
    begin
      ...
      if (Anchor <> '') then
      begin
        outputdebugstring('anchor found');
        Cursor := crHandPoint;
...
begin inherited Create; FTextColor := clNone;
FBackgroundColor := clNone; FInitialDelay := -1;
FAutoHideDelay := -1; FReshowDelay := -1; FEnable := True;
end;
