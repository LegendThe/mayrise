...
type
  ...
  public
    PrevContentTile, NextContentTile, SelectedTile, HoverTile: TAdvSmoothTile;
...
begin
  ...
  begin
    for I := SelectedTile.Index + ACount to FWorkTiles.Count - 1 do
    begin
      if not FWorkTiles[I].Empty and FWorkTiles[i].Enabled then
      begin
        Result := I;
...
begin
  ...
  if Assigned(SelectedTile) then
  begin
    for I := SelectedTile.Index - ACount downto 0 do
    begin
      if not FWorkTiles[I].Empty and FWorkTiles[I].Enabled then
      begin
        Result := I;
...
begin
  ...
  begin  
    case Key of
      VK_SPACE: ToggleMaximized(SelectedTile, tmEnter);
      VK_ESCAPE: ToggleMaximized(SelectedTile, tmExit);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (sTile <> SelectedTile) then
      begin
        SelectedTile := sTile;
        Invalidate;
      end;

      if Assigned(OnTileClick) and Assigned(SelectedTile) then
        OnTileClick(Self, SelectedTile, SelectedTile.TileState);
    end;
...
begin
  ...
  begin
    ...
    begin
      FMove.Enabled := False;
      if (MoveTile = SelectedTile) and (toAllowMove in Options) then
        FMode := tmEdit
      else
      begin
    ...
    begin
      ...
      begin
        case Key of
          ...
          begin
            SelectedTile := FWorkTiles[idx];
            pgidx := Self.PageIndexForTileIndex(SelectedTile.Index);
...
begin
  ...
  begin
    ...
    else if (FOwner.SelectedTile = Self) then
...
begin
  ...
  begin
    ...
    begin
      ...
      GetTextPosition(x, y, r, sizer.Width, sizer.Height, tpCenterCenter);
      b := TGPSolidBrush.Create(MakeColor(255, Font.Color));
      g.DrawString(Caption, Length(Caption), ft, MakePoint(x, y), b);
      b.Free;
      sf.Free;
      ft.Free;
    end;
...
begin
  ...
  begin
    ...
    else if (Tile.FOwner.SelectedTile = Tile) then
