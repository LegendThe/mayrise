...
begin
  ...
  if (SelLength = Length(Text)) and (Text <> '') and (EditType <> etPassword) then
  begin
    FBlockChange := true;
  end;
...
begin
  ...
  begin
    ...
    if (csDesigning in ComponentState) and not(csLoading in ComponentState) and (FEditType = etFloat) and (Precision = 0) then
      Precision := 2;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ValStr := '';
        if FLookup.ShowValue and (i <= FLookup.FValueList.Count) then
          ValStr := ' (' + Flookup.FValueList.Strings[i - 1]+')';
...
procedure TAdvMaskEdit.UpdateLabeLPos;
var
  tw: integer;
  r: TRect;
begin
...
procedure TAdvMaskEdit.SetBorderColor(const Value: TColor);
begin
  if (FBorderColor <> Value) then
  begin
    FBorderColor := Value;
