...
uses
  Windows, Forms, SysUtils, imgList, Graphics, Classes, GDIPImageTextItem, GDIPCustomItem,
  AdvGDIP, GDIPPictureContainer, Controls;
...
type
  ...
  THTMLItem = class(TImageTextItem)
  private
...
procedure Register;
begin
  RegisterPolyItem(THTMLItem);
end;

{ THTMLItem }

procedure THTMLItem.AssignEvents(Item: TCustomItem);
begin
  inherited;
  if Item is THTMLItem then
  begin
    OnCaptionAnchorClick := (Item as THTMLItem).OnCaptionAnchorClick;
    OnDescriptionAnchorClick := (Item as THTMLItem).OnDescriptionAnchorClick;
  end;
end;

procedure THTMLItem.Changed;
begin
  FRefreshHTMLCache := true;
  inherited;
end;

function THTMLItem.CreateNewItem(AOwner: TComponent): TCustomItem;
begin
  Result := THTMLItem.Create(AOwner);
end;

class function THTMLItem.CustomClassName: String;
begin
  Result := 'Normal HTML Item';
end;
...
procedure THTMLItem.DoMouseMove(Sender: TObject; Shift: TShiftState; pX,
...
procedure THTMLItem.DoMouseUp(Sender: TObject; Button: TMouseButton;
...
function THTMLItem.DrawText(g: TGPGraphics;
...
begin
  ...
  begin
    ...
    if g.GetTextRenderingHint = TextRenderingHintClearTypeGridFit then
...
function THTMLItem.GetAnchorAtXY(pX, pY: integer; ItemAppearance: TItemAppearance): TAnchorRecord;
var
  r: TGPRectF;
  textr, textoutr: TGPRectF;
  textx, texty: Double;
...
function THTMLItem.GetClassType: TComponentClass;
begin
  Result := THTMLItem;
end;

function THTMLItem.GetVersionNr: integer;
begin
  Result := MakeLong(MakeWord(BLD_VER,REL_VER),MakeWord(MIN_VER,MAJ_VER));
end;

procedure THTMLItem.UseCaption(ACaption: String);
begin
