...
uses
  Classes, Windows, Graphics, Controls, Messages, StdCtrls, ImgList, Math, SysUtils, AdvDropDown, AdvStyleIF
  {$IFDEF DELPHI6_LVL}
  , Types
  {$ENDIF}
  {$IFNDEF TMSDOTNET}
...
type
  ...
  TButtonItems = class(TCollection)
  private
  ...
  public
    ...
    property Buttons: TButtonItems read FButtons write SetButtons;
...
begin
  ...
  begin
    Caption := (Source as TButtonItem).Caption;
    CaptionColor := (Source as TButtonItem).CaptionColor;
    FTag := (Source as TButtonItem).Tag;
    ImageIndex := (Source as TButtonItem).ImageIndex;
    Image.Assign((Source as TButtonItem).Image);
    Enabled := (Source as TButtonItem).Enabled;
    Name := (Source as TButtonItem).Name;
  end
  else
...
function TButtonItem.GetRect: TRect;
begin
...
procedure TButtonItem.SetCaptionColor(const Value: TColor);
begin
...
procedure TButtonItem.SetImage(const Value: TPicture);
begin
  FImage.Assign(Value);
end;
...
procedure TButtonItem.SetTag(const Value: Integer);
begin
  FTag := Value;
end;
...
constructor TButtonItems.Create(AOwner: TPersistent);
begin
...
begin
  Result := TButtonItem(inherited Items[Index]);
end;

//------------------------------------------------------------------------------

function TButtonItems.GetOwner: TPersistent;
begin
  Result := FMyOwner;
end;
...
function TButtonItems.Insert(Index: Integer): TButtonItem;
begin
  Result := TButtonItem(inherited Insert(Index));
end;

//------------------------------------------------------------------------------

procedure TButtonItems.SetItem(Index: Integer;
  const Value: TButtonItem);
begin
...
begin
  inherited;
  FButtons := TButtonItems.Create(Self);
...
procedure TCustomButtonPanel.SetButtons(const Value: TButtonItems);
begin
...
begin
  ...
  {$IFNDEF DELPHI_UNICODE}
  if ((Text='0') or (FNewval)) and (key in ['0'..'9']) then
  {$ENDIF}
  {$IFDEF DELPHI_UNICODE}
  if ((Text='0') or (FNewval)) and (character.IsNumber(key)) then
  {$ENDIF}
  begin
    Text := Key;
    Key := #0;
