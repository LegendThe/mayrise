...
uses
  ...
  AdvFindDialogForm, AdvReplaceDialogForm
  {$IFDEF DELPHI6_LVL}
  , Types
  {$ENDIF}
  {$IFDEF DELPHIXE3_LVL}
...
const
  ...
  // 2.1.7.12 : Fixed : issue with pasting text in wordwrapped memo
  // 2.1.7.13 : Fixed : issue with OnActiveLineChange event
  // 2.1.8.0  : New : Added method FindTextInMemo (workaround for C++ 2009 compiler bug with FindText)
  // 2.1.8.1  : Fixed : Delete line shortcut blocked when memo is ReadOnly
  // 2.1.8.1  : Fixed : Issue with some fixed size fonts
  // 2.1.8.2  : Fixed : very small issue with scrollbar border color difference between designtime & runtime
  // 2.1.8.3  : Fixed : issue with AdvMemo.Clear
  // 2.1.8.4  : Fixed : issue with PgDn/PgUp when nr. of lines in memo is equal to height of memo
  //          : Fixed : issue with trailing spaces when saving to file
  // 2.2.0.0  : New : Terminal, Vista & Windows 7 styles
  // 2.2.0.1  : Fixed : issue with using a memosource and loading files with tabs
  // 2.2.0.2  : Fixed : issue with pressing return on selected text
  // 2.2.1.0  : New : SaveToFile method overload with Encoding added for Delphi 2009
  // 2.2.1.1  : Fixed : issue with updating horiz. scrollbar after loading text
  // 2.2.1.2  : Fixed : issue with InsertTextAtXY()
  // 2.2.1.3  : Fixed : scrollbar drawing issue on Windows Vista, Windows 7
  //          : Fixed : issue with Undo & SmartTabs
  // 2.2.1.4  : Fixed : issue with Lines.SaveToFile and use of a memosource
  // 2.2.1.5  : Fixed : issue with Redo & OnChange event
  //          : Fixed : Cursor position setting from gutter click
  // 2.2.2.0  : Improved : Appearance of autocompletion dropdown when not sizeable
  //          : Improved : Undo of text overwrite
  // 2.2.2.1  : Improved : Behavior of Undo for copy & paste
  //          : Fixed : Issue with literals & brackets
  // 2.2.3.0  : New : WrappedText: string property added to get all wordwrapped text
  // 2.2.3.1  : Fixed : Issue with click on parameter hint
  // 2.2.3.2  : Fixed : Issue with unassigned MemoSource.SaveToFile
  //          : Fixed : Issue with gutter click and setting ActiveLineAtCursor = true
  //          : Fixed : Issue with autocompletion started from '(' character
  // 2.2.4.0  : New : Built in support for Office 2010 colors
  // 2.2.4.1  : Fixed : Issue with click on parameter hint when parent is not form
  // 2.2.5.0  : New : Recognizes file:// as URL specifier
  // 2.2.5.1  : Fixed : Issue with autocompletion dropdown in older Delphi versions
  // 2.2.6.0  : Improved : Find function with frWholeWord option
  // 2.2.6.1  : Fixed : Issue with updating horiz. scrollbar under specific circumstances
  // 2.2.6.2  : Improved : Use of code folding
  //          : Fixed : Issue with selection after line delete
  //          : Fixed : Issue with highlight caching
  // 2.2.6.3  : Fixed : Issue with incorrect triggering of OnKeyPress
  // 2.2.6.4  : Fixed : Issue with using AutoCompletionListImages
  // 2.2.7.0  : New : public function IsEmpty added
  //          : Improved : Selected line background drawing
  // 2.2.7.1  : Fixed : Issue with search text in backwards direction
  // 2.2.7.2  : Fixed : Issue with arrow key navitation when AutoExpand = false
  // 2.2.7.3  : Fixed : Issue with updating horz. scrollbar position
  // 2.2.7.4  : Fixed : Issue with Pascal syntax highlighting with '+' sign.
  // 2.2.7.5  : Fixed : Issue with updating horz. scrollbar position
  //          : Improved : When ActiveLineSettings.ActiveLineTextColor = clNone, it is ignored
  // 2.3.0.0  : New : UndoLineByLine property added
  ...
  // 2.3.0.1  : Fixed : Issue with Imagelist image drawing in gutter
  // 2.3.1.0  : New : Method SaveToHTMLStream() added
  // 2.3.2.0  : New : Configurable autocompletion start tokens in Memo.AutoCompletion
  // 2.3.2.1  : Fixed : Speed issue with SaveToFile()
  // 2.3.2.2  : Fixed : Small issue with drawing of last char of a keyword
  // 2.3.2.3  : Fixed : Issue with gutter line number positioning
  // 2.3.3.0  : Improved : SaveToStream works identical to SaveToFile now wrt ShowModified
  //          : Fixed : Issue with ShowActiveLine
  //          : Fixed : Issue with AutoExpand = false under specific circumstances
  // 2.3.4.0  : New : OnGetAutoCompletionListIndex event added
  // 2.3.4.1  : Improved : Automatic fallback for fonts that do not have a real fixed width
  //          : Fixed : Issue with Wordwrap on older Delphi versions
  // 2.3.5.0  : New : DragMode property added
  ...
  // 2.3.5.1  : Fixed : Issue with text drop depending on position
  // 2.3.6.0  : New : Cursor changes automatically to crDefault when mouse is over selected text to start drag & drop
  //          : Improved : Caret positioning during drag & drop + crtl-drag behavior
  // 2.3.7.0  : New : AutoCompletion.KeepLastSize property added to persist last size of dropdown completion box
  ...
  // 2.3.7.1  : Fixed : Issue with keyword background color
  // 2.3.7.2  : Improved : Drag & drop text with AutoExpand = true
  // 2.3.7.3  : Improved : Performance when deleting large amounts of text
  // 2.3.7.4  : Fixed : Issue with OnChange event during clipboard copy
  // 2.3.7.5  : Fixed : Issue with OnChange in particular circumstances during paste
  // 2.3.7.6  : Fixed : Issue with autocompletion on mouseup
  ...
  // 2.3.8.0  : New : Automatic undo for indent added via memo.BlockIndent()
  //          : New : Function CanCut added
  //          : Improved : Gutter drawing when code folding is enabled
  // 2.3.9.0  : New : Property SelectSingleLine added
  // 2.3.9.1  : Fixed : Issue with gutter drawing when gutter size goes negative
  // 2.3.9.2  : Fixed : Issue with Undo & AutoIndent
  // 2.3.10.0 : New : HintParameter.HintClassDelimiter property added
  //          : Improved : Autocompletion handling
  // 2.3.10.1 : Fixed : Token handling in OnGetParameterHint event
  //          : Fixed : Issue with Undo after linefeed
  // 2.3.11.0 : New : Event OnRetrievedParameterHint added
  // 2.3.12.0 : New : method SelectWordAtCursor added
...
uses
  ClipBrd, ComObj, ShellAPI, Math, IniFiles, ImgList, Consts
...
{$IFNDEF DELPHI7_LVL}
function DupeString(str: string; count: integer): string;
var
  i: integer;
begin
  Result := '';
...
begin
  ...
  if AllowUndo then
  begin
    bi := TIndentUndo.Create(FromLine,ToLine,Indent);
    if Assigned(FMemoSource) then
...
function varposl(sub,s:string; sl:integer; from: integer; var vp: integer): integer;
var
  fnd: boolean;
begin
  Result := -1;
...
begin
  ...
  if FSelButtonDown and (FDragMode = dmAutomatic) and not ReadOnly then
  begin
    FSelDrag := true;
    FSelButtonDown := false;
    BeginDrag(false, 2);
  ...
  begin
    ...
    if (FSelectionMode = smBlock) then
    begin
      BlockSelection := Rect(Min(newpos.x + LeftCol,FSelStartX), min(newpos.y + TopLine,FSelStartY), Max(newpos.X + LeftCol, FSelStartX), Max(newpos.Y + TopLine, FSelStartY));
      Repaint;
...
var
  ...
  AStyle: TStyle;
  lit: char;
  bracketbefore,commentbefore: boolean;
            ...
            for allb := 0 to InternalStyles.FAllStyles.Count - 1 do
              ...
              begin
                ...
                begin
                  ...
                  if (length(s) > 0) and ((s[1] = chBS) or (s[1] = chBE)) and not (lc = InternalStyles.EscapeChar) then
                  begin
                    bracket := not bracket;
                    Break;
                  end;
...
var
  ...
  LineSelStart, LineSelEnd, posln, i, ep, XC, ls, dw, gw, hvp: integer;
  urls: TStringList;
  lnstr,hs: string;
  S, S1, S2, S3: string;
  xSelStartX, xSelStartY, xSelEndX, xSelEndY: integer;
  ...
  begin
    ...
    begin
      ...
      if pos < erect.Left + dw  then
      begin
        LineCanvas.Pen.Color := FRightMarginColor;
        LineCanvas.Pen.Width := 2;
        LineCanvas.MoveTo(eRect.Left + dw, 0);
        LineCanvas.LineTo(eRect.Left + dw, rct.Bottom - rct.Top);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            DrawText(LineCanvas.Handle, PChar(s1), ls1, rct1,
          ...
          begin
            ...
            DrawText(LineCanvas.Handle, PChar(s2), ls2, rct1,
              DT_LEFT or DT_SINGLELINE or DT_NOPREFIX or DT_NOCLIP or DT_EDITCONTROL);
          ...
          begin
            ...
            DrawText(LineCanvas.Handle, PChar(s3), ls3, rct1,
              DT_LEFT or DT_SINGLELINE or DT_NOPREFIX or DT_NOCLIP or DT_EDITCONTROL);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (s[tostart] = SegmentStyle.EndBracket) and (lastch <> InternalStyles.EscapeChar) then
            begin
              BufferingDraw(s[tostart], SegmentStyle);
              Delete(s, tostart, 1);
          ...
          begin
            ...
            begin
              ...
              if (SegmentStyle.EndBracket <> #0) and
                (s[toStart] = SegmentStyle.StartBracket) and (lastch <> InternalStyles.EscapeChar) then
              begin
...
begin
  ...
  begin
    ...
    begin
      ...
      while (HighlightText <> '') and (VarPos(HighlightText, s, hvp) > 0) do
      begin
        hs := copy(s, 1, hvp - 1);
    ...
    begin
      with LineRct do
      begin
        //outputdebugstring(pchar(inttostr(top)+':'+inttostr(bottom - top)));
//        FLineBitmap.Canvas.Brush.Color := clRed;
//        FLineBitmap.Canvas.FillRect(Rect(0,0,FlineBitmap.Width, flinebitmap.Height));

//        FLineBitmap.SaveToFile('.\line'+inttostr(lineno)+'.bmp');
...
{$ENDIF}
  ...
  begin
    ...
    if Gutter.Visible then
      eRect.Left := Gutter.GutterWidth + 1;
...
{$IFDEF FREEWARE}
    ...
    Canvas.TextOut(eRect.Left, eRect.Bottom - 14, 'Copyright © 2012 tmssoftware.com');
...
{$ENDIF}
    ...
    begin
      dw := FCellSize.W * (FRightMargin - LeftCol) + 3;
...
begin
  ...
  begin
    ...
    StdMenu := TPopupMenu.Create(Application);

    StdMenu.Images := FContextMenuImages;
    FContextMenuImages.DrawingStyle := dsTransparent;

    mnu := TMenuItem.Create(StdMenu);
    ...
    DoCustomizeContextMenu(MCurX, MCurY, StdMenu);

    StdMenu.Popup(Message.XPos, Message.YPos);
  end
  else
    inherited;
end;
...
begin
  ...
  begin
    Pen.Color := FBorderColor;
    MoveTo(Width - 1, Top);
    LineTo(Width - 1, Top + Height);
    Pen.Color := clWhite;
    MoveTo(Width - 2, Top);
    LineTo(Width - 2, Top + Height);

    Brush.Color := FColor;

    if (FColorTo = clNone) then
      FillRect(Rect(Left, Top, Left + Width - 3, Top + Height))
    else
      DrawGradient(bmp.Canvas, FColor, FColorTo, 48, Rect(Left, Top, Left + Width - 3, Top + Height), True);
    ...
    begin
      ...
      begin
        ...
        begin
          Memo.OnGutterDraw(Memo, bmp.Canvas, LineNoR, Rect(Left, T, Left + Width - 2, T + H));
          Inc(LineNoR);
        end;
        T := T + H;
...
var
  ...
  procedure NextLine;
  begin
    if (freDown in Options) or (freSelection in Options) then inc(i)
    else
      dec(i);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FInitParentHandle and not FDialogInitialized then
      begin
        SetWindowLong(FFindDialog.Handle, GWL_HWNDPARENT,  Application.MainFormHandle);
...
begin
  ...
  begin
    ...
    begin
      ...
      if FInitParentHandle and not FDialogInitialized then
      begin
        SetWindowLong(FReplaceDialog.Handle, GWL_HWNDPARENT, Application.MainFormHandle);
        FDialogInitialized := true;
      end;
      {$ENDIF}
    end;
...
begin
  ...
  begin
    ...
    until rz = -1; // no more occurences found
...
//check if event handler modified the string, if not then apply std behaviour
  ...
  //so Test(parama : string); becomes Test
      ...
      for i := 1 to Length(sEntry) do
      begin
        if not ((sEntry[i] = '(') or (sEntry[i] = ';') or (sEntry[i] = ' ') or (sEntry[i] = ':')) then
          s := s + sEntry[i]
        else
          break;
      end;
    ...
    begin
      ...
      begin
        for vp := CurX + 1 to length(s) do
          if (s[vp] = #32) or (s[vp] = '(') or (s[vp] = ')') or (s[vp] = '[') or (s[vp] = ']') then
          begin
            ae := vp - 1;
            Break;
          end;
