...
uses
  ...
  Buttons, Menus, AdvGDIP, AdvHintInfo
  {$IFDEF DELPHIXE3_LVL}
...
type
  ...
  public
    ...
    //ADVSMOOTHCOMBOBOX
    procedure Draw(ACanvas: TCanvas; R: TRect; DisplayIndex: integer; DragItem: Boolean = false; Transparent: boolean = false);
...
begin
  ...
  if FDisplayList.Count > 0 then
    Result := FDisplayList.GetItem(FDisplayList.Count - 1).ItemRect.Top + GetItemHeight(FDisplayList.GetItem(FDisplayList.Count - 1).FItem);
        ...
        case Key of
          ...
          VK_UP, VK_LEFT:
            begin
              ...
              repeat
                ...
                FSelectedItemIndex := Min(Items.Count - 1, Max(0, FSelectedItemIndex));
              until ((Items[FSelectedItemIndex].Visible) or (FSelectedItemIndex = 0));
          ...
          VK_HOME:
            begin
              ...
              while not Items[FSelectedItemIndex].Visible and (FSelectedItemIndex < Items.Count - 1) do
              begin
                inc(FSelectedItemIndex);
                FSelectedItemIndex := Min(Items.Count - 1, Max(0, FSelectedItemIndex));
              end;
          ...
          VK_END:
            begin
              FSelectedItemIndex := FDisplayList.Count - 1;
              while not Items[FSelectedItemIndex].Visible and (FSelectedItemIndex > 0) do
              begin
                dec(FSelectedItemIndex);
                FSelectedItemIndex := Min(Items.Count - 1, Max(0, FSelectedItemIndex));
              end;
          ...
          VK_NEXT:
            begin
              FSelectedItemIndex := FSelectedItemIndex + 5;
              FSelectedItemIndex := Min(Items.Count - 1, Max(0, FSelectedItemIndex));
              while not Items[FSelectedItemIndex].Visible and (FSelectedItemIndex > 0) do
              begin
                dec(FSelectedItemIndex);
...
begin
  ...
  begin
    ...
    begin
      ...
      fcaptionr := MakeRect(gppointf.X + GraphicLeftMargin, gppointf.Y, src.Width + 3, src.Height);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          w := 0;
          if FGraphicLeftType <> gtNone then
            w := FGraphicLeftWidth + GraphicLeftMargin;
          ...
          fcaptionr := MakeRect(gppointf.X, gppointf.Y, src.Width + 3, src.Height);
...
procedure TAdvSmoothListBoxItem.Draw(ACanvas: TCanvas; R: TRect; DisplayIndex: integer; DragItem: Boolean = false; Transparent: boolean = false);
var
  o: TAdvSmoothListBox;
  a, s, k: String;
  XSize, YSize: integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          fselected.Focus := FFocused and TabStop and ShowFocus and focustopitem;
        ...
        begin
          ...
          fnormal.Focus := FFocused and TabStop and ShowFocus and focustopitem;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  gpbmp := TGPBitmap.Create(imgbmp.Width, imgbmp.Height);
                  gp := TGPGraphics.Create(gpbmp);
                  h := gp.GetHDC;
                  cah := TCanvas.Create;
                  cah.Handle := h;
                  Images.Draw(cah, 0, 0, GraphicLeftIndex);
                  cah.Free;
                  gp.ReleaseHDC(h);
                  gp.Free;
                  g.DrawImageRect(gpbmp, MakeRect(fgrLeft.Left, fgrLeft.Top, fgrLeft.Right - fgrLeft.Left, fgrLeft.Bottom - fgrLeft.Top));
                  gpbmp.Free;
                end;
                imgbmp.Free;
              end;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  cah := TCanvas.Create;
                  cah.Handle := h;
                  Images.Draw(cah, 0, 0, GraphicRightIndex);
                  cah.Free;
                  gp.ReleaseHDC(h);
                  gp.Free;
                  g.DrawImageRect(gpbmp, MakeRect(fgrRight.Left, fgrRight.Top, fgrRight.Right - fgrRight.Left, fgrRight.Bottom - fgrRight.Top));
                  gpbmp.Free;
                end;
                imgbmp.Free;
              end;
    ...
    begin
      ...
      pth := CreateRoundRectangle(delr, 2, rtBoth, false);
      ...
      pthinner := CreateRoundRectangle(delr, 2, rtBoth, false);
...
begin
  ...
  begin
    ...
    case Fowner.CategoryType of
      alphanumeric: Result := bmp.Canvas.TextWidth('W') + 4;
