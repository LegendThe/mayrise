...
begin
  ...
  if IsVista then
  begin
    if (verinfo.dwMajorVersion = 6) and (verinfo.dwMinorVersion = 1) then
      Result := Result + '(Win 7)'
    else
      Result := Result + '(Vista)';
  end;
...
begin
  ...
  begin
    ...
    if FCertCheck = ccDisable then
      OpenFlags := OpenFlags or INTERNET_FLAG_IGNORE_CERT_DATE_INVALID or INTERNET_FLAG_IGNORE_CERT_CN_INVALID;
...
begin
  ...
  FProgressBar.Width := Width - 20 - 2 * GetSystemMetrics(SM_CXBORDER);
  Caption := LoadStr(978);
