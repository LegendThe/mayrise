...
uses
  Forms, Windows, SysUtils, Classes, Graphics, Menus, Controls,
  AdvGDIP
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  (SendMessage(ClientHandle, WM_MDIGETACTIVE, 0,
                      Integer(@Maximized)) <> 0)
                  and Maximized then
                  begin
                    NrOfIcons := 0;
...
{$IFDEF DELPHI_UNICODE}
          ...
          begin
            if not Enabled and Assigned
              (((MenuItem.Action as TCustomAction)
                  .ActionList as TActionManager).DisabledImages)
              then
            begin
              drwenabled := True;
              ImgList := ((MenuItem.Action as TCustomAction)
                  .ActionList as TActionManager).DisabledImages;
            end
            else
              ImgList := ((MenuItem.Action as TCustomAction)
                  .ActionList as TActionManager).Images;
...
begin
  ...
  if AdvMenu.Self is TPopupMenu then
    Inc(Level);
    if (GetWindowLong(WindowHandle, GWL_WNDPROC) <> Integer(@MenuWindowProc))
      then
    begin
      SetWindowLong(WindowHandle, GWL_USERDATA, GetWindowLong(WindowHandle,
          GWL_WNDPROC));
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            SetWindowLong(OldWndHandle, GWL_WNDPROC,
              Integer(@MDIClientWndProc));
