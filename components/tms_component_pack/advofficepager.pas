...
begin
  ...
  begin
    ...
    begin
      ...
      if (Button = mbLeft) and AdvPages[Tab].TabEnabled then
      begin
        if TabReorder and ((csDesigning in ComponentState) or not AllowTabUndock or AdvPages[Tab].Locked) then
        begin
          BeginDrag(false,4);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (TabPosition in [tpTop, tpBottom]) then
        begin
          if (j + sp + AdvPages[i].FTabRect.Right - AdvPages[i].FTabRect.Left < avlspace) then
          begin
        ...
        begin
          if (j + sp + AdvPages[i].FTabRect.Bottom - AdvPages[i].FTabRect.Top < avlspace) then
          begin
      ...
      begin
        if (j <> acvTabR) then
        begin
          FTabRows[i].X := tbRows[j].Y;     // tab row start index
