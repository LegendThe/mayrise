...
uses
{$DEFINE USE_ADSCONNECTIONLIST}
{$IFDEF USE_ADSCONNECTIONLIST}
  AdsConnectionList, //KM 29/05/2009
{$ENDIF}
...
{$IFNDEF USE_ADSCONNECTIONLIST} //KM 29/05/2009
...
begin
   ...
   ACECHECK( self, ACEUNPUB.AdsGetSQLStmtParams( PAceChar( AnsiString( strParams ) ) ) );
...
{$ENDIF }
   ...
   ACECheck(self,AdsStmtSetTableLockType(mhStmt,usLockType));
...
{$ENDIF }
   ...
   if ( mhConnection = INVALID_ACE_HANDLE ) then
      ...
      begin
         ...
         {$IFDEF USE_ADSCONNECTIONLIST} //KM 29/05/2009
         mhConnection:=GlobalAdsConnectionList.AddConnection(GetDatabasePath);
         {$ELSE} //KM 29/05/2009
...
{$ENDIF}
               ...
               ftAutoInc:
               begin
                  ACECheck( self, ACE.AdsSetString( mhStmt, ADSFIELD( I + 1 ), PAceChar( AnsiString( moParams[i].AsString ) ),
               ...
               begin
                  ...
                  ACECheck( self, ACEUNPUB.AdsSetTimeStampRaw( mhStmt, ADSFIELD( I + 1 ),
                                                    PAceChar( @stDateType ),
                                                    sizeof( stDateType )));
               end;
               ...
               ftCurrency,
               ftFloat:
                  ACECheck( self, AdsSetDouble( mhStmt, ADSFIELD( I + 1 ),
                                          moParams[i].AsFloat ));

               ftBoolean:
                  if ( moParams[i].AsBoolean ) then
                     ACECheck( self, AdsSetLogical( mhStmt, ADSFIELD( I + 1 ), ADS_TRUE ))
                  else
                     ACECheck( self, AdsSetLogical( mhStmt, ADSFIELD( I + 1 ), ADS_FALSE ));
...
{$ENDIF }
   ...
   if meIndexCollationMismatch = icmIgnore then
     ...
     if meIndexCollationMismatch = icmReindex then
  ...
  if( ( AdsConnection <> nil ) and
   ...
   begin
       ...
       *}
      {$IFDEF USE_ADSCONNECTIONLIST} //KM 29/05/2009
      hConnect:=GlobalAdsConnectionList.AddConnection(GetDatabasePath);
      {$ELSE} //KM 29/05/2009
