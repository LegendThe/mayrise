...
begin
   ...
   ACECheck2( self, ACE.AdsCreateIndex61( GetAceTableHandle, acFileName, acTagName,
             acExpression, acCondition, acWhile, sOptions, mpoAdsTableOptions.AdsIndexPageSize, @hTemp ));
...
begin
   ...
   ACECheck ( self, ACE.AdsCreateTable( hConnect,
                                  acFileName,
                                  nil,
                                  usTableType,
                                  usCharType,
...
{$ELSE}
   ...
   if usFieldType = ADS_MEMO then
      ACECheck2( self, ACE.AdsGetMemoLength( GetAceTableHandle, PAceChar( AnsiString( strFieldName ) ), @ulLen ))
   else if usFieldType = ADS_BINARY then
      ACECheck2( self, ACE.AdsGetBinaryLength( GetAceTableHandle, PAceChar( AnsiString( strFieldName ) ), @ulLen ))
   else if usFieldType = ADS_DATE then
      ulLen := ADS_MAX_DATEMASK
   else
      ACECheck2( self, ACE.AdsGetFieldLength( GetAceTableHandle, PAceChar( AnsiString( strFieldName ) ), @ulLen ));
{$ENDIF}
...
begin
   ...
   if( AdsGetFieldType( strFieldName ) = AdsfldMEMO ) then
      ulLen := AdsGetMemoLength( strFieldName ) + 1
   else
      ulLen := AdsGetFieldLength( strFieldName ) + 1;
...
var
   ...
   hIndex  : ADSHANDLE;
   hConnect: ADSHANDLE;
   ShadowHandle:ADSHANDLE;
   Buffer: array[0..ADS_MAX_INDEX_EXPR_LEN] of char;
   KeyBuffer: array[0..ADS_MAX_KEY_LENGTH] of char;
   s:string;
begin
   {  No need to check browse mode since we are not moving in the table }

   //Get the index handle
   ACECheck2( self, ACE.AdsGetIndexHandle( GetAceTableHandle, pchar(strTag), @hIndex ) );

   //Do the look up
   ACECheck2( self, ACE.AdsLookupKey( hIndex, pchar(strKey),length(strKey), ADS_STRINGKEY, @usFound ));
   Result := ( usFound <> 0 );

   //We found it, but it could be a deleted record
   if Result then
     begin
       //First, check to see if the index condition excludes deleted records
       usLen := ADS_MAX_INDEX_EXPR_LEN;
       ACECheck2( self, ACE.AdsGetIndexCondition( hIndex, @Buffer, @usLen ) );
       s:=uppercase(StrPas(@Buffer));
       if pos('DELETED',s)=0 then
         begin
           //Index can contain deleted records
           //So we must open up a shadow table and do a gotokey

           //Grab the open tables connection handle
           ACECheck2( self, ACE.AdsGetTableConnection( GetAceTableHandle, @hConnect ));

           {  Set the Lock type from the properties associated to the table }
           ACECheck2( self, ACE.AdsGetTableLockType(GetAceTableHandle, @usLockType));//#10802 Get actual lock type used
...
//              usLockType := ADS_COMPATIBLE_LOCKING;
           ...
           s:=GetDatabasePath + '\' + TableName;//Table path + name

           //Open the shadow table
           ACECheck( self, ACE.AdsOpenTable(hConnect,
                                            PChar(s),
                                            nil,
                                            musOpenTableType,
                                            usCharType,
           ...
           try
             //Get the shadow table index handle
             ACECheck2( self, ACE.AdsGetIndexHandle(ShadowHandle, pchar(strTag), @hIndex ) );

             //Work out the length of the Index entry
             //Get the index exression
             usLen := ADS_MAX_INDEX_EXPR_LEN;
             ACECheck2( self, ACE.AdsGetIndexExpr( hIndex, @Buffer, @usLen ) );

             //Evaluate the index expression against the current record - just for the length
             KeyLen:=ADS_MAX_KEY_LENGTH; //max size of buffer
             ACECheck2( self, ACE.AdsEvalStringExpr(ShadowHandle,Buffer,@KeyBuffer, @KeyLen ));//The length returned is the index entry length
             s:=copy(strKey+StringOfChar(' ',KeyLen),1,KeyLen);//Spacefill the search key
             //Do the Gotokey
             ACECheck2( self, ACE.AdsSeek(hIndex,pchar(s),KeyLen,ADS_STRINGKEY,ADS_HARDSEEK, @usFound ) );
             Result := ( usFound <> 0 );

           finally
             ACE.AdsCloseTable(ShadowHandle);
           end;
