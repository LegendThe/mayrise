...
{$ENDIF}
   ...
   TAdsTableTypes = ( ttAdsCDX, ttAdsNTX, ttAdsADT, ttAdsDefault );
   ...
   {  enumerated list of the character types whether ANSI or OEM }
...
{$ENDIF}
   ...
   protected {  Protected methods for the TAdsDataSet }
      ...
      function  IsFieldBlank( pcData : PAdsBuffer; ulDataLen : UNSIGNED32; usADSDataType : UNSIGNED16 ) : Boolean;
...
{$ENDIF}
      ...
      property StoreActive:     Boolean read FbStoreActive write FbStoreActive default false;
...
{  third party controls. }
   ...
   public
      onAfterCreateFields: TNotifyEvent;
...
function GetAliasInfo( strAlias : string; var strPath : string;
                       var eTableType : TAdsTableTypes ) : boolean; forward;

function GetAliasPathAndFileName : string; forward;
...
begin
   ...
   begin
      ...
      if ( usFieldType = ADS_STRING ) then
...
begin
   ...
   if ( strAliasSearchPath <> '' ) then
      pcPath := pChar( strAliasSearchPath )
   else
      pcPath := nil;
...
{$ENDIF}
   ...
   begin
      ...
      StrPCopy( pcResultStr, AnsiString( Filter ) );
   ...
   begin
      ...
      if ( usCurrPos + 8 < usStrLen ) and
         ( AnsiStrLIComp( @pcResultStr[usCurrPos], 'CONTAINS', 8 ) = 0 ) then
      begin
...
{$ENDIF }
   ...
   {  structural CDX }
   if ( TableType = ttAdsCDX ) or ( TableType = ttAdsADT ) then
   ...
   Try
      ...
      if ( TableType = ttAdsCDX ) or ( TableType = ttAdsADT ) then
         ACECheck( self, ACE.AdsCreateIndex61( hHoldHandle, nil, PAceChar( AnsiString( strName ) ) ,
                   PAceChar( AnsiString( strFields ) ), nil, nil, ulOptions, mpoAdsTableOptions.AdsIndexPageSize, @hTempIndex ) )
      else
         ACECheck( self, ACE.AdsCreateIndex61( hHoldHandle, PAceChar( AnsiString( strName ) ), nil,
                   PAceChar( AnsiString( strFields ) ), nil, nil, ulOptions, mpoAdsTableOptions.AdsIndexPageSize, @hTempIndex ) );
   Finally
...
{$ENDIF }
   ...
   Try
      {  If the table is open use the open handle and create the index }
      if ( TableType = ttAdsCDX ) or ( TableType = ttAdsADT ) then
         ACECheck( self, ACE.AdsCreateIndex61( hHoldHandle, PAceChar( AnsiString( strFileName ) ), PAceChar( AnsiString( strIndexName ) ) ,
                   PAceChar( AnsiString( strFields ) ), nil, nil, ulOptions, mpoAdsTableOptions.AdsIndexPageSize, @hTempIndex ) )
      else
         ACECheck( self, ACE.AdsCreateIndex61( hHoldHandle, PAceChar( AnsiString( strIndexName ) ), nil,
                   PAceChar( AnsiString( strFields ) ), nil, nil, ulOptions, mpoAdsTableOptions.AdsIndexPageSize, @hTempIndex ) );
   Finally
...
{$ENDIF }
   ...
   if Active then
     begin
//       if (DebugHook<>0) and (State in [dsEdit, dsInsert]) then  //2007
   //      showMessage('TAdsDataSet.SetIndexName while table in edit mode');
       if State in [dsEdit, dsInsert] then  //2007
         Cancel;                            //make it as D3 use to work
       CheckBrowseMode;
     end;
...
{$ENDIF}
   ...
   CheckBrowseMode;
   DoBeforeScroll;
   if ( meActiveIndexType = ndxNone ) then
   begin
      {  no index is active }
      if filtered then
      begin
...
begin
   ...
   if ( mpoAdsTableOptions.AdsCharType = OEM ) then
   begin
      for usFieldCount := 1 to usNumFields do
      begin
         ...
         if ( usFieldType = ADS_STRING ) or ( usFieldType = ADS_CISTRING ) then
         begin
...
{$IFDEF UseADSTableList}
   ...
   ADStableList.AddObject(Tablename+'['+s+']',TObject(handle));
{$ENDIF}
...
{$endif}
   ...
   if DefaultFields then
     CreateFields //Creates all TFields anyway
   else
     begin
       i:=FieldDefs.IndexOf('CHANGED_TS');
       if i>-1 then
         begin
           if FindField('CHANGED_TS')=nil then
             begin
               // Make sure there is a CHANGED_TS TField if present in the table.
               Changed_TSField:=TStringField.Create(self);
               with Changed_TSField do
                 begin
                   FieldName:='CHANGED_TS';
                   Size:=FieldDefs[i].Size;
                   FieldKind:=fkData;
                   Visible:=false;//Hide it from Grids!
                   DataSet:=self;
                 end;
             end
         end
       else
         begin // No Changed_TS field in DBF
           aField:=FindField('CHANGED_TS');
           if aField<>nil then //Yet we have a TField!
             if (aField.Name='') and (aField.FieldKind=fkData) then // did we auto created it ?
               aField.Free;
         end;
     end;
...
{$ENDIF}
   ...
   if assigned(onAfterCreateFields) then
     onAfterCreateFields(self);
...
{$ENDIF }
   ...
   try
      if Handle <> INVALID_ACE_HANDLE then { Close the table }
         begin
{$IFDEF UseADSTableList}
           i:=ADStableList.IndexOfObject(TObject(Self.Handle));
           if i>-1 then
             ADStableList.Delete(i);
{$ENDIF}
...
{$ENDIF}
   ...
   if musAceTableType = ADS_ADT then
      case usAdsDataType of
         ...
         ADS_MEMO, ADS_BINARY, ADS_IMAGE, ADS_VARCHAR :
         Begin
...
{$ENDIF }
   ...
   if ( state in [ dsEdit, dsInsert ] ) And
      ...
      bIsBlank := IsFieldBlank( pcData, ulFieldLength, usAdsFieldType );
...
{$ENDIF}
      ...
      ADS_MODTIME :
         begin
         { The value returned must be a doubleThe double contains the number of
      ...
      begin
         ...
         for sCount := 0 to usDataLength - 1 do
           PChar( pBuffer )[sCount] := pcData[ sCount ];

         PChar( pBuffer )[usDataLength] := #0;
         ...
         if usDataLength <> 0 then
           begin
             ...
             while usDataLength <> 0 do
               begin
                  if PChar( pBuffer )[usDataLength] = ' ' then
                    PChar( pBuffer )[usDataLength] := #0
                  else
                    Break;
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            if (State <> dsBrowse) then  // if in OnValidate edit mode has been canceled, we dont want to change field's buffer
              begin
                ADSPutField( FieldNo, DataType, pcRecBuf, Buffer );   {  put the field to the given buffer and to the database }
      ...
      begin
         ...
         begin
            ...
            begin
               ...
               if ( ADS_STRING = usFieldType ) or ( ADS_CISTRING = usFieldType ) then
               begin
                  ...
                  begin
                     ...
                     if ( ulOffset = ulFieldOffset+ulFieldLen ) then
                        ...
                        ACECheck( self, ACE.AdsSetField( Handle, ADSFIELD( usFieldNums ),
                                                   @pRecBuffer[ulFieldOffset], ulFieldLen ));
                  end
                  else
                     ACECheck( self, ACE.AdsSetField( Handle, ADSFIELD( usFieldNums ),
                                                   @pRecBuffer[ulFieldOffset], ulFieldLen ));
                  ...
                  {  Set field raw }
                  ACECheck( self, ACEUNPUB.AdsSetFieldRaw( Handle, ADSFIELD( usFieldNums ),
                                                           @pRecBuffer[ulFieldOffset], ulFieldLen ));
            end;
...
{$ENDIF }
    ...
    *}
   ulBufferSize := SizeOf( TAdsKeyBuffer ) + musRecordSize + usNumFields * SizeOf( UNSIGNED16 );
...
{$ENDIF }
   ...
   if ( Result ) and (( meTableType = ttAdsCDX ) or ( meTableType = ttAdsNTX )) then
   begin
   ...
   try  { Use this try block to free the poFields object }
      ...
      try  {  Use this try block to set the state back }
         ...
         begin
            ...
            for usFieldCount := 0 to poFields.Count - 1 do
               ...
               begin
                  ...
                  if ( ( ADS_STRING = usFieldType ) or ( ADS_CISTRING = usFieldType ) ) And
         ...
         begin
            ...
            begin
               {  We expect to get AE_INSUFFICIENT_BUFFER }
...
{$ENDIF }
   ...
   ACECheck( nil, ACE.AdsCreateIndex61( hTable, PAceChar( AnsiString( strIndexName ) ),
                                 PAceChar( AnsiString( strTagName ) ), PAceChar( AnsiString( strIndexExpr ) ),
                                 Nil, Nil, ulOptions, ulPageSize, @hIndex ) );
...
{$IFDEF ADSDELPHI6_OR_NEWER }
         ftFMTBCD:
            strFieldsList := strFieldsList + 'Num,' + IntToStr( poCurFieldDef.Precision )
...
{$ENDIF}
   ...
   if mpoAdsTableOptions.AdsLockType = Proprietary then
     ...
     {$IFDEF USE_ADSCONNECTIONLIST}
     usLockType:=GlobalAdsConnectionList.GetLockingType(GetDatabasePath);
     {$ELSE}
   ...
   ACECheck( self, ACE.AdsCreateTable71( hConnect, PAceChar( AnsiString( strFileName ) ), nil, musOpenTableType,
...
{$ENDIF }
   ...
   Try
      ...
      begin
         ACECheck( nil, ACE.AdsGetMemoDataType( hAdsTable, ADSFIELD( ulFieldNo ), @usFieldType ) );
      ...
      begin
         ulRetCode := ACE.AdsGetMemoLength( hAdsTable, ADSFIELD( ulFieldNo ), @ulSize );
         inc( ulSize );
      end
      else
      ...
      begin
         ACECheck( nil, ACE.AdsGetBinary( hAdsTable, ADSFIELD( ulFieldNo ), 0, Memory, @ulSize ) );
      end;
...
{$ENDIF }
   ...
   if not ( usFieldType in [ ADS_VARCHAR, ADS_BINARY, ADS_IMAGE, ADS_MEMO ] ) then
...
begin
   ...
   if ( mpoAdsTableOptions.meAdsCharType = ANSI ) then
      oAdsDatasetOptions.musAdsCharType := ADS_ANSI
   else
      oAdsDatasetOptions.musAdsCharType := ADS_OEM;
