...
type
  ...
  protected {event variables}
    ...
    itemJustChanged        : boolean;
    procedure setffCurrentItem(ci: TAbArchiveItem);
    property FCurrentItem  : TAbArchiveItem read ffCurrentItem write setffCurrentItem;    
  protected {methods}
...
begin
  ...
  if itemJustChanged then
    begin
      itemJustChanged:=false;
      DoArchiveProgress(min((((findItem(item))*100) div count)+1,100), Abort);
    end;
end;

procedure TAbArchive.setffCurrentItem(ci: TAbArchiveItem);
begin
{$IFDEF VERSION7}      { Delphi 7.0 or higher }
  if (ci.FAction=aaAdd)and(ffCurrentItem<>ci) then    // missing progress on item change
    itemJustChanged:=true;
{$ENDIF}
  ffCurrentItem:=ci;
end;
