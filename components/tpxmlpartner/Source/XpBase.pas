...
begin
  ...
  if (iIndex < 1) or (iCount = 0) or (iIndex > iLen) then
    ...
    iResultIndx := 1;
    for iIndx := iIndex to iIndex + iCountTmp -1 do begin
...
begin
  if (aCh <= #$7F) then
    ...
    Result := 1              { $80--$BF is an error } //Accept it!!!!!!!!! KM
  else if (aCh <= #$DF) then
    Result := 2
  else if (aCh <= #$EF) then
    Result := 3
...
begin
{
  ...
  Result := ((1 <= aCh) and (aCh <= $D7FF)) or //#9781
            (($E000 <= aCh) and (aCh <= $FFFD)) or
            (($10000 <= aCh) and (aCh <= $10FFFF));
end;
{--------}
...
{==TXpMemoryStream===================================================}
{$IFDEF VERSIONXE2}
procedure TXpMemoryStream.SetPointer(Ptr: Pointer; const Size: NativeInt);
{$ELSE}
