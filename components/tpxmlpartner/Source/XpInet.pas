...
const
  AorW = {$IFDEF UNICODE}'W'{$ELSE}'A'{$ENDIF};
...
function HttpQueryInfo(hRequest: THandle; dwInfoLevel: DWORD;
    lpBuffer: Pointer; var lpdwBufferLength: DWORD;
    lpdwIndex: PDWORD): BOOL; stdcall; external 'wininet.dll' name 'HttpQueryInfo'+AorW;  // Win2000


function GetInfo(hreq: THandle; InfoLevel: Cardinal;
...
begin
  InfoLen := SizeOf(InfoBuf^);
  GetMem(InfoBuf, InfoLen);
  try
    Result := HttpQueryInfo(hreq, InfoLevel or HTTP_QUERY_FLAG_NUMBER, InfoBuf, InfoLen, nil);
    if not Result then
    begin
      ...
      begin
        case ErrorCode of
          ERROR_INSUFFICIENT_BUFFER: raise EXpException.Create('Buffer too small.');
    ...
    if hConn <> nil then begin
      ...
      if hRequest <> nil then begin
        @lHttpSendRequest := GetProcAddress(XphWininet,
                                            'HttpSendRequest'+AorW);
        if lHttpSendRequest(hRequest, nil, 0, nil, 0) then
        begin
          //SZ check for 404 error
          GetInfo(THandle(hRequest), HTTP_QUERY_STATUS_CODE, StatusCode);
          ...
          try
            while lInternetQueryDataAvailable(hRequest,
              ...
              // SZ 12.06.2010 now done correctly using GetInfo
//              if (FirstLoop) and
    ...
    if hConn <> nil then begin
      if sDir <> '' then begin
        @lFtpSetCurrentDirectory :=
          GetProcAddress(XphWininet,
                         'FtpSetCurrentDirectory'+AorW);
        if not lFtpSetCurrentDirectory(hConn, PChar(sDir)) then
          oErrors.Add(sInvalidFtpDir);
      end;
      @lFtpOpenFile := GetProcAddress(XphWininet, 'FtpOpenFile'+AorW);
...
begin
  ...
  if Assigned(hInter) then begin
    @lInternetConnect := GetProcAddress(XphWininet, 'InternetConnect'+AorW);
    ...
    if Assigned(hConn) then begin
      if sDir <> '' then begin
        @lFtpSetCurrentDirectory := GetProcAddress(XphWininet, 'FtpSetCurrentDirectory'+AorW);
        if not lFtpSetCurrentDirectory(hConn, PChar(sDir)) then
          oErrors.Add(sInvalidFtpDir);
      end;
      @lFtpPutFile := GetProcAddress(XphWininet, 'FtpPutFile'+AorW);
