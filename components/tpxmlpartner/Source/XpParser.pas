...
{$ENDIF}
    ...
    function ParseMemory(const aBuffer; aSize : Longint) : Boolean;
...
{--------}
function TXpParser.ParseMemory(const aBuffer; aSize : Longint) : Boolean;
var
...
begin
  ...
  while True do begin
    ...
    if (TempChar = '%') or (TempChar = '<') or (TempChar = '>') or
       ...
       (TempChar = ';') or (TempChar = '/') or (TempChar = #0) or      {!!.51}{!!.52}
       (TempChar = #1) then                                            {!!.52}
      Break
    else
