...
type
  ...
  public
    ...
    procedure AddParamText(const name,value: string);
...
function EscapeSQLvalue(const aStr:string):string;
...
procedure TSQLiteDatabase.AddParamText(const name,value: string);
var
  par: TSQliteParam;
begin
  par := TSQliteParam.Create;
...
function TSQLiteTable.GetFieldByName(const FieldName: string): string;
begin
...
function TSQLiteUniTable.GetFieldByName(const FieldName: string): string;
begin
  Result := GetFields(self.GetFieldIndex(FieldName));
end;
...
{AddStringToPChar *************************************************************}
procedure AddStringToPChar(var P:PChar; const aStr:string);
var
  xlen:integer;
begin
  xLen:=length(aStr);
  ...
  while Sp^ <> #0 do
    begin
      case Sp^ of
        '''': AddStringToPchar(Rp,'''''');
//        '"':  AddStringToPchar(Rp,'\"');
//        #10:  AddStringToPchar(Rp,'\n');
//        #13:  AddStringToPchar(Rp,'\r');
//        #9:   AddStringToPchar(Rp,'\t');
//        '\':  AddStringToPchar(Rp,'\\');
      else
        Rp^ := Sp^;
        Inc(Rp);
