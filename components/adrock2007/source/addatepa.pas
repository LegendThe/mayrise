     ...
     begin
      ...
      if (InputStyle =isDuration) then
        DOTextRect(Canvas, (SubFieldBeingEdited = SubFieldHour),not IsValidHour, TRUE,
           (SubFieldBeingEdited = SubFieldHour), ARect,FormatFloat('000',fDate.HourLong))
...
begin
  ...
  (((fDate.DateTime = 0) and (deoShowWhenNil in Options)) or (fDate.DateTime <> 0))) then
    begin
     ...
     if (InputStyle <> isTime) and (InputStyle <> isTimeKeepDate) and (InputStyle <> IsDuration) then
       begin
...
begin
    ...
    if (inputStyle = isduration) then
      HourWidth          := Canvas.TextWidth('000')+TEXTWIDTH_EXTRAPIXELS
    else
    ...
    if (InputStyle = IsDuration) then
     begin
         ...
         MonthStart               := 0;
         YearStart                := 0;
         FirstDateSeparatorStart  := 0;
         FirstDateSeparatorEND    := 0;
         SecondDateSeparatorStart := 0;
         SecondDateSeparatorEnd   := 0;

     end
    else if (InputStyle = isTime) or (InputStyle = isDateTime) or (InputStyle = IsTimeKeepDate) then
