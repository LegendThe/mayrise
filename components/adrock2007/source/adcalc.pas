...
uses
  ...
  {$endif}
  Clipbrd,
  DesignIntf, Forms, StdCtrls, Buttons, Dialogs, ExtCtrls, DB, DBTables,
  DesignEditors,
  AdButEdt;
...
begin
 ...
 {Initialize form Height & Width }
  AHeight := 5 * fButtonHeight - (TAdrockCalcEdit(RealctlParent).fButtonGap)+2+2; { +2; }
  ...
  if (IsCalculatorOpen = FALSE) then
    begin
      ...
      { Bug:
        ...
        Make sure the calculator is above the current form, if the formStyle was set
        to fsStayOnTop then the calculator would open below the current form (not a
        very good situation), the line below (SetWindowPos) fix's this problem.

        March 8, 2000
      }
      SetWindowPos(Calculator.Handle, HWND_TOPMOST, 0,0,0,0, SWP_NOMOVE	or SWP_NOSIZE);
