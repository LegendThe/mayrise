...
# 3 - gzclose Error
-----------------------------------------------------------}
function TGzip.gz_uncompress (infile:gzFile; var outfile:file; fsize:ulong) : integer;
var
  len     : integer;
  written : uInt;
  buf  : packed array [0..BUFLEN-1] of char{byte}; { Global uses BSS instead of stack }
  errorcode : byte;
  lensize : ulong;
begin
  errorcode := 0;
    ...
    begin
      ...
      if FProgressStep > 0 then
        begin
          {$WARNINGS OFF}
          lensize := lensize + len;
          if ((lensize / fsize) * 100 >= FProgress + FProgressStep) or
             (lensize = fsize) then
            begin
              FProgress := Trunc((lensize / fsize) * 100);
              DoOnProgress
    ...
    begin
      ...
      if (ioerr <> 0) then
        ...
        begin
          ...
          if FDeleteSource then
            DeleteFile(FFileSource);
...
begin
  ...
  if (ioerr <> 0) then
    ...
    TRY
      ...
      if (head[0] <> $1F) or (head[1] <> $8B) or (len<10) then
        begin
      ...
      if (head[2] <> Z_DEFLATED) or ((head[3] and RESERVED) <> 0) then
        begin
          // Can not handle this
          errorcode := 3;
          abort
      ...
      if ((head[3] and EXTRA_FIELD) <> 0) then
        begin
          // the extra field
          blockRead(infile, len, 1);
          blockread(infile, dummy, 1);
          len := len + (dummy shl 8);
          if FileSize( infile) < int(len+12) then
            begin
              errorcode := 2;
              abort
            end;
      ...
      if ((head[3] and ORIG_NAME) <> 0) then
        begin
          // the original file name
          str := '';
          blockread( infile, ch, 1);
          while (ch <> char(0)) and not eof( infile) do
            begin
              str := str + ch;
              blockread( infile, ch, 1)
            end;
