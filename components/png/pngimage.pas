{Portable Network Graphics Delphi 2.0        (04 June 2012)   }

{           DISCLAIMER                                        }
{ This is a modified version of the original code written by: }
{ Gustavo Huffenbacher Daud                                   }
{ This code has become unavailable on the SourceForge website }
{ hence it was moved to a new project in SourceForge.         }
...
{http://pngdelphi2.sourceforge.net                            }
{Gustavo Huffenbacher Daud (gustavo.daud@terra.com.br)        }


{
  Version 2.0
  2012-06-04   Rym       - Fixed Delphi 2010 compatibility
                         - First public release since Gustavo Daud's release.
                         - The original library is no longer available officially
  Version 1.564
...
uses
 ...
 {zlibpas,} pngzlib, pnglang;

const
  LibraryVersion : AnsiString = '1.564';
...
type
  ...
  public
    ...
    destructor Destroy; override;
    {$IFNDEF UseDelphi}procedure LoadFromFile(const Filename: AnsiString);{$ENDIF}
    {$IFNDEF UseDelphi}procedure SaveToFile(const Filename: AnsiString);{$ENDIF}
    procedure LoadFromStream(Stream: TStream);
      {$IFDEF UseDelphi}override;{$ENDIF}
    procedure SaveToStream(Stream: TStream); {$IFDEF UseDelphi}override;{$ENDIF}
    {Loading the image from resources}
    procedure LoadFromResourceName(Instance: HInst; const Name: String); overload;
    procedure LoadFromResourceName(Instance: HInst; const Name, Section: String); overload;
...
{$IFNDEF UseDelphi}
  ...
  procedure TPngObject.LoadFromFile(const Filename: AnsiString);
  var
...
procedure TPngObject.LoadFromResourceName(Instance: HInst; const Name, Section: String);
var
  ResStream: TResourceStream;
begin
  {Creates an especial stream to load from the resource}
  try ResStream := TResourceStream.Create(Instance, Name, PAnsiChar(Section));
  except RaiseError(EPNGCouldNotLoadResource, EPNGCouldNotLoadResourceText);
  exit; end;
...
begin
  LoadFromResourceName(Instance, AnsiString(ResID));
end;
